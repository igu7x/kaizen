...
export function Layout({ children }: LayoutProps) {
  ...
  const isWhiteBgPage = isHomePage || isOKRPage || isExecucaoPage || isSprintsPage || isProcessosPage || isPainelIndicadoresPage || isComitesPage || isComiteDetalhe || isContratacoesPage || isCompetenciasPage || isPainelPessoas || isPerfilPage || isAdministracaoPage || isCadastrosPage || isContratosPage || isGerenciamentoPage;
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className={`flex flex-1 overflow-hidden ${isWhiteBgPage ? 'bg-white' : 'bg-[#0a2351]'}`}>
          ...
          {showDevBanner && (
            <div className="fixed top-[85px] left-1/2 -translate-x-1/2 z-10 flex items-center gap-3 bg-violet-600 rounded-b-xl px-4 py-1.5 shadow-lg shadow-violet-600/30">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-white">{devEnvironment}</span>
              </div>
              <button
                onClick={() => { setDevEnvironment(null); navigate('/desenvolvimento'); }}
