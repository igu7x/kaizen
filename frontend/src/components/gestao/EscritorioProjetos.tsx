...
export function EscritorioProjetos() {
  ...
  const [unidades, setUnidades] = useState<
    { id: number; nome_area: string; diretoria: string }[]
  >([]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const areas = await cadastrosProjetosApi.getAreas(
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        let projs: Projeto[];
        const instrId =
          selectedInstrumento !== "all" ? parseInt(selectedInstrumento) : null;
        if (instrId !== null) {
          projs = await cadastrosProjetosApi.getProjetosByInstrumentoId(
            instrId,
            dirFiltro,
          );
        } else {
  ...
  const stats = useMemo(
    () => ({
      ...
      concluidos: filteredProjetos.filter((p) => p.status === "concluido")
        .length,
      emAndamento: filteredProjetos.filter((p) => p.status === "em_execucao")
        .length,
      naoIniciado: filteredProjetos.filter((p) => p.status === "planejado")
        .length,
    }),
    [filteredProjetos],
  );
  ...
  const statusChartData = useMemo(
    () => [
      {
        name: "Planejado",
        value: filteredProjetos.filter((p) => p.status === "planejado").length,
      },
      {
        name: "Em Execução",
        value: filteredProjetos.filter((p) => p.status === "em_execucao")
          .length,
      },
      {
        name: "Suspenso",
        value: filteredProjetos.filter((p) => p.status === "suspenso").length,
      },
      {
        name: "Concluído",
        value: filteredProjetos.filter((p) => p.status === "concluido").length,
      },
    ],
    [filteredProjetos],
  );
  ...
  const saudeChartData = useMemo(
    () => [
      {
        name: "Saudável",
        value: filteredProjetos.filter((p) => p.saude === "verde").length,
        color: "#4ade80",
      },
      {
        name: "Atenção",
        value: filteredProjetos.filter((p) => p.saude === "amarelo").length,
        color: "#fbbf24",
      },
      {
        name: "Crítico",
        value: filteredProjetos.filter((p) => p.saude === "vermelho").length,
        color: "#f87171",
      },
    ],
    [filteredProjetos],
  ...
  const prioridadeChartData = useMemo(() => {
    const baixa = filteredProjetos.filter(
      (p) => p.prioridade === "baixa",
    ).length;
    const media = filteredProjetos.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <Select
            ...
            onValueChange={setSelectedInstrumento}
          >
        ...
        <p className="text-sm text-gray-400">
          {activeTab === "meus"
            ...
            : selectedInstrumento !== "all"
              ? instrumentos.find((i) => i.id === parseInt(selectedInstrumento))
                  ?.nome || "Filtrado"
              : "Portfólio Completo"}
          {activeTab !== "meus" &&
            selectedUnidade !== "all" &&
            ` — ${unidades.find((u) => u.id === parseInt(selectedUnidade))?.nome_area || ""}`}
        </p>
      </div>
      ...
      {!loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* Saúde Bar Chart */}
          <Card
            className="border-0 shadow-md rounded-lg flex flex-col"
            style={{ height: "400px" }}
          >
        ...
        {loading ? (
          ...
          <div>
            {filteredProjetos.map((projeto) => {
              ...
              const prioridade =
                prioridadeConfig[projeto.prioridade] || prioridadeConfig.media;
              return (
                <div
                  key={projeto.id}
                ...
                >
                  ...
                  <div className="hidden lg:flex items-center flex-shrink-0">
                    <div className="w-32 flex justify-center">
                      <Badge
                        className={`text-xs whitespace-nowrap ${statusBadgeColors[projeto.status] || "bg-gray-400 text-white"}`}
                      >
                    ...
                    <div className="w-28 flex items-center justify-center gap-1.5">
                      <div
                        className={`w-2 h-2 rounded-full ${saude.dotColor}`}
                      />
                      <span className={`text-sm ${saude.textColor}`}>
                        {saude.label}
                      </span>
                    </div>
                    <div className="w-24 text-center text-sm text-gray-600">
