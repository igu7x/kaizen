...
export function ProjetoFormDialog({
  ...
  const [validandoCamadaTAP, setValidandoCamadaTAP] = useState<number | null>(null);
  ...
  const [versoes, setVersoes] = useState<Array<{ versao: number; validado_em: string | null; validado_por_nome: string | null }>>([]);
  ...
  const [unidadesDiretorias, setUnidadesDiretorias] = useState<(Unidade & { diretoria_sigla?: string })[]>([]);
  ...
  const [editingEntregaIdx, setEditingEntregaIdx] = useState<number | null>(null);
  const [editEntregaDraft, setEditEntregaDraft] = useState({ nome: '', area_responsavel_id: null as number | null, prazo_estimado: '' });
  const [editEntregaAreaSearch, setEditEntregaAreaSearch] = useState('');
  const [showEditEntregaAreaDropdown, setShowEditEntregaAreaDropdown] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadAuxiliares = async () => {
      ...
      try {
        const instrumentosData = await planosProgramasApi.getInstrumentosParaAncoragem(diretoria);
        setInstrumentos(instrumentosData);
      } catch (e) {
        console.error('Erro ao carregar instrumentos:', e);
      }
      setAuxLoaded(true);
    };
    loadAuxiliares();
  ...
  useEffect(() => {
    ...
    const patrocinador = colaboradores.find(c => c.id === selectedProjeto.patrocinador_id);
    const gestor = colaboradores.find(c => c.id === selectedProjeto.gestor_id);
  ...
  const handleAddEntrega = () => {
    ...
    setNovaEntrega({ nome: '', area_responsavel_id: null, prazo_estimado: '' });
    toast({ title: 'Entrega adicionada', description: `"${novaEntrega.nome}" foi adicionada à lista` });
  };
  ...
  const handleValidarCamadaTAP = async (camada: number) => {
    ...
    try {
      ...
      toast({
        title: camada === 3 ? 'TAP Vigente! Validação concluída.' : `TAP validado — Camada ${camada} (${labels[camada]})`,
      });
      onSaved?.(updated);
    } catch (err: any) {
  ...
  return (
    <>
        ...
        <DialogContent className={cn(
            ...
            <DialogTitle className="text-xl font-bold flex items-center gap-3">
              <span>
                {mode === 'create'
                  ? 'Novo Projeto'
                  : mode === 'edit'
                    ? tapEditMode ? 'Editar TAP do Projeto' : 'Editar Projeto'
                    : 'Visualizar TAP'}
              </span>
              {selectedProjeto?.codigo && <span className="text-gray-500 font-mono text-sm">({selectedProjeto.codigo})</span>}
            </DialogTitle>
          </DialogHeader>
          ...
          {tapEditMode && mode === 'edit' && (
            <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              salvos. Alterações em outros campos serão ignoradas.
              {tapEditDiretoria ? <> Escopo: projetos da diretoria <strong>{tapEditDiretoria}</strong>.</> : null}
            </div>
          )}

          {/* Banner de TAP recusado — visível enquanto não há nova validação do gestor */}
          {selectedProjeto?.tap_recusado_em && !selectedProjeto?.tap_validado_gestor_em && (
            <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 flex items-start gap-3">
              <XCircle className="h-5 w-5 text-red-600 flex-shrink-0 mt-0.5" />
              <div className="flex-1 text-sm">
                <p className="font-semibold text-red-900">
                  TAP recusado pelo {selectedProjeto.tap_recusado_camada === 'patrocinador' ? 'Patrocinador' : 'Diretor'}
                </p>
                <p className="text-red-700 text-xs mt-0.5">
                  {selectedProjeto.tap_recusado_por_nome ? `Por ${selectedProjeto.tap_recusado_por_nome} — ` : ''}
                  {new Date(selectedProjeto.tap_recusado_em).toLocaleDateString('pt-BR')}
                </p>
                {selectedProjeto.tap_recusado_comentario && (
                  <p className="text-red-800 text-sm mt-2 italic">"{selectedProjeto.tap_recusado_comentario}"</p>
                )}
                <p className="text-red-600 text-xs mt-2">
                  O gestor deve ajustar o que for necessário e validar novamente para recomeçar o ciclo.
                </p>
              </div>
            </div>
          )}
          ...
          {slim && selectedProjeto && !selectedProjeto.tap_validado_patrocinador_em && (() => {
            const { valid, missingFields } = validateTAPFields(selectedProjeto);
            if (valid) return null;
            return (
              <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 flex items-start gap-3">
                <AlertCircle className="h-5 w-5 text-amber-600 flex-shrink-0 mt-0.5" />
                <div className="flex-1 text-sm">
                  <p className="font-semibold text-amber-900">Campos pendentes para gerar o TAP</p>
                  <p className="text-amber-700 text-xs mt-1">
                    Os campos abaixo precisam estar preenchidos no projeto para iniciar o ciclo de validação:
                  </p>
                  ...
                  <p className="text-amber-700 text-xs mt-3">
                    Use o botão <strong>Editar Projeto</strong> para preencher esses campos.
                  </p>
                </div>
              </div>
            );
            ...
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 text-sm">
                ...
                <p className="text-emerald-700 text-xs">
                  Validado em {new Date(selectedProjeto.tap_validado_patrocinador_em).toLocaleDateString('pt-BR')}
                  {selectedProjeto.tap_versao ? ` — versão ${selectedProjeto.tap_versao}` : ''}
                </p>
              </div>
              <Button
                size="sm"
              ...
              <Button
                ...
                variant="outline"
                className="border-emerald-500 text-emerald-700 hover:bg-emerald-100"
                onClick={() => selectedProjeto && void generateTAPPdf(selectedProjeto)}
              >
          ...
          {selectedProjeto && (slim || selectedProjeto.tap_validado_gestor_em || mode === 'view') && (
            <div className="bg-white border border-gray-200 rounded-lg p-3">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-xs font-semibold text-gray-700">Validação do TAP</h4>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className={cn(
                  ...
                  selectedProjeto.tap_validado_gestor_em ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
                )}>
                  <p className="text-[10px] text-gray-500 mb-0.5">Camada 1 — Gestor</p>
                  <p className="text-xs font-medium truncate">{selectedProjeto.gestor_nome || '-'}</p>
                  {selectedProjeto.tap_validado_gestor_em ? (
                    <div className="mt-1">
                      <Badge className="bg-green-600 text-white text-[10px] h-5">Validado</Badge>
                      <p className="text-[9px] text-gray-400 mt-0.5">
                        {new Date(selectedProjeto.tap_validado_gestor_em).toLocaleDateString('pt-BR')}
                      </p>
                    </div>
                  ) : currentUserId && selectedProjeto.gestor_user_id && Number(selectedProjeto.gestor_user_id) === currentUserId ? (
                    <Button
                      size="sm"
                    ...
                    <Badge variant="outline" className="text-[10px] text-gray-400 mt-1 h-5">Pendente</Badge>
                  )}
                </div>
                ...
                <div className={cn(
                  'flex-1 rounded-lg border p-2 text-center',
                  selectedProjeto.tap_validado_diretor_em ? 'bg-green-50 border-green-200'
                    : !selectedProjeto.tap_validado_gestor_em ? 'bg-gray-100 border-gray-200 opacity-50'
                    : 'bg-gray-50 border-gray-200'
                )}>
                  <p className="text-[10px] text-gray-500 mb-0.5">Camada 2 — Diretor</p>
                  <p className="text-xs font-medium truncate">{selectedProjeto.diretoria || '-'}</p>
                  {selectedProjeto.tap_validado_diretor_em ? (
                    <div className="mt-1">
                      <Badge className="bg-green-600 text-white text-[10px] h-5">Validado</Badge>
                      <p className="text-[9px] text-gray-400 mt-0.5">
                        {new Date(selectedProjeto.tap_validado_diretor_em).toLocaleDateString('pt-BR')}
                      </p>
                    </div>
                  ) : selectedProjeto.tap_validado_gestor_em && currentUserId && (selectedProjeto as any).diretor_user_id && Number((selectedProjeto as any).diretor_user_id) === currentUserId ? (
                    ...
                    <Badge variant="outline" className="text-[10px] text-gray-400 mt-1 h-5">
                      {selectedProjeto.tap_validado_gestor_em ? 'Pendente' : 'Bloqueado'}
                    </Badge>
                  )}
                </div>
                ...
                <div className={cn(
                  'flex-1 rounded-lg border p-2 text-center',
                  selectedProjeto.tap_validado_patrocinador_em ? 'bg-green-50 border-green-200'
                    : !selectedProjeto.tap_validado_diretor_em ? 'bg-gray-100 border-gray-200 opacity-50'
                    : 'bg-gray-50 border-gray-200'
                )}>
                  <p className="text-[10px] text-gray-500 mb-0.5">Camada 3 — Patrocinador</p>
                  <p className="text-xs font-medium truncate">{selectedProjeto.patrocinador_nome || '-'}</p>
                  {selectedProjeto.tap_validado_patrocinador_em ? (
                    <div className="mt-1">
                      <Badge className="bg-green-700 text-white text-[10px] h-5 cursor-pointer" onClick={() => void generateTAPPdf(selectedProjeto)}>
                        TAP Vigente
                      </Badge>
                      <p className="text-[9px] text-gray-400 mt-0.5">
                        {new Date(selectedProjeto.tap_validado_patrocinador_em).toLocaleDateString('pt-BR')}
                      </p>
                    </div>
                  ) : selectedProjeto.tap_validado_diretor_em && currentUserId && selectedProjeto.patrocinador_user_id && Number(selectedProjeto.patrocinador_user_id) === currentUserId ? (
                    <div className="mt-1 space-y-1">
                      <Button
                        size="sm"
                    ...
                    <Badge variant="outline" className="text-[10px] text-gray-400 mt-1 h-5">
                      {selectedProjeto.tap_validado_diretor_em ? 'Pendente' : 'Bloqueado'}
                    </Badge>
                  )}
                </div>
              </div>
          ...
          {slim && selectedProjeto?.tap_validado_patrocinador_em && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 flex items-start gap-3">
              ...
              <p className="text-sm text-blue-900">
                Para gerar uma <strong>nova versão</strong> do TAP, altere as informações do projeto pelo botão
                {' '}<strong>Editar Projeto</strong>. As mudanças vão reiniciar o ciclo de validação (gestor → diretor → patrocinador).
              </p>
            </div>
          )}
          ...
          <div className="space-y-4">
            <Accordion type="multiple" defaultValue={['identificacao', 'governanca', 'escopo', 'temporalidade', 'classificacao', 'execucao', 'entregas', 'formalizacao']} className="w-full">

              {/* IDENTIFICAÇÃO */}
              <AccordionItem value="identificacao">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><FolderKanban className="h-4 w-4" />Identificação do Projeto</div>
                </AccordionTrigger>
              ...
              <AccordionItem value="governanca">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><Users className="h-4 w-4" />Governança e Responsáveis</div>
                </AccordionTrigger>
                <AccordionContent className="p-4 border border-t-0 rounded-b">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative">
                      ...
                      {showPatrocinadorList && buscaPatrocinador.length > 0 && (
                        <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg max-h-48 overflow-y-auto">
                          ...
                          {filteredColaboradores.filter(c => c.nome.toLowerCase().includes(buscaPatrocinador.toLowerCase())).length === 0 && (
                    ...
                    <div className="relative">
                      ...
                      {showGestorList && buscaGestor.length > 0 && (
                        <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg max-h-48 overflow-y-auto">
                          ...
                          {filteredColaboradores.filter(c => c.nome.toLowerCase().includes(buscaGestor.toLowerCase())).length === 0 && (
                            <div className="px-3 py-2 text-sm text-gray-500">Nenhum resultado</div>
                          )}
                        </div>
                      )}
                    ...
                    <div className="md:col-span-2">
                          ...
                          {formData.areas_vinculadas_ids.map(id => {
                            ...
                            return (
                              <Badge key={id} variant="secondary" className="bg-blue-100 text-blue-800 px-3 py-1.5 text-sm gap-2">
                                {dir.sigla} - {dir.nome}
                                {mode !== 'view' && (
                                  <button type="button" onClick={() => handleRemoverDiretoria(id)} className="ml-1 hover:text-red-600 transition-colors">
                      ...
                      {mode !== 'view' && (
                        <Select value="" onValueChange={handleAdicionarDiretoria}>
                    ...
                    {formData.areas_vinculadas_ids && formData.areas_vinculadas_ids.length > 0 && (
                      <div className="md:col-span-2 animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        {mode !== 'view' && unidadesDiretorias.length > 0 && (
                          <Select
                            value=""
                            onValueChange={(value) => {
                        ...
                        {mode === 'view' && (!formData.areas_execucao || formData.areas_execucao.length === 0) && (
                    ...
                    <div className="md:col-span-2">
                      <Label className={isTapFieldMissing('instrumentos') ? 'text-red-600 font-semibold mb-2 block' : 'text-blue-700 font-semibold mb-2 block'}>
                      ...
                      {mode !== 'view' && (
                        ...
                        >
                          <SelectTrigger className={cn('w-full bg-white border-gray-300 border-dashed', isTapFieldMissing('instrumentos') ? 'border-red-500' : '')}>
                            <div className="flex items-center gap-2 text-gray-500">
                              <Plus className="h-4 w-4" />
              ...
              <AccordionItem value="escopo">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><Target className="h-4 w-4" />Escopo</div>
                </AccordionTrigger>
                <AccordionContent className="p-4 border border-t-0 rounded-b">
                  <div className="space-y-4">
                    <div>
              ...
              <AccordionItem value="temporalidade">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><Calendar className="h-4 w-4" />Temporalidade e Situação</div>
                </AccordionTrigger>
                <AccordionContent className="p-4 border border-t-0 rounded-b">
              ...
              <AccordionItem value="classificacao">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><Target className="h-4 w-4" />Classificação para Gestão do Portfólio</div>
                </AccordionTrigger>
                <AccordionContent className="p-4 border border-t-0 rounded-b">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        disabled={mode === 'view'}
                      >
                        <SelectTrigger className={isTapFieldMissing('prioridade') ? 'border-red-500' : ''}><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        disabled={mode === 'view'}
                      >
                        <SelectTrigger className={isTapFieldMissing('complexidade') ? 'border-red-500' : ''}><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="baixa">Baixa</SelectItem>
                          <SelectItem value="media">Média</SelectItem>
                          <SelectItem value="alta">Alta</SelectItem>
              ...
              <AccordionItem value="execucao">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4" />Execução e Monitoramento</div>
                </AccordionTrigger>
                <AccordionContent className="p-4 border border-t-0 rounded-b">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="verde"><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-green-500" />Saudável (Verde)</div></SelectItem>
                          <SelectItem value="amarelo"><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-yellow-500" />Atenção (Amarelo)</div></SelectItem>
                          <SelectItem value="vermelho"><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500" />Crítico (Vermelho)</div></SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
                ...
                <AccordionContent className="p-4 border border-t-0 rounded-b">
                  <div className="space-y-3">
                    {tempEntregas.length === 0 ? (
                      <p className="text-sm text-gray-500 italic p-3 bg-gray-50 rounded">Nenhuma entrega cadastrada. {mode !== 'view' && 'Adicione usando o formulário abaixo.'}</p>
                    ) : (
                    ...
                    {tempEntregas.map((entrega, idx) => {
                      ...
                      if (editingThis && mode !== 'view') {
                        const areaSelected = areas.find(a => a.id === editEntregaDraft.area_responsavel_id);
                        return (
                          <div key={idx} className="p-3 border-2 border-blue-300 rounded-lg bg-blue-50 space-y-2">
                            <div className="flex gap-2 items-center flex-wrap">
                              <Input
                                placeholder="Nome da entrega"
                                value={editEntregaDraft.nome}
                                onChange={(e) => setEditEntregaDraft({ ...editEntregaDraft, nome: e.target.value })}
                                className="flex-1 bg-white min-w-[200px]"
                              />
                              <Input
                                type="date"
                                value={editEntregaDraft.prazo_estimado || ''}
                                onChange={(e) => setEditEntregaDraft({ ...editEntregaDraft, prazo_estimado: e.target.value })}
                            ...
                            <div className="relative">
                              <Input
                                ...
                                onFocus={() => setShowEditEntregaAreaDropdown(true)}
                                className="bg-white"
                              />
                              {areaSelected && (
                                <div className="mt-1">
                                  <Badge variant="secondary" className="bg-blue-100 text-blue-800 gap-1.5">
                                    {areaSelected.diretoria} - {areaSelected.nome_area}
                                    <button type="button" onClick={() => { setEditEntregaDraft({ ...editEntregaDraft, area_responsavel_id: null }); setEditEntregaAreaSearch(''); }} className="hover:text-red-600">
                                      <XCircle className="h-3 w-3" />
                                    </button>
                                  </Badge>
                                </div>
                              )}
                              {showEditEntregaAreaDropdown && editEntregaAreaSearch.trim().length > 0 && (
                                <div className="absolute z-50 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                  {areas
                                    .filter(a => {
                                      const term = editEntregaAreaSearch.toLowerCase();
                                    ...
                                    .map(area => (
                                      <button
                                        ...
                                        onClick={() => {
                                          setEditEntregaDraft({ ...editEntregaDraft, area_responsavel_id: area.id });
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              <Button size="sm" onClick={() => {
                                ...
                                setTempEntregas(prev => prev.map((e, i) => i === idx ? {
                                  ...e,
                                  nome: editEntregaDraft.nome.trim(),
                                  area_responsavel_id: editEntregaDraft.area_responsavel_id,
                                  prazo_estimado: editEntregaDraft.prazo_estimado || null,
                                } : e));
                                setEditingEntregaIdx(null);
                                setEditEntregaAreaSearch('');
                                setShowEditEntregaAreaDropdown(false);
                              ...
                              <>
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setEditEntregaDraft({
                                      nome: entrega.nome,
                                      area_responsavel_id: entrega.area_responsavel_id ?? null,
                                      prazo_estimado: entrega.prazo_estimado ? String(entrega.prazo_estimado).slice(0, 10) : '',
                                    });
                                    setEditEntregaAreaSearch('');
                                    setShowEditEntregaAreaDropdown(false);
                                  }}
                                ...
                                <Button variant="ghost" size="icon" onClick={() => setTempEntregas(tempEntregas.filter((_, i) => i !== idx))} title="Remover entrega">
              ...
              <AccordionItem value="formalizacao">
                <AccordionTrigger className="bg-amber-50 px-4 rounded-t">
                  <div className="flex items-center gap-2"><FileText className="h-4 w-4" />Formalização</div>
                </AccordionTrigger>
                <AccordionContent className="p-4 border border-t-0 rounded-b">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
          ...
          <DialogFooter className="mt-4">
            ...
            {!slim && mode === 'view' && selectedProjeto && (
              <Button onClick={() => onModeChange('edit')} className="bg-blue-500 hover:bg-blue-600">
      ...
      <Dialog open={versoesDialogOpen} onOpenChange={(o) => { if (!o) setVersoesDialogOpen(false); }}>
        <DialogContent className="sm:max-w-md">
          ...
          {loadingVersoes ? (
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              {versoes.map(v => (
                <div key={v.versao} className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                  <div>
                    <span className="font-semibold text-emerald-700 font-mono">v{v.versao}</span>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {v.validado_em ? `Validado em ${new Date(v.validado_em).toLocaleDateString('pt-BR')}` : '—'}
      ...
      <Dialog open={recusaDialogOpen} onOpenChange={(open) => {
        if (!open) { setRecusaDialogOpen(false); setRecusaCamada(null); setRecusaComentario(''); }
      }}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-3 py-2">
            <p className="text-sm text-gray-600">
              O TAP voltará para o gestor reiniciar o ciclo de validação. Todas as validações
              anteriores serão resetadas. Você pode incluir um comentário (opcional) para indicar
              o que precisa ser ajustado.
            </p>
            <div>
              <Label htmlFor="recusa-tap-comentario">Comentário (opcional)</Label>
              <Textarea
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRecusaDialogOpen(false)} disabled={recusandoTAP}>
              Cancelar
            </Button>
            <Button onClick={handleRecusarTAP} disabled={recusandoTAP} className="bg-red-600 hover:bg-red-700 text-white">
              {recusandoTAP ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <XCircle className="h-4 w-4 mr-2" />}
              Recusar TAP
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="max-w-md">
          {tapMissingFields.length > 0 ? (
            <>
              ...
              <DialogFooter>
                <Button onClick={() => setTapValidationDialogOpen(false)}>Entendi</Button>
              </DialogFooter>
            </>
          ) : (
            <>
              ...
              <DialogFooter>
                <Button onClick={() => setTapValidationDialogOpen(false)}>OK</Button>
              </DialogFooter>
            </>
          )}
        </DialogContent>
