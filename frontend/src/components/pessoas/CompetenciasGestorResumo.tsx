...
import { FormularioCompetencias, CompetenciaItem, competenciasGestorApi } from '@/services/competenciasGestorApi';
...
import { COMPETENCIAS_COMPORTAMENTAIS, COMPETENCIAS_ESTRATEGICAS, COMPETENCIAS_GERENCIAIS } from '@/constants/competencias';
...
const pesoLabels: Record<number, string> = { 1: 'Útil', 2: 'Importante', 3: 'Crítica' };
...
export function CompetenciasGestorResumo({ formulario, onValidated, onEdit }: CompetenciasGestorResumoProps) {
  const { user } = useAuth();
  const [validating, setValidating] = useState(false);
  const [recusaDialogOpen, setRecusaDialogOpen] = useState(false);
  const [recusaCamada, setRecusaCamada] = useState<'diretoria' | 'final' | null>(null);
  const [recusaComentario, setRecusaComentario] = useState('');
  const [areas, setAreas] = useState<Area[]>([]);
  const [compNomesComportamentais, setCompNomesComportamentais] = useState(() => new Set(COMPETENCIAS_COMPORTAMENTAIS.map(c => c.nome)));
  const [compNomesEstrategicas, setCompNomesEstrategicas] = useState(() => new Set(COMPETENCIAS_ESTRATEGICAS.map(c => c.nome)));
  const [compNomesGerenciais, setCompNomesGerenciais] = useState(() => new Set(COMPETENCIAS_GERENCIAIS.map(c => c.nome)));
  // Mapa de nome → descrição atualizada (para sobrescrever descrições antigas armazenadas no formulário)
  const [compDescricaoAtual, setCompDescricaoAtual] = useState<Map<string, string>>(new Map());
  ...
  useEffect(() => {
    ...
    competenciasPadraoApi.getAll().then(data => {
      ...
      if (data.comportamental?.length) {
        setCompNomesComportamentais(new Set(data.comportamental.map(c => c.nome)));
        data.comportamental.forEach(c => descMap.set(c.nome, c.descricao));
      }
      if (data.estrategica?.length) {
        setCompNomesEstrategicas(new Set(data.estrategica.map(c => c.nome)));
  ...
  const withCurrentDesc = (items: typeof allComps) =>
    items.map(c => compDescricaoAtual.has(c.nome) ? { ...c, descricao: compDescricaoAtual.get(c.nome)! } : c);

  const allComps = formulario.competencias || [];
  const comportamentais = withCurrentDesc(allComps.filter(c => nomesComportamentais.has(c.nome)));
  const estrategicas = withCurrentDesc(allComps.filter(c => nomesEstrategicas.has(c.nome)));
  const gerenciais = withCurrentDesc(allComps.filter(c => nomesGerenciais.has(c.nome)));
  const tecnicas = allComps.filter(c =>
  ...
  const areaForm = areas.find(a => (a.sigla || '').toUpperCase() === formDiretoria.toUpperCase());
  const subdiretorId = areaForm ? Number((areaForm as any).subdiretor_user_id || 0) : 0;
  const autorUserId = Number((formulario as any).user_id || 0);
  const preenchidoPorSubdiretor = isGestor && subdiretorId > 0 && autorUserId === subdiretorId;

  // Camada Diretoria: usuário precisa ser o gestor da diretoria (cadastros_areas.gestor_user_id)
  const isGestorDaDiretoria = !!(userId && areaForm?.gestor_user_id && Number(areaForm.gestor_user_id) === userId);
  ...
  const statusParaDiretoria = (isGestor && !preenchidoPorSubdiretor) ? 'enviado' : 'validado_autor';
  const canValidateDiretoria = formulario.status === statusParaDiretoria && isGestorDaDiretoria;

  // Camada Final
  const canValidateFinal = formulario.status === 'validado_diretoria' && isValidadorFinal(userEmail);
  ...
  const isAutor = !!formulario.email_institucional && userEmail === formulario.email_institucional.toLowerCase().trim();
  const isSubdiretorValidando = preenchidoPorSubdiretor && userId === subdiretorId;
  const canValidateAutor = formulario.status === 'enviado' && ((!isGestor && isAutor) || isSubdiretorValidando);
  ...
  const foiRecusado = !!formulario.recusado_em && formulario.status === 'enviado';
  const podeEditar = (() => {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <ValidacaoStatusBanners formulario={formulario} preenchidoPorSubdiretor={preenchidoPorSubdiretor} />
      ...
      <Dialog open={recusaDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              O formulário voltará para o autor preservando todas as respostas. Ele poderá ajustar o que for necessário e validar novamente.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => { setRecusaDialogOpen(false); setRecusaCamada(null); setRecusaComentario(''); }}
              disabled={validating}
            >
              Cancelar
            </Button>
      ...
      {formulario.status === 'validado_final' && (
        <div className="flex justify-end">
          <Button
            onClick={() => { void generateCompetenciasPDF(formulario); }}
      ...
      <Card className="border border-gray-200 shadow-sm">
        ...
        <CardContent>
          <div className={`grid grid-cols-1 ${formulario.tipo === 'gestor' ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-4`}>
            <Field label="Diretoria" value={formulario.diretoria} />
            <Field label="Unidade" value={formulario.unidade_nome || '-'} />
            {formulario.tipo !== 'gestor' && (
              <Field label="Qtd. de colaboradores" value={String(formulario.qtd_colaboradores)} />
            )}
          </div>
        </CardContent>
      </Card>
...
function CompetenciaSection({ title, items, showPeso, showAplicabilidade, accentColor = 'blue' }: {
  title: string;
  ...
  return (
    <div className="space-y-4">
      ...
      {items.map((comp, index) => (
        <Card
          key={index}
          className={`shadow-sm ${comp.alterada
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between gap-2">
              <CardTitle className="text-base flex items-center gap-2 flex-1 min-w-0">
                <span className={`${accentColors[accentColor] || 'text-blue-600'} font-bold flex-shrink-0`}>{index + 1}.</span>
              ...
              {showPeso && (
                <Badge className={pesoColors[comp.peso] || 'bg-gray-100 text-gray-700'}>
          ...
          <CardContent>
            <div className="space-y-3">
              <div>
                ...
                <p className="text-gray-800 mt-0.5 whitespace-pre-wrap [overflow-wrap:anywhere]">{comp.descricao}</p>
              </div>
              {showAplicabilidade && (
                <div className="flex gap-6">
                  <div>
                    <span className="text-sm text-gray-500">Aplicabilidade</span>
                    <p className="text-gray-800 mt-0.5 font-medium">
                      {comp.aplicabilidade === 'todos' ? 'Todos os colaboradores' : 'Parte da equipe'}
                    </p>
                  </div>
                  {comp.aplicabilidade === 'parte' && comp.quantidade_pessoas && (
                    <div>
                      <span className="text-sm text-gray-500">Quantidade</span>
                      <p className="text-gray-800 mt-0.5 font-medium">{comp.quantidade_pessoas} colaboradores</p>
                    </div>
                  )}
                </div>
              )}
