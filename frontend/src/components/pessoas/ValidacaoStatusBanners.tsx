...
import { CheckCircle2, Clock, Lock, AlertCircle, XCircle, Eye, Loader2, Plus, Pencil, Minus } from 'lucide-react';
...
import { competenciasPadraoApi, CompetenciaPadrao } from '@/services/competenciasPadraoApi';
...
const STATUS_ORDER_EQUIPE = ['enviado', 'validado_autor', 'validado_diretoria', 'validado_final'];
const STATUS_ORDER_GESTOR = ['enviado', 'validado_diretoria', 'validado_final'];
const STATUS_ORDER_GESTOR_SUBDIRETOR = ['enviado', 'validado_autor', 'validado_diretoria', 'validado_final'];
...
export function ValidacaoStatusBanners({ formulario, preenchidoPorSubdiretor }: ValidacaoStatusBannersProps) {
  const [mudancasOpen, setMudancasOpen] = useState(false);
  const isGestor = formulario.tipo === 'gestor';
  const statusOrder = isGestor
    ? (preenchidoPorSubdiretor ? STATUS_ORDER_GESTOR_SUBDIRETOR : STATUS_ORDER_GESTOR)
    : STATUS_ORDER_EQUIPE;
  const statusIdx = getStatusIndex(formulario.status, statusOrder);

  const layers = isGestor && preenchidoPorSubdiretor
    ? [
        {
  ...
  const tiposAfetados = Array.isArray(formulario.padroes_tipos_afetados)
    ? formulario.padroes_tipos_afetados.map(t => tiposLabel[t] || t).join(', ')
    : '';

  // Banner de recusa: aparece se houver registro de recusa pendente
  // (formulário voltou para 'enviado' após uma recusa)
  const mostrarRecusa = !!formulario.recusado_em && formulario.status === 'enviado';
  const camadaRecusa = formulario.recusado_camada === 'final'
    ? 'Validação Final'
  ...
  return (
    <div className="space-y-2">
      ...
      {mostrarRecusa && (
        <div className="flex items-start gap-3 rounded-lg bg-red-50 border border-red-300 px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-red-900">Formulário recusado pela {camadaRecusa}</p>
            <p className="text-xs text-red-700 mt-0.5">
              Recusado por {formulario.recusado_por_nome || 'validador'}
              {formulario.recusado_em ? `, em ${formatDate(formulario.recusado_em)}` : ''}.
            ...
            {formulario.recusado_comentario && (
              <p className="text-sm text-red-800 mt-2 whitespace-pre-wrap [overflow-wrap:anywhere] bg-white/70 border border-red-200 rounded-md px-3 py-2">
                <span className="font-medium">Comentário:</span> {formulario.recusado_comentario}
              </p>
            )}
          </div>
        </div>
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-purple-900">Mudanças em competências padrão</p>
                <p className="text-xs text-purple-700 mt-0.5">
                  {tiposAfetados
                    ? `As competências padrão (${tiposAfetados}) foram atualizadas. `
                    : 'As competências padrão foram atualizadas. '}
                  Como elas constam neste referencial, ele precisa passar novamente pelas camadas de validação para ter sua versão atualizada.
                </p>
              </div>
              <Button
                size="sm"
      ...
      <MudancasPadraoDialog
        ...
        tiposAfetados={Array.isArray(formulario.padroes_tipos_afetados) ? formulario.padroes_tipos_afetados : []}
      />
      ...
      {layers.map((layer, i) => {
        if (layer.done) {
          return (
            <div key={i} className="flex items-center gap-3 rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-3">
              <CheckCircle2 className="h-5 w-5 text-emerald-600 flex-shrink-0" />
              <div>
                <p className="text-sm font-medium text-emerald-800">{layer.label}</p>
                <p className="text-xs text-emerald-600">
                  Validado por {layer.nome || 'usuário'}, em {layer.data ? formatDate(layer.data) : '-'}
                </p>
              </div>
            </div>
          );
        ...
        if (layer.pending) {
          return (
            <div key={i} className="flex items-center gap-3 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
              <Clock className="h-5 w-5 text-amber-600 flex-shrink-0" />
              <div>
                <p className="text-sm font-medium text-amber-800">{layer.label}</p>
        ...
        return (
          <div key={i} className="flex items-center gap-3 rounded-lg bg-gray-50 border border-gray-200 px-4 py-3">
...
function MudancasPadraoDialog({ open, onClose, tiposAfetados }: {
  open: boolean;
  onClose: () => void;
  ...
  const [detalhesPorId, setDetalhesPorId] = useState<Map<number, CompetenciaPadrao>>(new Map());
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        [...(all.comportamental || []), ...(all.estrategica || []), ...(all.gerencial || [])]
          .forEach(c => map.set(c.id, c));
        setDetalhesPorId(map);
      } catch (err) {
        console.error('Erro ao carregar mudanças do catálogo padrão:', err);
      } finally {
  ...
  const mudancasFiltradas = mudancas.filter(m => tiposAfetadosSet.size === 0 || tiposAfetadosSet.has(m.tipo));
  const totalMudancas = mudancasFiltradas.reduce(
    (acc, m) => acc + m.adicionadas.length + m.alteradas.length + m.removidas.length,
    0
  );
      ...
      <DialogContent className="sm:max-w-[680px] max-h-[85vh] overflow-y-auto">
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 py-8 text-center">
            Nenhuma alteração encontrada para os tipos afetados por este referencial.
          </p>
        ) : (
          <div className="space-y-6 py-2">
            {mudancasFiltradas.map(m => {
              const totalDoTipo = m.adicionadas.length + m.alteradas.length + m.removidas.length;
                  ...
                  {m.adicionadas.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-emerald-700 flex items-center gap-1">
                        <Plus className="h-3 w-3" /> Adicionadas ({m.adicionadas.length})
                      </p>
                      {m.adicionadas.map(a => {
                        const det = detalhesPorId.get(a.id);
                        return (
                          <div key={`add-${a.id}`} className="rounded-md border border-emerald-200 bg-emerald-50/50 px-3 py-2">
                            <p className="text-sm font-medium text-emerald-900">{det?.nome || a.nome}</p>
                            {det?.descricao && (
                              <p className="text-xs text-emerald-800/80 mt-1 [overflow-wrap:anywhere]">{det.descricao}</p>
                            )}
                          </div>
                        );
                      })}
                  ...
                  {m.alteradas.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-amber-700 flex items-center gap-1">
                        <Pencil className="h-3 w-3" /> Alteradas ({m.alteradas.length})
                      </p>
                      {m.alteradas.map(a => {
                        const det = detalhesPorId.get(a.id);
                        return (
                          <div key={`alt-${a.id}`} className="rounded-md border border-amber-200 bg-amber-50/60 px-3 py-2">
                            <p className="text-sm font-medium text-amber-900">{det?.nome || a.nome}</p>
                            {det?.descricao && (
                              <p className="text-xs text-amber-800/80 mt-1 [overflow-wrap:anywhere]">{det.descricao}</p>
                            )}
                          </div>
                        );
                      })}
                  ...
                  {m.removidas.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-gray-600 flex items-center gap-1">
                        <Minus className="h-3 w-3" /> Removidas ({m.removidas.length})
                      </p>
                      {m.removidas.map(r => (
                        <div key={`rem-${r.id}`} className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                          <p className="text-sm font-medium text-gray-700 line-through">{r.nome}</p>
                        </div>
                      ))}
                    </div>
                  )}
