...
export function Breadcrumbs({ items, className = '', showHome = true }: BreadcrumbsProps) {
  ...
  return (
    <nav aria-label="Breadcrumb" className={`flex items-center text-sm text-gray-500 flex-wrap gap-y-1 ${className}`}>
      {showHome && (
        <>
          <Link to="/" className="inline-flex items-center gap-1 hover:text-gray-800 transition-colors">
          ...
          <div key={index} className="inline-flex items-center">
            {isInteractive ? (
              item.onClick ? (
                ...
                <Link to={item.to!} className="hover:text-gray-800 transition-colors">
                  {item.label}
                </Link>
              )
            ) : (
            ...
            {!isLast && <ChevronRight className="h-3.5 w-3.5 mx-1.5 text-gray-400 flex-shrink-0" />}
          </div>
        );
      })}
    </nav>
