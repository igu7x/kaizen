...
function pesoColors(peso: number) {
  if (peso === 3) return { bg: PESO_CRITICA_BG, text: PESO_CRITICA_TEXT, bar: PESO_CRITICA_BAR };
  if (peso === 2) return { bg: PESO_IMPORTANTE_BG, text: PESO_IMPORTANTE_TEXT, bar: PESO_IMPORTANTE_BAR };
...
function drawCompetenciaCard(
  ...
  const { numero, nome, descricao, peso, aplicabilidade, quantidade, showAplicabilidade } = options;
  ...
  if (showAplicabilidade) {
    ...
    const labelColW = 32;
    const aplicText = aplicabilidade === 'parte' ? 'Parte da equipe' : 'Todos os colaboradores';
    ...
    if (aplicabilidade === 'parte' && quantidade) {
      ...
      doc.setDrawColor(...BORDER_GRAY);
      doc.setLineWidth(0.2);
      doc.line(MARGIN_LEFT + cellAplicLabelW, apY, MARGIN_LEFT + cellAplicLabelW, apY + aplicRowH);
      doc.line(MARGIN_LEFT + cellAplicLabelW + cellAplicValueW, apY, MARGIN_LEFT + cellAplicLabelW + cellAplicValueW, apY + aplicRowH);
      doc.line(MARGIN_LEFT + cellAplicLabelW + cellAplicValueW + cellQtdLabelW, apY, MARGIN_LEFT + cellAplicLabelW + cellAplicValueW + cellQtdLabelW, apY + aplicRowH);
    } else {
      ...
      doc.setDrawColor(...BORDER_GRAY);
      doc.setLineWidth(0.2);
      doc.line(MARGIN_LEFT + labelColW, apY, MARGIN_LEFT + labelColW, apY + aplicRowH);
    }
...
export async function generateCompetenciasPDF(formulario: FormularioCompetencias) {
  ...
  let comportamentaisPadrao = COMPETENCIAS_COMPORTAMENTAIS.map(c => ({ nome: c.nome, descricao: c.descricao }));
  let estrategicasPadrao = COMPETENCIAS_ESTRATEGICAS.map(c => ({ nome: c.nome, descricao: c.descricao }));
  let gerenciaisPadrao = COMPETENCIAS_GERENCIAIS.map(c => ({ nome: c.nome, descricao: c.descricao }));
  ...
  if (padroesSnapshot
    ...
    if (padroesSnapshot.comportamental?.length) comportamentaisPadrao = padroesSnapshot.comportamental.map(c => ({ nome: c.nome, descricao: c.descricao }));
    if (padroesSnapshot.estrategica?.length)    estrategicasPadrao    = padroesSnapshot.estrategica.map(c => ({ nome: c.nome, descricao: c.descricao }));
    if (padroesSnapshot.gerencial?.length)      gerenciaisPadrao      = padroesSnapshot.gerencial.map(c => ({ nome: c.nome, descricao: c.descricao }));
  } else {
    try {
      const data = await competenciasPadraoApi.getAll();
      if (data.comportamental?.length) comportamentaisPadrao = data.comportamental.map(c => ({ nome: c.nome, descricao: c.descricao }));
      if (data.estrategica?.length) estrategicasPadrao = data.estrategica.map(c => ({ nome: c.nome, descricao: c.descricao }));
      if (data.gerencial?.length) gerenciaisPadrao = data.gerencial.map(c => ({ nome: c.nome, descricao: c.descricao }));
    } catch {
      // mantém fallback das constantes
    }
  }
  ...
  doc.text('Matriz de Competências', MARGIN_LEFT + 5, headerY + 18, { maxWidth: leftMaxW });

  const tipoLabel = formulario.tipo === 'gestor' ? 'Matriz de Competências do Gestor' : 'Matriz de Competências da Equipe';
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(255, 255, 255);
  ...
  const validacaoDate = formulario.validado_final_em
    ...
    : formulario.validado_por_diretoria_em
      ...
      : '';
  const statusText = validacaoDate ? `Status: Validado em ${validacaoDate}` : `Status: ${formulario.status || 'Enviado'}`;
  doc.setFontSize(9);
  doc.setTextColor(180, 195, 220);
  doc.text(statusText, MARGIN_LEFT + 5, lotacaoEndY + 8, { maxWidth: leftMaxW });
  ...
  const brasaoW = 18;
  const brasaoH = 22;
  doc.addImage(LOGO_BRANCO_4K_BASE64, 'PNG', rightCenterX - brasaoW / 2, headerY + 10, brasaoW, brasaoH, undefined, 'FAST');
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(255, 255, 255);
  doc.text('Tribunal de Justiça do Estado de Goiás', rightCenterX, headerY + 43, { align: 'center' });
  ...
  y = drawSectionTitleBar(doc, `Competências Técnicas (${competencias.length})`, y);
  ...
  // ============================================================
  function renderFixedSection(title: string, items: { nome: string; descricao: string }[]) {
  ...
  renderFixedSection(`Competências Comportamentais (${comportamentaisPadrao.length})`, comportamentaisPadrao);

  if (formulario.tipo === 'gestor') {
    renderFixedSection(`Competências Estratégicas (${estrategicasPadrao.length})`, estrategicasPadrao);
    renderFixedSection(`Competências Gerenciais (${gerenciaisPadrao.length})`, gerenciaisPadrao);
  }
  ...
  const versionPart = formulario.versao_formulario && formulario.versao_formulario > 0
