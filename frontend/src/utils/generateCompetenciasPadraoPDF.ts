...
function drawTextCell(doc: jsPDF, text: string, x: number, y: number, w: number, h: number, options?: { bold?: boolean; fontSize?: number; align?: 'left' | 'center' | 'right'; bg?: readonly [number, number, number] }) {
  const fontSize = options?.fontSize || 9;
  const align = options?.align || 'left';
  if (options?.bg) { doc.setFillColor(...options.bg); doc.rect(x, y, w, h, 'F'); }
...
function drawHeader(doc: jsPDF, tipoFormulario: 'equipe' | 'gestor') {
  ...
  doc.text('Competências Padrão', MARGIN_LEFT + 5, headerY + 18, { maxWidth: leftMaxW });

  const subtitulo = tipoFormulario === 'gestor'
  ...
  doc.text('Documento somente leitura', MARGIN_LEFT + 5, headerY + 36, { maxWidth: leftMaxW });

  doc.setFontSize(9);
  doc.setTextColor(180, 195, 220);
  doc.text(`Gerado em ${formatDate(new Date())}`, MARGIN_LEFT + 5, headerY + 44, { maxWidth: leftMaxW });
...
function renderSecao(doc: jsPDF, yStart: number, titulo: string, items: CompetenciaPadrao[]): number {
  let y = yStart + 16;
  y = checkPageBreak(doc, y, 20);
  y = drawSectionHeader(doc, `${titulo} (${items.length})`, y);

  if (items.length === 0) {
    drawTextCell(doc, 'Nenhuma competência cadastrada.', MARGIN_LEFT, y, CONTENT_WIDTH, 8, { fontSize: 9, align: 'center' });
    return y + 8;
  }
  ...
  for (let i = 0; i < items.length; i++) {
    const comp = items[i];
    doc.setFontSize(9);
    const descLines = doc.splitTextToSize(comp.descricao || '', CONTENT_WIDTH - 6);
    ...
    // Label "Descrição"
    const descRowH = 6;
    drawTextCell(doc, 'Descrição', MARGIN_LEFT, y, 30, descRowH, { fontSize: 8, bg: GRAY_LIGHT });
    y += descRowH;
...
export function generateCompetenciasPadraoPDF(opts: GerarOptions) {
  const { tipoFormulario, comportamentais, estrategicas, gerenciais, versaoAtual } = opts;
  ...
  const tipoLabel = tipoFormulario === 'gestor' ? 'Formulário do Gestor' : 'Formulário da Equipe';
  const versaoPart = versaoAtual && versaoAtual > 0 ? `Catálogo v${versaoAtual}` : '';
  const footerLabel = [`Competências Padrão — ${tipoLabel}`, versaoPart].filter(Boolean).join(' — ');
