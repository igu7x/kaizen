...
const colorPresets = [
    ...
    '#2E7D32', '#558B2F', '#F57F17', '#EF6C00',
    '#D84315', '#AD1457', '#6A1B9A', '#4527A0',
    '#283593', '#37474F',
];
...
export default function Comites() {
    ...
    const handleEditComite = (e: React.MouseEvent, comite: Comite) => {
        ...
        setNewComite({ nome: comite.nome, sigla: comite.sigla, descricao: comite.descricao || '', cor: comite.cor || '#1565C0', icone: comite.icone || 'megafone' });
        setEditDialogOpen(true);
    };

    const handleSaveEdit = async () => {
        if (!editingComite || !newComite.nome.trim() || !newComite.sigla.trim()) return;
        try {
            setCreating(true);
            ...
            setNewComite({ nome: '', sigla: '', descricao: '', cor: '#1565C0', icone: 'megafone' });
            await loadComites();
        } catch (err: any) {
            console.error('Erro ao editar comitê:', err);
        } finally {
            setCreating(false);
        }
    };
    ...
    const handleDeleteComite = async (e: React.MouseEvent, comite: Comite) => {
        ...
        try {
            ...
            console.error('Erro ao excluir comitê:', err);
        }
    };
    ...
    const handleCreateComite = async () => {
        ...
        try {
            ...
            console.error('Erro ao criar comitê:', err);
                ...
                <div className="mx-auto px-6 py-8 max-w-[1600px]">
                    <div className="mb-10 flex items-center justify-between">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900 leading-tight">Comitês</h2>
                                <p className="text-sm text-gray-600 mt-1">Selecione um comitê para acessar</p>
                            </div>
                        </div>
                                ...
                                <DialogContent className="sm:max-w-[500px]">
                                    ...
                                    <div className="grid gap-4 py-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => setNewComite(prev => ({ ...prev, nome: e.target.value }))}
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => setNewComite(prev => ({ ...prev, sigla: e.target.value.toUpperCase() }))}
                                                placeholder="Ex: CGOV"
                                                maxLength={20}
                                            />
                                        </div>
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => setNewComite(prev => ({ ...prev, descricao: e.target.value }))}
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value) => setNewComite(prev => ({ ...prev, icone: value }))}
                                            >
                                                <SelectTrigger>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {colorPresets.map((color) => (
                                                    <button
                                                        key={color}
                                                        type="button"
                                                        onClick={() => setNewComite(prev => ({ ...prev, cor: color }))}
                                                        className={`w-8 h-8 rounded-lg border-2 transition-all ${
                                                            newComite.cor === color ? 'border-gray-900 scale-110' : 'border-transparent'
                                                        }`}
                                        ...
                                        {newComite.nome && (
                                            <div className="mt-2">
                                                <Label className="text-xs text-gray-500 mb-1 block">Pré-visualização</Label>
                                                <div
                                                    className="rounded-lg p-4 text-white flex items-center gap-3"
                                                    style={{ backgroundColor: newComite.cor }}
                                                >
                                                    {(() => {
                                                        const PreviewIcon = iconMap[newComite.icone] || Megaphone;
                                                        return <PreviewIcon className="h-6 w-6" />;
                                                    })()}
                                                    <div>
                                                        <div className="font-semibold text-sm">{newComite.nome}</div>
                                                        {newComite.sigla && (
                                    ...
                                    <DialogFooter>
                                        ...
                                        <Button
                                            ...
                                            disabled={creating || !newComite.nome.trim() || !newComite.sigla.trim()}
                                        >
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center min-h-[400px] text-gray-400">
                            ...
                            <p className="text-lg font-medium text-gray-500">Nenhum comitê cadastrado</p>
                            <p className="text-sm mt-1">
                                {isAdmin ? 'Clique em "Novo Comitê" para criar o primeiro.' : 'Solicite ao administrador a criação de comitês.'}
                            </p>
                        </div>
                    ) : (
                                        ...
                                        <div className="p-5 flex flex-col gap-3 text-white">
                                            ...
                                            <div className="flex items-end justify-between gap-3">
                                                ...
                                                <div className="flex items-center gap-1 flex-shrink-0 mb-0.5">
                                                    {isAdmin && (
                                                        <>
                                                            <button onClick={(e) => handleEditComite(e, comite)} className="p-1 rounded hover:bg-white/20 transition-colors">
                                                                <Pencil className="h-3.5 w-3.5 text-white/60 hover:text-white" />
                                                            </button>
                                                            <button onClick={(e) => handleDeleteComite(e, comite)} className="p-1 rounded hover:bg-white/20 transition-colors">
            ...
            <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid gap-2">
                            <Label>Nome *</Label>
                            <Input value={newComite.nome} onChange={(e) => setNewComite(prev => ({ ...prev, nome: e.target.value }))} />
                        </div>
                        <div className="grid gap-2">
                            <Label>Sigla *</Label>
                            <Input value={newComite.sigla} onChange={(e) => setNewComite(prev => ({ ...prev, sigla: e.target.value.toUpperCase() }))} />
                        </div>
                        <div className="grid gap-2">
                            <Label>Descrição</Label>
                            <Input value={newComite.descricao} onChange={(e) => setNewComite(prev => ({ ...prev, descricao: e.target.value }))} />
                        </div>
                        <div className="grid gap-2">
                            <Label>Ícone</Label>
                            <Select value={newComite.icone} onValueChange={(v) => setNewComite(prev => ({ ...prev, icone: v }))}>
                        ...
                        <div className="grid gap-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {colorPresets.map(c => (
                                    <button key={c} onClick={() => setNewComite(prev => ({ ...prev, cor: c }))}
