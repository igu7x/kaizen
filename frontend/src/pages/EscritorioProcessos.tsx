...
// ============================================================
const HUB_STATUS = ['Ativo', 'Em Revisão', 'Aguardando Revisão', 'Revisão Pendente'] as const;
...
const REV_SITUACAO = ['No prazo', 'Aguardando revisão', 'Revisão vencida', 'Em revisão'] as const;
...
function hubStatusOf(p: ProcessoNegocio): HubStatus {
  ...
  if (p.status === 'enviado' || p.status === 'validado_autor' || p.status === 'validado_diretoria') {
...
function StatCard({ title, subtitle, value, hint, icon, iconBg, iconColor, borderColor, activeRing, active, onClick }: StatCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className={`flex-shrink-0 w-12 h-12 rounded-xl ${iconBg} ${iconColor} flex items-center justify-center`}>
          {icon}
        </div>
      </div>
    </button>
...
function DonutChartCard({ title, data, total }: DonutChartCardProps) {
  ...
  const sweepKey = useMemo(() => chartData.map(d => `${d.name}:${d.value}`).join('|'), [chartData]);
      ...
      <div className="flex-1 flex items-center gap-3 min-h-0">
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                data={chartData.length > 0 ? chartData : [{ name: 'vazio', value: 1, color: '#e5e7eb' }]}
                cx="50%"
                cy="50%"
                innerRadius={42}
                outerRadius={65}
              ...
              >
                {(chartData.length > 0 ? chartData : [{ color: '#e5e7eb' }]).map((entry: any, idx) => (
          ...
          {chartData.length > 0 && !hovering && (
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              ...
              <span className="text-[10px] text-slate-500 mt-0.5 leading-none">total</span>
            </div>
          )}
        </div>
        <div className="flex-1 flex flex-col justify-center gap-1.5 min-w-0">
          {data.map((d) => (
            <div key={d.name} className="flex items-center gap-2 text-xs">
              ...
              />
              <span className="text-slate-700 flex-1 truncate" title={d.desc}>{d.name}</span>
...
function BarChartCard({ title, data }: BarChartCardProps) {
  ...
  const animKey = useMemo(() => data.map(d => `${d.name}:${d.value}`).join('|'), [data]);
  const wipeRef = useWipeAnimation(animKey, 1300);
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm h-[300px] flex flex-col">
      <h3 className="text-sm font-bold text-slate-900 mb-2">{title}</h3>
      <div ref={wipeRef} className="flex-1 min-h-0" style={{ overflow: 'visible' }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart key={animKey} data={data} layout="vertical" margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
            ...
            />
            <RTooltip
              formatter={(v: number, _n, props: any) => [`${v} (${props.payload.pct}%)`, 'Processos']}
            />
            <Bar
              dataKey="value"
              fill="#3b82f6"
            ...
            >
              {data.map((_, idx) => (
                <Cell key={idx} fill={['#1e3a8a', '#2563eb', '#3b82f6', '#60a5fa', '#7c3aed', '#94a3b8'][idx % 6]} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
...
export default function EscritorioProcessos() {
  ...
  const [filtroArtefato, setFiltroArtefato] = useState<'all' | 'mps' | 'pop' | 'flux'>('all');
  ...
  const areasOptions = useMemo(() => {
    const set = new Set<string>();
    processos.forEach(p => (p.areas_responsaveis || []).forEach(a => set.add(a)));
    return Array.from(set).sort();
  }, [processos]);
  ...
  const filteredBase = useMemo(() => {
    return processos.filter(p => {
      if (filtroMacro !== 'all' && p.macroprocesso !== filtroMacro) return false;
      if (filtroArea !== 'all' && !(p.areas_responsaveis || []).includes(filtroArea)) return false;
      if (filtroDiretoria !== 'all' && p.diretoria !== filtroDiretoria) return false;
      if (filtroStatus !== 'all' && hubStatusOf(p) !== filtroStatus) return false;
      return true;
    });
  ...
  const filtered = useMemo(() => {
    return filteredBase.filter(p => {
      const docs = p.documentos_anexados || [];
      if (filtroArtefato === 'mps' && !docs.some(d => d.tipo === 'MPS')) return false;
      if (filtroArtefato === 'pop' && !docs.some(d => d.tipo === 'POP')) return false;
  ...
  const statusChartData = useMemo(() => {
    const counts: Record<HubStatus, number> = { 'Ativo': 0, 'Em Revisão': 0, 'Aguardando Revisão': 0, 'Revisão Pendente': 0 };
    filtered.forEach(p => { counts[hubStatusOf(p)]++; });
    return HUB_STATUS.map(s => ({ name: s, value: counts[s], color: HUB_STATUS_DOT[s] }));
  }, [filtered]);
  ...
  const revisaoChartData = useMemo(() => {
    const counts: Record<RevSituacao, number> = { 'No prazo': 0, 'Aguardando revisão': 0, 'Revisão vencida': 0, 'Em revisão': 0 };
    filtered.forEach(p => { counts[revSituacaoOf(p)]++; });
    return REV_SITUACAO.map(s => ({ name: s, value: counts[s], color: REV_SITUACAO_COLOR[s] }));
  }, [filtered]);

  const maturidadeChartData = useMemo(() => {
    const counts: Record<1 | 2 | 3 | 4, number> = { 1: 0, 2: 0, 3: 0, 4: 0 };
    filtered.forEach(p => { counts[maturidadeOf(p)]++; });
  ...
  const tabelaProcessos = useMemo(() => {
    ...
    return filtered.filter(p => (p.nome_processo || '').toLowerCase().includes(q));
  ...
  useEffect(() => { setPage(1); }, [filtroMacro, filtroArea, filtroDiretoria, filtroStatus, filtroArtefato, buscaProcesso]);

  // ============================================================
  // HANDLERS
  // ============================================================

  const handleNovoProcesso = () => { setEditing(null); setFormOpen(true); };
  const handleEditar = (p: ProcessoNegocio) => { setDetalheOpen(false); setEditing(p); setFormOpen(true); };
  const handleAbrirDetalhe = (p: ProcessoNegocio) => { setSelecionado(p); setDetalheOpen(true); };
      ...
      <div className="space-y-5 page-transition-enter">
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold text-slate-800">Escritório de Processos</h1>
            </div>
          </div>
          ...
          <div className="flex items-end gap-3 flex-wrap">
            ...
            {podeFiltrarDiretoria && (
              <div className="flex flex-col">
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Diretoria</label>
                <Select value={filtroDiretoria} onValueChange={setFiltroDiretoria}>
            ...
            {isAdminOrManager && (
              <Button onClick={handleNovoProcesso} className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm h-10">
        ...
        <div className="flex items-end gap-3 flex-wrap">
          <div className="flex flex-col flex-1 min-w-[200px]">
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Macroprocesso</label>
            <Select value={filtroMacro} onValueChange={setFiltroMacro}>
              <SelectTrigger className="h-10 bg-white"><SelectValue placeholder="Todos os macroprocessos" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todos os macroprocessos</SelectItem>
                {macroprocessosOptions.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col flex-1 min-w-[200px]">
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Área</label>
            <Select value={filtroArea} onValueChange={setFiltroArea}>
              <SelectTrigger className="h-10 bg-white"><SelectValue placeholder="Todas as áreas" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todas as áreas</SelectItem>
                {areasOptions.map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col flex-1 min-w-[200px]">
            <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Status</label>
            <Select value={filtroStatus} onValueChange={setFiltroStatus}>
              <SelectTrigger className="h-10 bg-white"><SelectValue placeholder="Todos os status" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todos os status</SelectItem>
                {HUB_STATUS.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="ml-auto flex items-center gap-2 text-xs text-slate-500">
            ...
            <span className="font-medium text-slate-700">
              {refreshAt.toLocaleDateString('pt-BR')} {refreshAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
            </span>
            <button
              onClick={carregar}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            onClick={() => setFiltroArtefato(filtroArtefato === 'mps' ? 'all' : 'mps')}
          />
          <StatCard
            title="POPs"
            ...
            onClick={() => setFiltroArtefato(filtroArtefato === 'pop' ? 'all' : 'pop')}
          />
          <StatCard
            title="Fluxogramas"
            value={stats.flux}
            ...
            onClick={() => setFiltroArtefato(filtroArtefato === 'flux' ? 'all' : 'flux')}
          />
        </div>

        {/* GRÁFICOS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <DonutChartCard title="Status dos Processos" data={statusChartData} total={stats.total} />
          <BarChartCard title="Processos por Macroprocesso" data={macroChartData} />
          <DonutChartCard title="Situação das Revisões (Próxima Revisão)" data={revisaoChartData} total={stats.total} />
          <DonutChartCard title="Maturidade dos Processos" data={maturidadeChartData} total={stats.total} />
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-slate-700 font-semibold">Nenhum processo encontrado</p>
              <p className="text-slate-500 text-sm mt-1">Ajuste os filtros ou cadastre um novo processo.</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">Processo</th>
                      <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">Macroprocesso</th>
                      <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">Área</th>
                      <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">Documentos Vinculados</th>
                      <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">Última Atualização</th>
                      <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">Próxima Revisão</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginated.map(p => {
                      ...
                      const overdue = nextRev != null && nextRev.getTime() < Date.now();
                      const docs = p.documentos_anexados || [];
                      const hasMps = docs.some(d => d.tipo === 'MPS');
                      const hasPop = docs.some(d => d.tipo === 'POP');
                      const hasFlux = !!p.fluxograma_data;
                      const areaLabel = (p.areas_responsaveis || [])[0] || p.diretoria || '—';
                      return (
                        <tr
                          key={p.id}
                        ...
                        >
                          <td className="px-5 py-3 text-slate-900 font-medium">{p.nome_processo}</td>
                          <td className="px-5 py-3 text-slate-700">{p.macroprocesso || '—'}</td>
                          <td className="px-5 py-3 text-slate-700">{areaLabel}</td>
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              {hasMps && <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border bg-blue-50 text-blue-700 border-blue-200">MPS</span>}
                              {hasPop && <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border bg-emerald-50 text-emerald-700 border-emerald-200">POP</span>}
                              {hasFlux && <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border bg-pink-50 text-pink-700 border-pink-200">Fluxograma</span>}
                              {!hasMps && !hasPop && !hasFlux && <span className="text-xs italic text-slate-400">—</span>}
                            </div>
                          </td>
                          <td className="px-5 py-3 text-slate-700 tabular-nums">{formatDateShort(p.updated_at)}</td>
                          <td className={`px-5 py-3 tabular-nums font-medium ${overdue ? 'text-red-600' : 'text-slate-700'}`}>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-slate-200 bg-slate-50">
                <span className="text-xs text-slate-600">
                  Mostrando {(page - 1) * PAGE_SIZE + 1} a {Math.min(page * PAGE_SIZE, tabelaProcessos.length)} de {tabelaProcessos.length} processos
                </span>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                    <button
                      ...
                      className={`min-w-[28px] px-2 py-1 rounded text-sm font-medium ${
                        n === page ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-white'
                      }`}
                    >
                      {n}
                    </button>
