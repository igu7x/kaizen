...
export default function Administracao() {
  ...
  const adminDiretoria = ((currentUser as any)?.diretoria ||
    "SGJT") as Diretoria;
  ...
  useEffect(() => {
    ...
    const loadAreas = devEnvironment
      ? areasApi.getByDominio(devEnvironment)
      : areasApi.getAll();
    loadAreas
      .then((areas) => {
        setAllAreas(areas);
    ...
    const loadGlobal = devEnvironment
      ? areasApi.getByDominio(devEnvironment)
      : areasApi.getAllGlobal();
    loadGlobal
      .then((areas) => {
  ...
  const getAreaLabel = (sigla: string) => {
    const area =
      globalAreas.find((a) => (a.sigla || a.nome) === sigla) ||
      domainAreas.find((a) => (a.sigla || a.nome) === sigla);
    return area?.nome !== sigla && area?.nome
      ? area.nome
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!canSelectDiretoria && domainSiglas.size > 0) {
      result = result.filter((u) => domainSiglas.has((u as any).diretoria));
    }
  ...
  const loadUsers = async () => {
    try {
      ...
      const dominio =
        devEnvironment || (currentUser as any)?.dominio || undefined;
                ...
                {!isProduction() && (
                  <div>
                    <Label htmlFor="password">
                      Senha (opcional para login local)
                    </Label>
                    <Input
                      id="password"
                      name="password"
                      ...
                      placeholder={
                        editingUser
                          ...
                          : "Defina uma senha para login local"
                      }
                    />
