...
export default function GestaoEstrategica() {
  ...
  const getPageInfo = () => {
    ...
    return {
      subtitle:
        modelo === "metas" ? "Monitoramento de Metas" : "Monitoramento de OKRs",
    };
  };
  ...
  const isWhiteBgPage =
    location.pathname === "/gestao-estrategica/okrs" ||
    ...
    <Layout>
      <div className="space-y-4 lg:space-y-6">
        <Breadcrumbs
          ...
          className={
            isWhiteBgPage ? "" : "[&_*]:!text-white/70 [&_a:hover]:!text-white"
          }
        />
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p
                className={`text-xs font-medium uppercase tracking-wider ${isWhiteBgPage ? "text-slate-400" : "text-white/50"}`}
              >
                Gestão Estratégica
              </p>
              <h1
                className={`text-2xl font-bold ${isWhiteBgPage ? "text-slate-800" : "text-white"}`}
              >
                {subtitle}
              </h1>
            </div>
