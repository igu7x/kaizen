...
import { FileText, FolderKanban, Building2, Users, Target, ShieldCheck, ChevronRight } from 'lucide-react';
...
const cadastroItems = [
  ...
  },
  {
    ...
    description: 'Conceder a usuários a edição dos campos do TAP em projetos da sua diretoria.',
    icon: ShieldCheck,
      ...
      <div className="page-transition-enter min-h-full">
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="mb-10 text-left pl-2">
            <p className="text-xs font-semibold uppercase tracking-widest text-blue-600 mb-1">Administração</p>
            <h1 className="text-3xl font-bold text-slate-900">Cadastros</h1>
            <p className="text-slate-500 mt-1 text-sm">Selecione um módulo para gerenciar.</p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {cadastroItems.map((item) => (
              <button
                key={item.path}
                onClick={() => navigate(item.path, { state: { fromCadastros: true } })}
