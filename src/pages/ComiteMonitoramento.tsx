...
export default function ComiteMonitoramento() {
  ...
  const [modalQuadro, setModalQuadro] = useState<{
    open: boolean;
    item: ComiteQuadroControle | null;
  }>({ open: false, item: null });
  const [modalMembro, setModalMembro] = useState<{
    open: boolean;
    item: ComiteMembro | null;
  }>({ open: false, item: null });
  const [modalDescricao, setModalDescricao] = useState(false);
  const [modalReuniao, setModalReuniao] = useState<{
    open: boolean;
  ...
  const isGestorOrAdmin =
    isSgjt || ((user?.role === "MANAGER" || user?.role === "ADMIN") && isRoot);
  ...
  const loadComite = async () => {
    try {
      ...
      const [reunioesData, membrosData, quadroData, todasReunioesData] =
        await Promise.all([
      ...
      if (reunioesData.length > 0) {
        ...
        if (reuniaoHoje) {
          ...
          const realizadas = reunioesData.filter(
  ...
  const handleSaveReuniao = async (
    ...
    try {
      ...
      if (modalReuniao.item) {
        // Editar
        updatedReuniao = await reunioesApi.update(
          comite.id,
          modalReuniao.item.id,
          payload,
        );
        ...
        setReunioes((prev) =>
          prev
            .map((r) => (r.id === updatedReuniao.id ? updatedReuniao : r))
            .sort(
              (a, b) => new Date(a.data).getTime() - new Date(b.data).getTime(),
            ),
        );
  ...
  const handleSavePauta = async (data: {
    numero_item: number;
    descricao: string;
  }) => {
    ...
    try {
      if (modalPauta.item) {
        const updated = await pautaApi.update(
          comite.id,
          reuniaoAtiva.id,
          modalPauta.item.id,
          data,
        ...
        setPauta((prev) =>
          [...prev, created].sort((a, b) => a.numero_item - b.numero_item),
        );
      }
  ...
  const handleSaveQuadro = async (data: Partial<ComiteQuadroControle>) => {
    if (!comite) {
      toast({
        title: "Erro",
        description: "Comitê não carregado",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      if (modalQuadro.item) {
        // Editar
        const updated = await quadroControleApi.update(
          comite.id,
          modalQuadro.item.id,
          data,
        );

        setQuadroControle((prev) =>
  ...
  const handleSaveMembro = async (data: { nome: string; cargo: string }) => {
    ...
    try {
      if (modalMembro.item) {
        const updated = await membrosApi.update(
          comite.id,
          modalMembro.item.id,
          data,
        );
            ...
            <Tabs value={abaAtiva} onValueChange={setAbaAtiva}>
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  onClick={() => {
                    const link =
                      reunioes.find((r) => r.link_proad)?.link_proad ||
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (reuniaoAtiva.ata_filename && sigla) {
                              ...
                              window.open(
                                atasApi.getDownloadUrl(sigla, reuniaoAtiva.id),
                                "_blank",
                              );
                          ...
                          <div className="flex items-center justify-between mb-4">
                            ...
                            {isGestorOrAdmin && (
                              <Button
                                ...
                                onClick={() =>
                                  setModalPauta({ open: true, item: null })
                                }
                              >
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    {isGestorOrAdmin && (
                      <Button
                        ...
                        onClick={() =>
                          setModalQuadro({ open: true, item: null })
                        }
                      >
                        <Plus className="h-4 w-4" />
                        Nova Ação
                                    ...
                                    <span className="text-sm">
                                      {item.item_pauta_descricao && (
                                        <span
                                          ...
                                          title={item.item_pauta_descricao}
                                        >
                                          {item.item_pauta_descricao}
                                        </span>
                                      )}
                                  ...
                                  {isGestorOrAdmin ? (
                                    <Select
                                      ...
                                      onValueChange={async (newStatus) => {
                                        ...
                                        try {
                                          const updated =
                                            await quadroControleApi.update(
                                              comite.id,
                                              item.id,
                                              {
                                                status:
                                                  newStatus as QuadroControleStatus,
                                              },
                                            );
                                          setQuadroControle((prev) =>
                                            prev.map((q) =>
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    {isGestorOrAdmin && (
                      <Button
                        ...
                        className="bg-green-600 hover:bg-green-700 gap-1"
                        onClick={() =>
                          setModalMembro({ open: true, item: null })
                        }
                      >
                        <Plus className="h-4 w-4" />
                        Adicionar Membro
        ...
        <AlertDialogContent>
            ...
            <AlertDialogDescription>
              {deleteConfirm?.type === "reuniao" ? (
                <>
                  ...
                  <br />
                  <br />
                  Esta ação irá excluir também todos os itens da pauta desta
                  reunião.
                  <br />
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (deleteConfirm?.type === "reuniao")
                  handleDeleteReuniao(deleteConfirm.id);
                if (deleteConfirm?.type === "pauta")
                  handleDeletePauta(deleteConfirm.id);
                if (deleteConfirm?.type === "quadro")
                  handleDeleteQuadro(deleteConfirm.id);
                if (deleteConfirm?.type === "membro")
                  handleDeleteMembro(deleteConfirm.id);
              }}
            >
              Sim, Excluir
            </AlertDialogAction>
  ...
  useEffect(() => {
    if (open) {
      ...
      if (item) {
        ...
        const maxNumero =
          reunioes.length > 0 ? Math.max(...reunioes.map((r) => r.numero)) : 0;
        setFormData({
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {item
              ? `Editar Reunião ${item.numero} - ${item.ano}`
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setFormData({ ...formData, tipo_reuniao: v as TipoReuniao })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setFormData({ ...formData, status: v as ReuniaoStatus })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, link_proad: e.target.value })
              }
              placeholder="https://..."
            />
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, link_transparencia: e.target.value })
              }
              placeholder="https://..."
            />
          </div>
          ...
          <div>
            <label className="text-sm font-medium">
              Link Ata de Reunião (externo)
            </label>
            <Input
              type="url"
              value={formData.link_ata}
              onChange={(e) =>
                setFormData({ ...formData, link_ata: e.target.value })
              }
          ...
          <div className="border-t pt-4 mt-4">
            ...
            <p className="text-xs text-slate-500 mb-3">
              Envie o arquivo PDF da ata (máx. 10MB). Este arquivo ficará
              ...
              <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg mb-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-green-600">
                    ...
                    {item.ata_uploaded_at
                      ? ` • Enviado em ${new Date(item.ata_uploaded_at).toLocaleDateString("pt-BR")}`
                      : ""}
                  </p>
                </div>
                <Button
              ...
              <div className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg mb-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-blue-600">
                    {formatFileSize(ataFile.size)} • Pronto para enviar
                  </p>
                </div>
                <Button
                  type="button"
              ...
              <div className="flex items-center gap-3 p-3 bg-red-50 border border-red-200 rounded-lg mb-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-red-800">
                    Ata será removida ao salvar
                  </p>
                  <p className="text-xs text-red-600">
                    Clique em "Selecionar arquivo" para cancelar
                  </p>
                </div>
              </div>
            )}
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {item ? "Editar Item da Pauta" : "Adicionar Item da Pauta"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mb-1">
              Demanda estabelecida em reunião
            </p>
            <Textarea
              value={formData.item}
              onChange={(e) =>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mb-1">
              Relacionado à ação demandada na reunião
            </p>
            <Select
              ...
              onValueChange={(v) =>
                setFormData({
                  ...formData,
                  item_pauta_id: v && v !== "none" ? parseInt(v) : null,
                })
              }
                ...
                <SelectValue
                  placeholder={
                    loadingPauta
                      ? "Carregando..."
                      : !formData.reuniao_id
                        ? "Selecione uma reunião primeiro"
                        : pautaItems.length === 0
                ...
                {pautaItems.map((pauta) => (
                  <SelectItem key={pauta.id} value={pauta.id.toString()}>
                    <span className="font-medium">
                      Item {pauta.numero_item}:
                    </span>{" "}
                    <span className="text-slate-600">
                      {pauta.descricao.length > 50
                        ? pauta.descricao.substring(0, 50) + "..."
                        : pauta.descricao}
                    </span>
                  </SelectItem>
                ))}
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mb-1">
              Servidor(es) responsável(is) pela ação demandada
            </p>
            <Textarea
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setFormData({
                    ...formData,
                    status: v as QuadroControleStatus,
                  })
                }
              >
                <SelectTrigger>
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {item ? "Editar Membro" : "Adicionar Membro"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
