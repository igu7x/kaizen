...
export function validateTAPFields(projeto: Projeto): { valid: boolean; missingFields: string[] } {
  const missingFields: string[] = [];

  if (!projeto.nome) missingFields.push('Nome do Projeto');
  if (!projeto.tap_vinculado) missingFields.push('Nº do Proad');
  if (!projeto.data_prevista_inicio) missingFields.push('Data Prevista de Início');
  if (!projeto.data_prevista_conclusao) missingFields.push('Data Prevista de Conclusão');
  if (!projeto.objetivo) missingFields.push('Objetivo');
  if (!projeto.contexto_justificativa) missingFields.push('Contexto e Justificativa');
  ...
  if (!projeto.entregas || projeto.entregas.length === 0) missingFields.push('Entregas (pelo menos 1)');
  if (!projeto.instrumentos || projeto.instrumentos.length === 0) missingFields.push('Ancoragem Estratégica (pelo menos 1)');
...
function addFooter(doc: jsPDF, pageNum: number, totalPages: number, versao: number, tapId: string | null) {
  ...
  doc.text(`TAP  —  Termo de Abertura do Projeto  —  Versão ${versao}.0${idPart}`, MARGIN_LEFT, FOOTER_Y);
...
export function generateTAPPdf(projeto: Projeto) {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...TEXT_DARK);
  const titleLines = doc.splitTextToSize('Termo de Abertura do Projeto', leftMaxW);
  ...
  doc.setFontSize(9);
  doc.setTextColor(...TEXT_DARK);
  doc.text(doc.splitTextToSize(`Status: ${tapStatus}`, leftMaxW), MARGIN_LEFT + 5, areaEndY + 7);
  ...
  y = drawLabelValueRow(doc, 'Área Responsável', projeto.areas_execucao_diretorias || projeto.diretoria || '', y);
  ...
  const prazoText = prazoInicio && prazoConclusao
  ...
  drawTextCell(doc, 'Papel', MARGIN_LEFT, y, colPapel, govHeaderH, { bold: true, fontSize: 9, align: 'center', bg: ACCENT_BG_LIGHT });
  drawTextCell(doc, 'Responsabilidades-chave', MARGIN_LEFT + colPapel, y, colResp, govHeaderH, { bold: true, fontSize: 9, align: 'center', bg: ACCENT_BG_LIGHT });
  drawTextCell(doc, 'Nome', MARGIN_LEFT + colPapel + colResp, y, colNome, govHeaderH, { bold: true, fontSize: 9, align: 'center', bg: ACCENT_BG_LIGHT });
  y += govHeaderH;

  const areasText = projeto.areasExecucao?.map(a => `${a.diretoria} - ${a.area_nome}`).join(', ')
    || projeto.areas_execucao_diretorias
    || '';

  const respPatrocinador = 'Assegurar o alinhamento estratégico do projeto aos objetivos do Tribunal; aprovar o TAP e validar alterações de escopo; viabilizar os recursos necessários; e atuar na remoção de impedimentos organizacionais e institucionais.';
  const respGestor = 'Planejar e coordenar as atividades do projeto; acompanhar o cronograma e a execução das entregas; gerenciar riscos e comunicações; e reportar o progresso e eventuais desvios ao Patrocinador para tomada de decisão.';
  const respAreas = 'Executar as atividades técnicas e operacionais previstas no escopo; garantir a qualidade técnica das entregas (módulos e funcionalidades); e fornecer subsídios para o monitoramento de indicadores e progresso das ações.';

  const govRow1H = calcRowHeight(doc, [
    { text: respPatrocinador, width: colResp },
    { text: projeto.patrocinador_nome || '', width: colNome },
  ], 9, 12);
  y = checkPageBreak(doc, y, govRow1H);
  drawTextCell(doc, 'Patrocinador', MARGIN_LEFT, y, colPapel, govRow1H, { bold: true, fontSize: 9 });
  drawTextCell(doc, respPatrocinador, MARGIN_LEFT + colPapel, y, colResp, govRow1H, { fontSize: 9 });
  drawTextCell(doc, projeto.patrocinador_nome || '', MARGIN_LEFT + colPapel + colResp, y, colNome, govRow1H);
  y += govRow1H;

  const govRow2H = calcRowHeight(doc, [
    { text: respGestor, width: colResp },
    { text: projeto.gestor_nome || '', width: colNome },
  ], 9, 12);
  y = checkPageBreak(doc, y, govRow2H);
  drawTextCell(doc, 'Gestor', MARGIN_LEFT, y, colPapel, govRow2H, { bold: true, fontSize: 9 });
  drawTextCell(doc, respGestor, MARGIN_LEFT + colPapel, y, colResp, govRow2H, { fontSize: 9 });
  drawTextCell(doc, projeto.gestor_nome || '', MARGIN_LEFT + colPapel + colResp, y, colNome, govRow2H);
  y += govRow2H;

  const govRow3H = calcRowHeight(doc, [
    { text: respAreas, width: colResp },
    { text: areasText, width: colNome },
  ], 9, 12);
  y = checkPageBreak(doc, y, govRow3H);
  drawTextCell(doc, 'Áreas de Execução', MARGIN_LEFT, y, colPapel, govRow3H, { bold: true, fontSize: 9 });
  drawTextCell(doc, respAreas, MARGIN_LEFT + colPapel, y, colResp, govRow3H, { fontSize: 9 });
  drawTextCell(doc, areasText, MARGIN_LEFT + colPapel + colResp, y, colNome, govRow3H);
  y += govRow3H + 6;
  ...
  const colEntregaArea = CONTENT_WIDTH * 0.30;
  ...
  drawTextCell(doc, 'Área Responsável', MARGIN_LEFT + colEntregaNome, y, colEntregaArea, entregaHeaderH, { bold: true, fontSize: 9, align: 'center', bg: ACCENT_BG_LIGHT });
  drawTextCell(doc, 'Prazo Estimado', MARGIN_LEFT + colEntregaNome + colEntregaArea, y, colEntregaPrazo, entregaHeaderH, { bold: true, fontSize: 9, align: 'center', bg: ACCENT_BG_LIGHT });
  y += entregaHeaderH;
  ...
  for (let i = 0; i < entregas.length; i++) {
    ...
    drawTextCell(doc, entrega.nome || '', MARGIN_LEFT, y, colEntregaNome, rowH, { align: 'center' });
    drawTextCell(doc, entrega.area_responsavel_nome || '', MARGIN_LEFT + colEntregaNome, y, colEntregaArea, rowH, { align: 'center' });
    drawTextCell(doc, prazoTexto, MARGIN_LEFT + colEntregaNome + colEntregaArea, y, colEntregaPrazo, rowH, { align: 'center' });
    y += rowH;
  }
  y += 6;
  ...
  y = drawLabelValueRow(doc, 'Prioridade Institucional', translatePrioridade(projeto.prioridade), y);
  y = drawLabelValueRow(doc, 'Complexidade do Projeto', translateComplexidade(projeto.complexidade), y);
  y = drawLabelValueRow(doc, 'Abrangência Organizacional', translateAbrangencia(projeto.abrangencia), y);
  y = drawLabelValueRow(doc, 'Haverá contratação?', projeto.havera_contratacao ? 'Sim' : 'Não', y);
  if (projeto.havera_contratacao) {
    y = drawLabelValueRow(doc, 'Valor Estimado', formatCurrency(projeto.valor_estimado_contratacao), y);
  }
