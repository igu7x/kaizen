...
import type { AvaliacaoGestorFormulario } from '@/services/avaliacaoGestorApi';
...
export function generateAvaliacaoGestorPDF(formulario: AvaliacaoGestorFormulario) {
  const doc = new jsPDF('p', 'mm', 'a4');
  let y = 0;

  const tipoInv = formulario.tipo_inventario || 'equipe';
  const titleLabel = tipoInv === 'gestor' ? 'Avaliação da Liderança' : 'Avaliação do Gestor';
  ...
  doc.text(titleLabel, MARGIN_LEFT + 5, 18, { maxWidth: leftMaxW });
  ...
  // ============================================================
  // BLOCO - Dados da Avaliação
  // ============================================================
  const dadosItems: Array<{ label: string; value: string }> = [
    { label: 'Avaliador', value: formulario.avaliador_nome || '' },
    { label: 'Colaborador', value: formulario.pessoa_nome || '' },
  ];
  if (formulario.pessoa_cargo) dadosItems.push({ label: 'Cargo/Função', value: formulario.pessoa_cargo });
  if (formulario.pessoa_email) dadosItems.push({ label: 'E-mail', value: formulario.pessoa_email });
  ...
  const footerLabel = [titleLabel, formulario.unidade_nome || '', versionPart].filter(Boolean).join(' — ');
