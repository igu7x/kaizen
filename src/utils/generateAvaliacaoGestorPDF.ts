...
function addFooter(doc: jsPDF, pg: number, total: number, label: string) {
  ...
  doc.text(`Página ${pg} de ${total}`, PAGE_WIDTH - MARGIN_RIGHT, FOOTER_Y, {
    align: "right",
  });
}
...
export function generateAvaliacaoGestorPDF(
  ...
  const titleLabel =
    tipoInv === "gestor" ? "Avaliação da Liderança" : "Avaliação do Gestor";
  const tipoLabel =
  ...
  for (const secao of tiposSecao) {
    ...
    for (let i = 0; i < rs.length; i++) {
      ...
      const linesEstim = doc.splitTextToSize(
        (r.competencia_descricao || "") + (r.comentario || ""),
        CONTENT_WIDTH - 40,
      );
  ...
  const versionPart =
    formulario.versao_formulario && formulario.versao_formulario > 0
      ...
      : "";
  const footerLabel = [titleLabel, formulario.unidade_nome || "", versionPart]
    .filter(Boolean)
    .join(" — ");
