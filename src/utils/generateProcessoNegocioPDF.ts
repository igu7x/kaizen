...
function drawTextCell(
  ...
  options?: { bold?: boolean; fontSize?: number; align?: 'left' | 'center' | 'right'; bg?: readonly [number, number, number]; color?: readonly [number, number, number] }
) {
  ...
  doc.setTextColor(...(options?.color || (options?.bold ? LABEL_GRAY : TEXT_GRAY)));
...
function drawMultilineContent(doc: jsPDF, text: string, y: number, minHeight: number = 18): number {
  ...
  if (flat.length === 0) {
    if (y + minHeight > bottomLimit) { doc.addPage(); y = 15; }
    doc.setDrawColor(...BORDER_GRAY);
    doc.setLineWidth(0.3);
  ...
  while (lineIndex < flat.length) {
    ...
    const minNeeded = Math.max(lineHeight + paddingTop + paddingBottom, isFirstChunkNow ? minHeight : 0);
    if (availableHeight < minNeeded) { doc.addPage(); y = 15; availableHeight = bottomLimit - y; }
    ...
    while (subStart < chunkEnd) {
      ...
      if (paraIdx === -1) { subStart++; continue; }
    ...
    y += chunkHeight;
    lineIndex = chunkEnd;
    if (lineIndex < flat.length) { doc.addPage(); y = 15; }
  }
  return y;
}

// Bullet list dentro de uma célula (Proprietário, Atores, Áreas Responsáveis...)
function renderBulletList(doc: jsPDF, items: string[], x: number, y: number, w: number, h: number) {
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...TEXT_GRAY);
...
function calcBulletListHeight(doc: jsPDF, items: string[], w: number, minHeight: number = 16): number {
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
...
// ============================================================
function drawCabecalhoInstitucional(doc: jsPDF, processo: ProcessoNegocio, y: number): number {
  // Layout:
  ...
  doc.text(tituloHeader, MARGIN_LEFT + leftColW + rightColW / 2, y + rowH / 2 + 1.4, { align: 'center' });

  // Linha 2: Macroprocesso (full right col)
  const yMacro = y + rowH;
  const labelW = 35;
  drawTextCell(doc, 'Macroprocesso:', MARGIN_LEFT + leftColW, yMacro, labelW, rowH, { bold: true, fontSize: 9, bg: ACCENT_BG_LIGHT, color: BLUE_TITLE, align: 'center' });
  drawTextCell(doc, processo.macroprocesso || '—', MARGIN_LEFT + leftColW + labelW, yMacro, rightColW - labelW, rowH, { fontSize: 9 });

  // Linha 3: Diretoria | Período
  const yDir = yMacro + rowH;
  const halfRightW = rightColW / 2;
  drawTextCell(doc, 'Diretoria', MARGIN_LEFT + leftColW, yDir, labelW, rowH, { bold: true, fontSize: 9, bg: ACCENT_BG_LIGHT, color: BLUE_TITLE, align: 'center' });
  drawTextCell(doc, processo.diretoria || '—', MARGIN_LEFT + leftColW + labelW, yDir, halfRightW - labelW, rowH, { fontSize: 9 });
  drawTextCell(doc, 'Período', MARGIN_LEFT + leftColW + halfRightW, yDir, labelW, rowH, { bold: true, fontSize: 9, bg: ACCENT_BG_LIGHT, color: BLUE_TITLE, align: 'center' });
  drawTextCell(doc, formatPeriodoMesAno(processo.periodo), MARGIN_LEFT + leftColW + halfRightW + labelW, yDir, halfRightW - labelW, rowH, { fontSize: 9 });

  // Linha 4: Revisão | Código/Versão
  const yRev = yDir + rowH;
  drawTextCell(doc, 'Revisão:', MARGIN_LEFT + leftColW, yRev, labelW, rowH, { bold: true, fontSize: 9, bg: ACCENT_BG_LIGHT, color: BLUE_TITLE, align: 'center' });
  drawTextCell(doc, processo.revisao || '—', MARGIN_LEFT + leftColW + labelW, yRev, halfRightW - labelW, rowH, { fontSize: 9 });
  drawTextCell(doc, 'Código/Versão', MARGIN_LEFT + leftColW + halfRightW, yRev, labelW, rowH, { bold: true, fontSize: 9, bg: ACCENT_BG_LIGHT, color: BLUE_TITLE, align: 'center' });
  drawTextCell(doc, processo.codigo_versao || '—', MARGIN_LEFT + leftColW + halfRightW + labelW, yRev, halfRightW - labelW, rowH, { fontSize: 9 });

  // Linha 5: NOME DO PROCESSO (full width, label esquerda + valor à direita em itálico azul)
  const yNome = yRev + rowH;
  const nomeRowH = 11;
  drawTextCell(doc, 'NOME DO PROCESSO', MARGIN_LEFT, yNome, leftColW, nomeRowH, { bold: true, fontSize: 9, align: 'center' });
  doc.setFillColor(...WHITE);
  ...
  doc.text(processo.nome_processo || '—', MARGIN_LEFT + leftColW + 4, yNome + nomeRowH / 2 + 1.6);
...
// ============================================================
function drawHistoricoValidacao(doc: jsPDF, processo: ProcessoNegocio, y: number): number {
  ...
  const linhas = [
    { label: 'Validação do Autor', valor: processo.validado_autor_em && processo.validado_autor_nome
        ? `${processo.validado_autor_nome} — ${formatDateTime(processo.validado_autor_em)}`
        : 'Pendente' },
    { label: 'Validação da Diretoria', valor: processo.validado_diretoria_em && processo.validado_diretoria_nome
        ? `${processo.validado_diretoria_nome} — ${formatDateTime(processo.validado_diretoria_em)}`
        : 'Pendente' },
    { label: 'Validação Final', valor: processo.validado_final_em && processo.validado_final_nome
        ? `${processo.validado_final_nome} — ${formatDateTime(processo.validado_final_em)}`
        : 'Pendente' },
  ];

  for (const linha of linhas) {
    drawTextCell(doc, linha.label, MARGIN_LEFT, y, labelW, rowH, { bold: true, fontSize: 9, bg: [248, 250, 252] });
    drawTextCell(doc, linha.valor, MARGIN_LEFT + labelW, y, CONTENT_WIDTH - labelW, rowH, { fontSize: 9 });
    y += rowH;
  }
...
// ============================================================
function drawRodapeInstitucional(doc: jsPDF, processo: ProcessoNegocio, diretoriaNome: string, pageNum: number, totalPages: number) {
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...MUTED_GRAY);
  doc.text('ELABORADO POR:', MARGIN_LEFT, FOOTER_Y);
  doc.text('VERSÃO:', PAGE_WIDTH / 2, FOOTER_Y, { align: 'center' });
  doc.text('DATA DA ATUALIZAÇÃO:', PAGE_WIDTH - MARGIN_RIGHT, FOOTER_Y, { align: 'right' });

  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...TEXT_DARK);
  doc.text(diretoriaNome || processo.diretoria || '—', MARGIN_LEFT, FOOTER_Y + 4, { maxWidth: 60 });
  doc.text(processo.versao || '1.0', PAGE_WIDTH / 2, FOOTER_Y + 4, { align: 'center' });
  doc.text(formatDate(processo.updated_at), PAGE_WIDTH - MARGIN_RIGHT, FOOTER_Y + 4, { align: 'right' });

  // Linha discreta com paginação
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...MUTED_GRAY);
  doc.text(`Página ${pageNum} de ${totalPages}`, PAGE_WIDTH / 2, FOOTER_Y + 10, { align: 'center' });
}

// ============================================================
// PDF MAIN
// ============================================================
export function generateProcessoNegocioPDF(processo: ProcessoNegocio, diretoriaNome?: string) {
  ...
  drawTextCell(doc, 'PROPRIETÁRIO:', MARGIN_LEFT, y, govColW, govHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  drawTextCell(doc, 'ATORES:', MARGIN_LEFT + govColW, y, govColW, govHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  drawTextCell(doc, 'ÁREAS RESPONSÁVEIS', MARGIN_LEFT + govColW * 2, y, govColW, govHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  y += govHeaderH;
  ...
  renderBulletList(doc, processo.proprietarios || [], MARGIN_LEFT, y, govColW, govRowH);
  renderBulletList(doc, processo.atores || [], MARGIN_LEFT + govColW, y, govColW, govRowH);
  renderBulletList(doc, processo.areas_responsaveis || [], MARGIN_LEFT + govColW * 2, y, govColW, govRowH);
  y += govRowH + 6;
  ...
  drawTextCell(doc, 'ENTRADA', MARGIN_LEFT, y, infColW, infHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  drawTextCell(doc, 'SAÍDA', MARGIN_LEFT + infColW, y, infColW, infHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  y += infHeaderH;
  ...
  renderBulletList(doc, processo.entradas || [], MARGIN_LEFT, y, infColW, infRowH);
  renderBulletList(doc, processo.saidas || [], MARGIN_LEFT + infColW, y, infColW, infRowH);
  y += infRowH + 6;
  ...
  drawTextCell(doc, 'SISTEMAS / FERRAMENTAS', MARGIN_LEFT, y, infColW, infHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  drawTextCell(doc, 'NORMATIVO / REFERÊNCIAS', MARGIN_LEFT + infColW, y, infColW, infHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
  y += infHeaderH;
  ...
  renderBulletList(doc, processo.sistemas_ferramentas || [], MARGIN_LEFT, y, infColW, recRowH);
  renderBulletList(doc, processo.normativos_referencias || [], MARGIN_LEFT + infColW, y, infColW, recRowH);
  y += recRowH + 6;

  // 6. Modelagem / Fluxograma
  y = checkPageBreak(doc, y, 30);
  y = drawNumberedSectionHeader(doc, 6, 'Modelagem / Fluxograma', y);
  if (processo.fluxograma_data && processo.fluxograma_mime?.startsWith('image/')) {
    // Embedar imagem mantendo proporção
    try {
      const fmt = processo.fluxograma_mime.includes('png') ? 'PNG' : processo.fluxograma_mime.includes('webp') ? 'WEBP' : 'JPEG';
      ...
      doc.addImage(processo.fluxograma_data, fmt as any, MARGIN_LEFT, y + 2, imgW, imgH, undefined, 'FAST');
      doc.setDrawColor(...BORDER_GRAY);
      doc.rect(MARGIN_LEFT, y + 2, imgW, imgH, 'S');
      y += imgH + 6;
    } catch (e) {
      y = drawMultilineContent(doc, `Fluxograma anexado: ${processo.fluxograma_filename || ''} (não foi possível incorporar a imagem no PDF)`, y, 16);
    }
  } else if (processo.fluxograma_data && processo.fluxograma_mime === 'application/pdf') {
    y = drawMultilineContent(doc, `Fluxograma em PDF anexado: ${processo.fluxograma_filename || ''}`, y, 16);
  } else {
  ...
  if ((processo.documentos_anexados || []).length === 0) {
    ...
    drawTextCell(doc, 'TIPO', MARGIN_LEFT, y, colTipo, docHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
    drawTextCell(doc, 'DOCUMENTO', MARGIN_LEFT + colTipo, y, colDoc, docHeaderH, { bold: true, fontSize: 8.5, align: 'center', bg: ACCENT_BG_LIGHT, color: BLUE_TITLE });
    y += docHeaderH;
    for (const docAnx of processo.documentos_anexados) {
      ...
      drawTextCell(doc, docAnx.tipo, MARGIN_LEFT, y, colTipo, rowH, { align: 'center', bold: true });
      drawTextCell(doc, docAnx.nome, MARGIN_LEFT + colTipo, y, colDoc, rowH, { align: 'center' });
      y += rowH;
    }
  }
  y += 6;
  ...
  const histLinhas = [
    { label: 'Validação do Autor', valor: processo.validado_autor_em && processo.validado_autor_nome
        ? `${processo.validado_autor_nome} — ${formatDateTime(processo.validado_autor_em)}` : 'Pendente' },
    { label: 'Validação da Diretoria', valor: processo.validado_diretoria_em && processo.validado_diretoria_nome
        ? `${processo.validado_diretoria_nome} — ${formatDateTime(processo.validado_diretoria_em)}` : 'Pendente' },
    { label: 'Validação Final', valor: processo.validado_final_em && processo.validado_final_nome
        ? `${processo.validado_final_nome} — ${formatDateTime(processo.validado_final_em)}` : 'Pendente' },
  ];
  for (const linha of histLinhas) {
    drawTextCell(doc, linha.label, MARGIN_LEFT, y, histLabelW, histRowH, { bold: true, fontSize: 9, bg: [248, 250, 252] });
    drawTextCell(doc, linha.valor, MARGIN_LEFT + histLabelW, y, CONTENT_WIDTH - histLabelW, histRowH, { fontSize: 9 });
    y += histRowH;
  }
  ...
  for (let p = 1; p <= totalPages; p++) {
    doc.setPage(p);
    drawRodapeInstitucional(doc, processo, diretoriaNome || processo.diretoria || '', p, totalPages);
  }
