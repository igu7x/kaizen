...
function addFooter(
  ...
  doc.text(
    `TEP  —  Termo de Encerramento do Projeto  —  Versão ${versao}.0${idPart}`,
    MARGIN_LEFT,
    FOOTER_Y,
  );
  doc.text(
...
export function generateTEPPdf(
  projeto: Projeto,
  tep: Tep,
  entregas: Entrega[],
) {
  ...
  const areaText =
    (projeto as any).areas_execucao_diretorias || projeto.diretoria || "";
  ...
  doc.setTextColor(...TEXT_DARK);
  doc.text(
    doc.splitTextToSize(`Status: ${tepStatus}`, leftMaxW),
    MARGIN_LEFT + 5,
    areaEndY + 7,
  );
  ...
  y = drawMultilineContent(
    doc,
    `(${tep.tipo_encerramento === "concluido" ? "X" : " "}) Concluído     (${isDescontinuado ? "X" : " "}) Descontinuado`,
    y,
    12,
  );
  y += 6;
  ...
  if (isDescontinuado) {
    ...
    y = drawNumberedSectionHeader(
      doc,
      secao,
      "Motivo da Descontinuidade do Projeto",
      y,
    );
    y = drawMultilineContent(
      doc,
      tep.motivo_cancelamento || "Não informado",
      y,
      25,
    );
    y += 6;
    ...
    for (const entrega of entregas) {
      const dataConclusao =
        entrega.status === "concluida"
          ? formatDate(entrega.updated_at as any)
          : "";
  ...
  y = drawNumberedSectionHeader(
    doc,
    secao++,
    "Documentos Gerados pela Execução do Projeto",
    y,
  );
  ...
  if (evidencias.length === 0) {
    y = drawMultilineContent(
      doc,
      "Nenhuma evidência anexada às entregas.",
      y,
      12,
    );
  } else {
    ...
    drawTextCell(doc, "Documento", MARGIN_LEFT, y, colDocNome, evidHeaderH, {
      bold: true,
      fontSize: 9,
      align: "center",
      bg: ACCENT_BG_LIGHT,
