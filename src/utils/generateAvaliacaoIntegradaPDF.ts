...
function addFooter(doc: jsPDF, pageNum: number, totalPages: number, footerLabel: string) {
  ...
  doc.text(footerLabel, MARGIN_LEFT, FOOTER_Y, { maxWidth: CONTENT_WIDTH - 40 });
...
function drawIntegradaCard(
  ...
  const { numero, nome, descricao, notaAutoLabel, notaGestorLabel, notaIntegradaLabel, comentario } = options;
  let y = options.y;
  ...
  // Header das 3 notas
  drawTextCell(doc, 'Autoavaliação', MARGIN_LEFT, y, notaW, notaH, { fontSize: 9, bg: HEADER_BLOCK_BG, align: 'center', bold: true });
  drawTextCell(doc, 'Avaliação do Gestor', MARGIN_LEFT + notaW, y, notaW, notaH, { fontSize: 9, bg: HEADER_BLOCK_BG, align: 'center', bold: true });
  drawTextCell(doc, 'Nota Integrada', MARGIN_LEFT + notaW * 2, y, notaW, notaH, { fontSize: 9, bg: HEADER_BLOCK_BG, align: 'center', bold: true });
  y += notaH;

  // Valores
  drawTextCell(doc, notaAutoLabel, MARGIN_LEFT, y, notaW, notaH, { fontSize: 9, align: 'center' });
  drawTextCell(doc, notaGestorLabel, MARGIN_LEFT + notaW, y, notaW, notaH, { fontSize: 9, align: 'center' });
  drawTextCell(doc, notaIntegradaLabel, MARGIN_LEFT + notaW * 2, y, notaW, notaH, { fontSize: 9, align: 'center', bold: true });
  y += notaH;
...
export function generateAvaliacaoIntegradaPDF(formulario: AvaliacaoIntegradaFormulario) {
  const doc = new jsPDF('p', 'mm', 'a4');
  let y = 0;

  const tipoInventario = formulario.tipo_inventario || 'equipe';
  const tipoLabel = tipoInventario === 'gestor' ? 'Matriz de Competências do Gestor' : 'Matriz de Competências da Equipe';
  ...
  doc.text(doc.splitTextToSize(colabText, leftMaxW), MARGIN_LEFT + 5, lotacaoEndY + 8);
  ...
  const brasaoW = 18;
  const brasaoH = 22;
  doc.addImage(LOGO_BRANCO_4K_BASE64, 'PNG', rightCenterX - brasaoW / 2, 10, brasaoW, brasaoH, undefined, 'FAST');
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(255, 255, 255);
  ...
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.text('Tribunal de Justiça do Estado de Goiás', rightCenterX, 43, { align: 'center' });
  ...
  validationItems.push({ label: 'Avaliador', value: formulario.avaliador_nome || '' });
  validationItems.push({ label: 'Colaborador', value: formulario.pessoa_nome || '' });

  if (formulario.validado_gestor_nome) {
    const label1 = tipoInventario === 'gestor' ? 'Validação da Liderança' : 'Validação do Gestor';
  ...
  if (formulario.validado_colaborador_nome) {
    const label2 = tipoInventario === 'gestor' ? 'Validação do Gestor' : 'Validação do Colaborador';
    validationItems.push({
      label: label2,
  ...
  for (const secao of tiposSecao) {
    const respostasSecao = respostas.filter(r => (r.tipo || 'tecnica') === secao.tipo);
    if (respostasSecao.length === 0) continue;

    y = checkPageBreak(doc, y, 24);
    y = drawSectionTitleBar(doc, `${secao.titulo} (${respostasSecao.length})`, y);

    for (let i = 0; i < respostasSecao.length; i++) {
      const resp = respostasSecao[i];

      doc.setFontSize(9.5);
      const linesEstim = doc.splitTextToSize((resp.competencia_descricao || '') + (resp.comentario || ''), CONTENT_WIDTH - 40);
      const estimatedHeight = 10 + 9 + 9 + linesEstim.length * 4.5 + 18;
      y = checkPageBreak(doc, y, estimatedHeight);

      const tipoLabels = NOTA_LABELS[resp.tipo || secao.tipo] || NOTA_LABELS.tecnica;
      y = drawIntegradaCard(doc, {
        numero: i + 1,
        nome: resp.competencia_nome,
        descricao: resp.competencia_descricao || undefined,
        notaAutoLabel: tipoLabels[resp.nota_autoavaliacao] || `${resp.nota_autoavaliacao}`,
        notaGestorLabel: tipoLabels[resp.nota_gestor] || `${resp.nota_gestor}`,
        notaIntegradaLabel: tipoLabels[resp.nota_integrada] || `${resp.nota_integrada}`,
