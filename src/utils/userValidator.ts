 ...
 */
export const DIRETORIAS_VALIDAS = [
  "SGJT",
  "DPE",
  "DIJUD",
  "DITI",
 ...
 */
export function isDiretoriaValida(
  diretoria: string | null | undefined,
): diretoria is DiretoriaValida {
 ...
 */
export function normalizeDiretoria(
  diretoria: string | null | undefined,
): string | null {
...
export function syncUserDiretoria(user: Partial<User>): User {
  ...
  if (!validation.isConsistent) {
    console.warn(
      "[UserValidator] Inconsistência detectada:",
      validation.message,
    );
  }
...
export function validateUser(user: Partial<User>): UserValidationResult {
  ...
  if (!user.role) {
    ...
    errors.push(
      `Role inválida: ${user.role}. Valores válidos: VIEWER, MANAGER, ADMIN`,
    );
  }
...
export function validateAndFixLocalStorageUser(): UserValidationResult | null {
  try {
    ...
    if (validation.warnings.length > 0) {
      console.warn(
        "[UserValidator] Avisos ao validar usuário:",
        validation.warnings,
      );
    }
...
export function validateLoginResponse(userData: any): User {
  ...
  if (!validation.isValid) {
    throw new Error(
      `Dados de usuário inválidos: ${validation.errors.join(", ")}`,
    );
  }

  if (validation.warnings.length > 0) {
    console.warn(
      "[UserValidator] Avisos nos dados do usuário:",
      validation.warnings,
    );
  }
 ...
 */
export function getUserDiretoria(
  user: Partial<User> | null | undefined,
): DiretoriaValida | null {
  if (!user) return null;
