...
import { Loader2, FileText, Download, Eye, CheckCircle2, XCircle, Pencil, History, FileDown } from 'lucide-react';
...
export function TepDialog({ open, onOpenChange, projeto, entregas: entregasProp, onFinalized }: TepDialogProps) {
  const { user } = useAuth();
  const currentUserId = user?.id ? Number(user.id) : null;
  const isSuperadmin = (user as any)?.is_superadmin === true || (user as any)?.is_domain_root === true;
  ...
  const [consideracoesPatrocinador, setConsideracoesPatrocinador] = useState('');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (tep) {
        ...
        setMotivo(tep.tipo_encerramento === 'cancelado' ? (tep.motivo_cancelamento || '') : '');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        {tepExistente?.tep_recusado_em && !tepExistente?.tep_validado_gestor_em && (
          <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 flex items-start gap-3">
            <XCircle className="h-5 w-5 text-red-600 flex-shrink-0 mt-0.5" />
            <div className="flex-1 text-sm">
              <p className="font-semibold text-red-900">
                TEP recusado pelo {tepExistente.tep_recusado_camada === 'patrocinador' ? 'Patrocinador' : 'Diretor'}
              </p>
              <p className="text-red-700 text-xs mt-0.5">
                {tepExistente.tep_recusado_por_nome ? `Por ${tepExistente.tep_recusado_por_nome} — ` : ''}
                {formatDate(tepExistente.tep_recusado_em)}
              </p>
              {tepExistente.tep_recusado_comentario && (
                <p className="text-red-800 text-sm mt-2 italic">"{tepExistente.tep_recusado_comentario}"</p>
              )}
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 text-sm">
              ...
              <p className="text-emerald-700 text-xs">
                Validado em {formatDate(tepExistente.tep_validado_patrocinador_em)}
                {tepExistente.tep_versao ? ` — versão ${tepExistente.tep_versao}` : ''}
              </p>
            </div>
            <Button
              size="sm"
        ...
        {loadingExisting ? (
          ...
          <div className="space-y-6 py-2">
            ...
            {tepExistente && (
              <div className="bg-white border border-gray-200 rounded-lg p-3">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-xs font-semibold text-gray-700">Validação do TEP</h4>
                  ...
                  {!tepExistente.tep_validado_patrocinador_em && (
                    <Button size="sm" variant="outline" onClick={handleVisualizarPdf} className="h-7 text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className={cn(
                    ...
                    tepExistente.tep_validado_gestor_em ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
                  )}>
                    <p className="text-[10px] text-gray-500 mb-0.5">Camada 1 — Gestor</p>
                    <p className="text-xs font-medium truncate">{projeto.gestor_nome || '-'}</p>
                    {tepExistente.tep_validado_gestor_em ? (
                      <div className="mt-1">
                        <Badge className="bg-green-600 text-white text-[10px] h-5">Validado</Badge>
                        <p className="text-[9px] text-gray-400 mt-0.5">{formatDate(tepExistente.tep_validado_gestor_em)}</p>
                      </div>
                    ) : currentUserId && (projeto as any).gestor_user_id && Number((projeto as any).gestor_user_id) === currentUserId ? (
                      <Button
                        size="sm"
                      ...
                      >
                        {validandoCamada === 1 ? <Loader2 className="h-3 w-3 animate-spin" /> : <><CheckCircle2 className="h-3 w-3 mr-1" /> Validar</>}
                      </Button>
                    ) : (
                  ...
                  <div className={cn(
                    'flex-1 rounded-lg border p-2 text-center',
                    tepExistente.tep_validado_diretor_em ? 'bg-green-50 border-green-200'
                      : !tepExistente.tep_validado_gestor_em ? 'bg-gray-100 border-gray-200 opacity-50'
                      : 'bg-gray-50 border-gray-200'
                  )}>
                    <p className="text-[10px] text-gray-500 mb-0.5">Camada 2 — Diretor</p>
                    <p className="text-xs font-medium truncate">{projeto.diretoria || '-'}</p>
                    {tepExistente.tep_validado_diretor_em ? (
                      <div className="mt-1">
                        <Badge className="bg-green-600 text-white text-[10px] h-5">Validado</Badge>
                        <p className="text-[9px] text-gray-400 mt-0.5">{formatDate(tepExistente.tep_validado_diretor_em)}</p>
                      </div>
                    ) : tepExistente.tep_validado_gestor_em && currentUserId && (projeto as any).diretor_user_id && Number((projeto as any).diretor_user_id) === currentUserId ? (
                      <div className="mt-1 space-y-1">
                        ...
                        >
                          {validandoCamada === 2 ? <Loader2 className="h-3 w-3 animate-spin" /> : <><CheckCircle2 className="h-3 w-3 mr-1" /> Validar</>}
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                      ...
                      <Badge variant="outline" className="text-[10px] text-gray-400 mt-1 h-5">
                        {tepExistente.tep_validado_gestor_em ? 'Pendente' : 'Bloqueado'}
                      </Badge>
                    )}
                  </div>
                  ...
                  <div className={cn(
                    'flex-1 rounded-lg border p-2 text-center',
                    tepExistente.tep_validado_patrocinador_em ? 'bg-green-50 border-green-200'
                      : !tepExistente.tep_validado_diretor_em ? 'bg-gray-100 border-gray-200 opacity-50'
                      : 'bg-gray-50 border-gray-200'
                  )}>
                    <p className="text-[10px] text-gray-500 mb-0.5">Camada 3 — Patrocinador</p>
                    <p className="text-xs font-medium truncate">{projeto.patrocinador_nome || '-'}</p>
                    {tepExistente.tep_validado_patrocinador_em ? (
                      <div className="mt-1">
                        <Badge className="bg-green-700 text-white text-[10px] h-5 cursor-pointer" onClick={handleVisualizarPdf}>
                          TEP Vigente
                        </Badge>
                        <p className="text-[9px] text-gray-400 mt-0.5">{formatDate(tepExistente.tep_validado_patrocinador_em)}</p>
                      </div>
                    ) : tepExistente.tep_validado_diretor_em && currentUserId && (projeto as any).patrocinador_user_id && Number((projeto as any).patrocinador_user_id) === currentUserId ? (
                      <div className="mt-1 space-y-1">
                        <Button
                          size="sm"
                        ...
                        >
                          {validandoCamada === 3 ? <Loader2 className="h-3 w-3 animate-spin" /> : <><CheckCircle2 className="h-3 w-3 mr-1" /> Validar</>}
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                      ...
                      <Badge variant="outline" className="text-[10px] text-gray-400 mt-1 h-5">
                        {tepExistente.tep_validado_diretor_em ? 'Pendente' : 'Bloqueado'}
                      </Badge>
                    )}
                  </div>
                </div>
            ...
            <section>
              <Label className="text-sm font-semibold">Tipo de Encerramento <span className="text-red-500">*</span></Label>
              <div className="flex gap-6 mt-2">
                <label className={cn('flex items-center gap-2', camposEditaveis ? 'cursor-pointer' : 'cursor-default opacity-70')}>
                  <input
                    type="radio"
            ...
            {tipo === 'cancelado' && (
              <section>
                <Label htmlFor="motivo-descontinuidade" className="text-sm font-semibold">
                  Motivo da Descontinuidade <span className="text-red-500">*</span>
                </Label>
                <Textarea
                  ...
                  maxLength={500}
                />
                <p className="text-xs text-gray-400 text-right mt-1">{motivo.length}/500</p>
              </section>
            )}
            ...
            {tipo === 'concluido' && (
              <>
                <section>
                  <Label htmlFor="consideracoes-gestor" className="text-sm font-semibold">
                  ...
                  <Textarea
                    ...
                    maxLength={500}
                  />
                  <p className="text-xs text-gray-400 text-right mt-1">{consideracoesGerente.length}/500</p>
                </section>

                <section>
                  <Label htmlFor="consideracoes-patrocinador" className="text-sm font-semibold">
                    Considerações Finais do Patrocinador do Projeto
                  </Label>
                  <Textarea
                    id="consideracoes-patrocinador"
                    value={consideracoesPatrocinador}
                    onChange={e => setConsideracoesPatrocinador(e.target.value)}
                    ...
                    maxLength={500}
                  />
                  <p className="text-xs text-gray-400 text-right mt-1">{consideracoesPatrocinador.length}/500</p>
                </section>
              </>
            )}
          </div>
        ...
        <DialogFooter className="gap-2">
          ...
          {isSuperadmin && tepExistente && editMode && (
            <Button variant="outline" onClick={() => setEditMode(false)} disabled={saving} className="border-gray-300 text-gray-700">
              Cancelar Edição
            </Button>
          )}
          {isSuperadmin && (
            ...
            >
              {saving
                ...
                : (tepExistente && !editMode ? <Pencil className="h-4 w-4 mr-2" /> : null)}
              {!tepExistente
                ? (tipo === 'concluido' ? 'Finalizar como Concluído' : 'Finalizar como Descontinuado')
                : editMode
                  ? 'Salvar Alterações'
                  : 'Editar TEP'}
            </Button>
      ...
      <Dialog open={recusaDialogOpen} onOpenChange={(o) => {
        if (!o) { setRecusaDialogOpen(false); setRecusaCamada(null); setRecusaComentario(''); }
      }}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-3 py-2">
            <p className="text-sm text-gray-600">
              O TEP voltará para o gestor reiniciar o ciclo de validação. Todas as validações
              anteriores serão resetadas. Você pode incluir um comentário (opcional) para indicar
              o que precisa ser ajustado.
            </p>
            <div>
              <Label htmlFor="recusa-tep-comentario">Comentário (opcional)</Label>
              <Textarea
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRecusaDialogOpen(false)} disabled={recusandoTEP}>
              Cancelar
            </Button>
            <Button onClick={handleRecusarTEP} disabled={recusandoTEP} className="bg-red-600 hover:bg-red-700 text-white">
              {recusandoTEP ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <XCircle className="h-4 w-4 mr-2" />}
              Recusar TEP
            </Button>
          </DialogFooter>
        </DialogContent>
