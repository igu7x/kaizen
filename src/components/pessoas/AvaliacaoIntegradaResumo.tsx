...
import { CheckCircle2, ShieldCheck, Loader2, Clock, Lock, FileDown } from 'lucide-react';
import {
...
function NotaBadge({ nota, labels }: { nota: number; labels: Record<number, string> }) {
  return (
        ...
        <div className="space-y-3">
          {(descOverride || resposta.competencia_descricao) && (
            <div>
              ...
              <p className="text-gray-800 mt-0.5 whitespace-pre-wrap [overflow-wrap:anywhere]">{descOverride || resposta.competencia_descricao}</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-lg border border-emerald-200">
              <span className="text-sm font-medium text-gray-700">Integrada (consenso):</span>
          ...
          {resposta.comentario && (
            <div>
              ...
              <p className="text-gray-800 mt-0.5 [overflow-wrap:anywhere]">{resposta.comentario}</p>
            </div>
          )}
        </div>
      </CardContent>
...
export function AvaliacaoIntegradaResumo({ formulario, onValidated, onEdit, tipoInventario = 'equipe', currentUserId }: AvaliacaoIntegradaResumoProps) {
  const [validando, setValidando] = useState<'gestor' | 'colaborador' | null>(null);
  ...
  const labelCamada1 = tipoInventario === 'gestor' ? 'Validação da Liderança' : 'Validação do Gestor';
  const labelCamada2 = tipoInventario === 'gestor' ? 'Validação do Gestor' : 'Validação do Colaborador';
  const labelCamada2Pendente = tipoInventario === 'gestor' ? 'Aguardando validação do gestor' : 'Aguardando validação do colaborador';
  ...
  const canValidateCamada1 = !currentUserId || formulario.avaliador_user_id === currentUserId;
  // Camada 2: o colaborador/gestor avaliado pode validar
  const canValidateCamada2 = !currentUserId || (formulario as any).colaborador_user_id === currentUserId;
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-gray-900 font-semibold">Avaliação Integrada enviada com sucesso</p>
            ...
            <p className="text-gray-500 text-sm">Colaborador: {formulario.pessoa_nome}</p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-xl bg-purple-50 border border-purple-200 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {onEdit && !!currentUserId && formulario.avaliador_user_id === currentUserId && (
                <Button
                  variant="outline"
      ...
      {onEdit && !gestorValidado && !colaboradorValidado && !!currentUserId && formulario.avaliador_user_id === currentUserId && (
      ...
      <div className="space-y-3">
        ...
        <div className={`rounded-xl border p-4 ${gestorValidado ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className={`font-medium ${gestorValidado ? 'text-emerald-800' : 'text-amber-800'}`}>
                  1. {labelCamada1}
                </p>
                {gestorValidado ? (
                  <p className="text-sm text-emerald-600">
                    Validado por {formulario.validado_gestor_nome} em {formatDate(formulario.validado_gestor_em!)}
                  </p>
                ) : (
            ...
            {!gestorValidado && canValidateCamada1 && (
              ...
              >
                {validando === 'gestor' ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <ShieldCheck className="h-4 w-4 mr-2" />}
                Validar
              </Button>
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {colaboradorValidado ? (
                  <p className="text-sm text-emerald-600">
                    Validado por {formulario.validado_colaborador_nome} em {formatDate(formulario.validado_colaborador_em!)}
                  </p>
                ) : gestorValidado ? (
                  <p className="text-sm text-amber-600">Pendente de validação</p>
                ) : (
                  <p className="text-sm text-gray-400">{labelCamada2Pendente}</p>
                )}
              </div>
            </div>
            {gestorValidado && !colaboradorValidado && canValidateCamada2 && (
              <Button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white"
              >
                {validando === 'colaborador' ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <ShieldCheck className="h-4 w-4 mr-2" />}
                Validar
              </Button>
            )}
          </div>
