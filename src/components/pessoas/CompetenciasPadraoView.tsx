...
import { ArrowLeft, Loader2, BookOpen, Users, UserCog, FileDown } from 'lucide-react';
...
const SECAO_LABEL: Record<string, { titulo: string; descricao: string; cor: string }> = {
  comportamental: {
...
export function CompetenciasPadraoView({ podeAlternarTipo, onVoltar }: CompetenciasPadraoViewProps) {
  const [loading, setLoading] = useState(true);
  const [tipoSelecionado, setTipoSelecionado] = useState<TipoFormulario>('equipe');
  ...
  useEffect(() => {
    ...
    competenciasPadraoApi.getAll()
      ...
      .catch(() => { /* silencioso — exibe vazio */ })
      .finally(() => { if (ativo) setLoading(false); });
    competenciasPadraoApi.getVersaoAtual()
      .then(v => { if (ativo) setVersaoAtual(v.versao); })
      .catch(() => { /* opcional — só usado no rodapé do PDF */ });
    return () => { ativo = false; };
  }, []);

  // Quando NÃO pode alternar, força tipo equipe (que mostra apenas comportamentais)
  const tipoExibido: TipoFormulario = podeAlternarTipo ? tipoSelecionado : 'equipe';

  const secoes = tipoExibido === 'equipe'
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={onVoltar} className="text-gray-600 hover:text-gray-900 hover:bg-gray-100">
            <ArrowLeft className="h-4 w-4 mr-1" /> Voltar
          </Button>
          <h2 className="text-2xl font-bold text-gray-900">Visualizar Competências Padrão</h2>
        </div>
        <Button
      ...
      <p className="text-sm text-gray-600 max-w-3xl">
        Catálogo das competências padrão que compõem o formulário do referencial. Esta visualização é somente leitura.
      </p>
            ...
            return (
              <section key={secao.chave} className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">
                      {meta.titulo} <span className="text-sm font-normal text-gray-500">({secao.items.length})</span>
                    </h3>
                ...
                {secao.items.length === 0 ? (
                  <p className="text-sm text-gray-500 italic px-2">Nenhuma competência cadastrada.</p>
                ) : (
                      ...
                      <Card key={c.id} className="border border-gray-200 shadow-sm">
                        <CardHeader className="pb-2">
                          <CardTitle className="text-base flex items-start gap-2">
                            <span className={`${meta.cor} font-bold flex-shrink-0`}>{idx + 1}.</span>
                            <span className="flex-1">{c.nome}</span>
                          </CardTitle>
                        </CardHeader>
                        <CardContent>
                          <p className="text-sm text-gray-700 whitespace-pre-wrap [overflow-wrap:anywhere]">{c.descricao}</p>
                        </CardContent>
                      </Card>
                    ))}
                  </div>
