...
export function FormFiller() {
  ...
  const handleSubmit = async () => {
    ...
    if (missingFields.length > 0) {
      toast.warning(
        `Por favor, preencha todos os campos obrigatórios: ${missingFields.map((f) => f.label).join(", ")}`,
      );
      return;
    }
    ...
    if (
      !confirm(
        "Tem certeza que deseja enviar este formulário? Após o envio, não será possível editar as respostas.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (
        ...
        error.message?.includes("já respondeu")
      ) {
        toast.warning("Você já respondeu este formulário.");
      } else if (error.status === 400) {
        toast.warning(
          "Dados inválidos. Verifique as respostas e tente novamente.",
        );
  ...
  const renderField = (field: FormField) => {
    ...
    const rawType =
      field.type || (field as any).fieldType || (field as any).field_type || "";
    ...
    switch (fieldType) {
          ...
          <div key={fieldId} className="space-y-2">
            ...
            {isReadOnly ? (
              ...
              <Input
                ...
                placeholder={
                  field.config?.placeholder || "Texto de resposta curta"
                }
                required={isRequired}
              />
            )}
          ...
          <div key={fieldId} className="space-y-2">
            ...
            {isReadOnly ? (
              ...
              <Textarea
                ...
                placeholder={
                  field.config?.placeholder || "Texto de resposta longa"
                }
                required={isRequired}
                rows={4}
              />
          ...
          <div key={fieldId} className="space-y-2">
            ...
            {isReadOnly ? (
              <div className="p-3 bg-gray-100 rounded-md border border-gray-200">
                {(() => {
                  const selectedOption = (field.config?.options || []).find(
                    (opt: FormFieldOption) => opt.value === value,
                  );
          ...
          <div key={fieldId} className="space-y-2">
            ...
            {isReadOnly ? (
              <div className="p-3 bg-gray-100 rounded-md border border-gray-200">
                ...
                {!value && (
                  <em className="text-gray-400 block mt-2">Não respondido</em>
                )}
              </div>
            ) : (
              <div className="flex items-center gap-4">
                ...
                <RadioGroup
                  ...
                  onValueChange={(val) =>
                    setAnswers({ ...answers, [fieldId]: parseInt(val) })
                  }
            ...
            {isReadOnly ? (
              ...
              <Input
                ...
                onChange={(e) =>
                  setAnswers({
                    ...answers,
                    [fieldId]: parseFloat(e.target.value),
                  })
                }
        ...
        return (
          <div key={fieldId} className="space-y-2">
            ...
            {isReadOnly ? (
              <div className="p-3 bg-gray-100 rounded-md border border-gray-200">
                <span className="text-gray-800">
                  {value || <em className="text-gray-400">Não respondido</em>}
                </span>
              </div>
            ) : (
              <>
      ...
      default:
        console.warn(
          `[FormFiller] ⚠️ Unsupported field type: "${fieldType}" for field "${field.label}"`,
          field,
        );
        return (
          <div
            key={fieldId}
            className="space-y-2 p-4 border border-red-200 rounded bg-red-50"
          >
            <Label className="text-red-700">
              {field.label} (Tipo não suportado: {fieldType || "vazio"})
            </Label>
            <p className="text-sm text-red-600">
              Este tipo de campo não é reconhecido. Dados do campo: type="
              {field.type}", fieldType="{(field as any).fieldType}",
  ...
  return (
    <Layout>
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <Card
          className={isReadOnly ? "bg-green-50 border-green-200" : "bg-blue-50"}
        >
          <CardHeader>
            <div className="flex items-center justify-between">
            ...
            {isReadOnly && response && (
              <div className="mt-4 p-4 bg-green-100 border border-green-300 rounded-lg">
                ...
                <p className="text-xs text-green-600 mt-2">
                  As respostas abaixo são somente para visualização e não podem
                  ser editadas.
                </p>
              </div>
            )}
        ...
        {/* Campos sem seção */}
        {form.fields.filter((f) => !f.sectionId || f.sectionId === "undefined")
          .length > 0 && (
          <Card>
        ...
        {form.sections
          ...
          .map((section) => {
            ...
            if (sectionFields.length === 0) {
              if (process.env.NODE_ENV === "development") {
                console.warn(
                  `[FormFiller] ⚠ Section "${section.title}" has NO fields`,
                );
              }
              return null;
            }
