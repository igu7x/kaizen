...
import { competenciasGestorApi, CompetenciaPorUnidade, UnidadeAutorizada } from '@/services/competenciasGestorApi';
import { autoavaliacaoApi, AutoavaliacaoFormulario } from '@/services/autoavaliacaoApi';
// pessoasApi removido - nome agora é digitado pelo usuário
import { ESCALA_NOTAS, ESCALA_COMPORTAMENTAL, ESCALA_ESTRATEGICA, ESCALA_GERENCIAL, COMPETENCIAS_COMPORTAMENTAIS, COMPETENCIAS_ESTRATEGICAS, COMPETENCIAS_GERENCIAIS } from '@/constants/competencias';
...
export function AutoavaliacaoForm({ onSubmitted, onViewResposta, tipoInventario, editMode }: AutoavaliacaoFormProps) {
  const { user } = useAuth();
  ...
  const [jaPreenchido, setJaPreenchido] = useState<AutoavaliacaoFormulario | null>(null);
  const [forceEditMode, setForceEditMode] = useState(false);
  const [savedVersaoAnterior, setSavedVersaoAnterior] = useState<number | null>(null);
  // Chaves (nome||descricao) de competências padrão adicionadas/alteradas — só essas ficam editáveis
  const [changedCompKeys, setChangedCompKeys] = useState<Set<string>>(new Set());
  // Chaves (nome||descricao) de competências técnicas adicionadas/alteradas no referencial
  const [changedTecnicasKeys, setChangedTecnicasKeys] = useState<Set<string>>(new Set());

  const [competenciasUnidade, setCompetenciasUnidade] = useState<CompetenciaPorUnidade[]>([]);
  const [loadingCompetencias, setLoadingCompetencias] = useState(false);

  // Competências padrão carregadas da API (com fallback para constantes)
  const [compComportamentais, setCompComportamentais] = useState(COMPETENCIAS_COMPORTAMENTAIS);
  const [compEstrategicas, setCompEstrategicas] = useState(COMPETENCIAS_ESTRATEGICAS);
  ...
  useEffect(() => {
    competenciasPadraoApi.getAll().then(data => {
      if (data.comportamental?.length) setCompComportamentais(data.comportamental.map(c => ({ nome: c.nome, descricao: c.descricao })));
      if (data.estrategica?.length) setCompEstrategicas(data.estrategica.map(c => ({ nome: c.nome, descricao: c.descricao })));
      if (data.gerencial?.length) setCompGerenciais(data.gerencial.map(c => ({ nome: c.nome, descricao: c.descricao })));
      competenciasPadraoApi.getVersaoAtual().then(v => setCompVersao(v.versao)).catch(() => {});
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setForm(prev => {
      const merge = (lista: { nome: string; descricao: string }[], atuais: typeof prev.respostas_comportamentais) =>
        lista.map(c => {
      ...
      return {
        ...prev,
        respostas_comportamentais: merge(compComportamentais, prev.respostas_comportamentais),
        respostas_estrategicas: merge(compEstrategicas, prev.respostas_estrategicas),
        respostas_gerenciais: merge(compGerenciais, prev.respostas_gerenciais),
      };
    });
  }, [compComportamentais, compEstrategicas, compGerenciais, editMode, forceEditMode]);
  ...
  const loadCompetenciasEdit = async (unidadeId: number, respostasExistentes: any[], skipChangeDetection = false) => {
    setLoadingCompetencias(true);
    try {
      const competencias = tipoInventario === 'gestor'
        ? await competenciasGestorApi.getCompetenciasGestorPorUnidade(unidadeId)
      ...
      const novasRespostas = competencias.map(c => {
        ...
        const exactIdx = respostasExistentes.findIndex((r: any, i: number) =>
          !usedIdx.has(i) && isTecResp(r) &&
        ...
        const nameIdx = respostasExistentes.findIndex((r: any, i: number) =>
        ...
        const posIdx = respostasExistentes.findIndex((r: any, i: number) =>
          !usedIdx.has(i) && isTecResp(r)
        );
        if (posIdx !== -1) {
          usedIdx.add(posIdx);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        try {
          formularioExistente = await autoavaliacaoApi.getMeu(tipoInventario || 'equipe');
        ...
        if ((editMode || forceEditMode) && formularioExistente) {
          const fullForm = await autoavaliacaoApi.getById(formularioExistente.id);
          if (fullForm) {
            ...
            try {
              ...
              if (padrao.comportamental?.length) padraoComp = padrao.comportamental.map(c => ({ nome: c.nome, descricao: c.descricao }));
              if (padrao.estrategica?.length) padraoEstr = padrao.estrategica.map(c => ({ nome: c.nome, descricao: c.descricao }));
              if (padrao.gerencial?.length) padraoGer = padrao.gerencial.map(c => ({ nome: c.nome, descricao: c.descricao }));
            ...
            setForm(prev => ({
              ...
              email_institucional: fullForm.email_institucional || prev.email_institucional,
              unidade_id: String(fullForm.unidade_id || ''),
              unidade_path: fullForm.unidade_id ? [String(fullForm.unidade_id)] : [],
              respostas: (fullForm.respostas || []).filter((r: any) => r.tipo === 'tecnica' || !r.tipo).map((r: any) => ({
              ...
              respostas_comportamentais: padraoComp.map(c => {
                const existing = (fullForm.respostas || []).find((r: any) => r.tipo === 'comportamental' && r.competencia_nome === c.nome);
                return {
                  competencia_nome: c.nome,
                  competencia_descricao: c.descricao,
              ...
              respostas_estrategicas: padraoEstr.map(c => {
                const existing = (fullForm.respostas || []).find((r: any) => r.tipo === 'estrategica' && r.competencia_nome === c.nome);
                return {
                  competencia_nome: c.nome,
                  competencia_descricao: c.descricao,
                  nota: existing ? String(existing.nota || '') : '',
              ...
              respostas_gerenciais: padraoGer.map(c => {
                const existing = (fullForm.respostas || []).find((r: any) => r.tipo === 'gerencial' && r.competencia_nome === c.nome);
                return {
                  competencia_nome: c.nome,
                  competencia_descricao: c.descricao,
                  nota: existing ? String(existing.nota || '') : '',
            ...
            const hasStoredKeys = editMode && storedUpdateKeys && (Array.isArray(storedUpdateKeys.tecnicas) || Array.isArray(storedUpdateKeys.padrao));

            if (fullForm.unidade_id) {
              loadCompetenciasEdit(fullForm.unidade_id, fullForm.respostas || [], hasStoredKeys);
            }
            ...
            if (hasStoredKeys) {
              if (Array.isArray(storedUpdateKeys.tecnicas)) setChangedTecnicasKeys(new Set(storedUpdateKeys.tecnicas));
              if (Array.isArray(storedUpdateKeys.padrao)) setChangedCompKeys(new Set(storedUpdateKeys.padrao));
            } else {
              ...
              const checkPadrao = (tipo: string, lista: { nome: string; descricao: string }[]) => {
                const used = new Set<number>();
                lista.forEach(c => {
                  const exactIdx = respostas.findIndex((r: any, i: number) =>
                    !used.has(i) && r.tipo === tipo &&
                    r.competencia_nome === c.nome &&
                    (r.competencia_descricao || '') === (c.descricao || '')
                  );
                  if (exactIdx !== -1) {
                  ...
                  const nameIdx = respostas.findIndex((r: any, i: number) =>
                    !used.has(i) && r.tipo === tipo && r.competencia_nome === c.nome
                  );
  ...
  const hasUpdateLock = forceEditMode || (editMode && (changedCompKeys.size > 0 || changedTecnicasKeys.size > 0));
  ...
  // Handlers
  const updateField = (field: Exclude<keyof FormState, 'respostas' | 'unidade_path'>, value: string) => {
    setForm(prev => ({ ...prev, [field]: value }));
  };

  const updateResposta = (index: number, field: 'nota' | 'comentario', value: string) => {
    setForm(prev => {
  ...
  const updateRespostaComportamental = (index: number, field: 'nota' | 'comentario', value: string) => {
    setForm(prev => {
      const respostas_comportamentais = [...prev.respostas_comportamentais];
      respostas_comportamentais[index] = { ...respostas_comportamentais[index], [field]: value };
      return { ...prev, respostas_comportamentais };
    });
  };

  const updateRespostaEstrategica = (index: number, field: 'nota' | 'comentario', value: string) => {
    setForm(prev => {
      const respostas_estrategicas = [...prev.respostas_estrategicas];
      respostas_estrategicas[index] = { ...respostas_estrategicas[index], [field]: value };
      return { ...prev, respostas_estrategicas };
    });
  };

  const updateRespostaGerencial = (index: number, field: 'nota' | 'comentario', value: string) => {
    setForm(prev => {
      const respostas_gerenciais = [...prev.respostas_gerenciais];
      respostas_gerenciais[index] = { ...respostas_gerenciais[index], [field]: value };
  ...
  const handleSubmit = async () => {
    ...
    if (!form.cargo_funcao.trim()) return toast.error('Informe o cargo/função.');
    if (!form.email_institucional.trim()) return toast.error('Informe o e-mail institucional.');
    if (!form.unidade_id) return toast.error('Selecione a unidade.');
    if (!form.nome_completo.trim()) return toast.error('Digite seu nome completo.');

    if (form.respostas.length === 0) {
      return toast.error('Nenhuma competência encontrada para a unidade selecionada.');
    }

    for (let i = 0; i < form.respostas.length; i++) {
      const r = form.respostas[i];
      if (!r.nota) {
        return toast.error(`Selecione a nota para a competência técnica "${r.competencia_nome}".`);
      }
    }
    ...
    for (let i = 0; i < form.respostas_comportamentais.length; i++) {
      ...
      if (!r.nota) {
        return toast.error(`Selecione a nota para a competência comportamental "${r.competencia_nome}".`);
      }
    }
    ...
    if (tipoInventario === 'gestor') {
      for (let i = 0; i < form.respostas_estrategicas.length; i++) {
        ...
        if (!r.nota) {
          return toast.error(`Selecione a nota para a competência estratégica "${r.competencia_nome}".`);
        }
      }
      for (let i = 0; i < form.respostas_gerenciais.length; i++) {
        const r = form.respostas_gerenciais[i];
        if (!r.nota) {
          return toast.error(`Selecione a nota para a competência gerencial "${r.competencia_nome}".`);
        }
      }
    }
    ...
    try {
      ...
      const respostasComportamentais = form.respostas_comportamentais.map(r => ({
        competencia_nome: r.competencia_nome,
        competencia_descricao: r.competencia_descricao,
        nota: Number(r.nota),
        comentario: r.comentario.trim() || undefined,
      ...
      const respostasEstrategicas = tipoInventario === 'gestor' ? form.respostas_estrategicas.map(r => ({
        competencia_nome: r.competencia_nome,
        competencia_descricao: r.competencia_descricao,
        nota: Number(r.nota),
        comentario: r.comentario.trim() || undefined,
      ...
      const respostasGerenciais = tipoInventario === 'gestor' ? form.respostas_gerenciais.map(r => ({
        competencia_nome: r.competencia_nome,
        competencia_descricao: r.competencia_descricao,
        nota: Number(r.nota),
        comentario: r.comentario.trim() || undefined,
      ...
      const updateKeys = (forceEditMode || changedCompKeys.size > 0 || changedTecnicasKeys.size > 0)
        ? { tecnicas: Array.from(changedTecnicasKeys), padrao: Array.from(changedCompKeys) }
        : null;
      ...
      const payload: any = {
        ...
        respostas: [...respostasTecnicas, ...respostasComportamentais, ...respostasEstrategicas, ...respostasGerenciais],
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="border-2 border-emerald-200 bg-emerald-50">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <h3 className="text-xl font-bold text-emerald-900">Autoavaliação já enviada</h3>
            <p className="text-sm text-emerald-700 max-w-md mx-auto">
              Você já enviou sua autoavaliação. Para preencher novamente, é necessário que o gestor exclua o formulário enviado.
            </p>
            <p className="text-xs text-emerald-600">
              Enviado em: {new Date(jaPreenchido.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </p>
            {onViewResposta && (
              <Button
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="border-2 border-purple-200 bg-purple-50">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <h3 className="text-xl font-bold text-purple-900">Atualização Requisitada</h3>
            <p className="text-sm text-purple-700 max-w-md mx-auto">
              ...
              para as novas competências adicionadas ou modificadas.
            </p>
            ...
            <div className="flex gap-3 justify-center mt-2">
              ...
              <Button
                onClick={() => { setSavedVersaoAnterior((jaPreenchido as any)?.competencias_versao || null); setJaPreenchido(null); setForceEditMode(true); setLoadingUnidades(true); }}
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex gap-4">
          ...
          <div className="text-base text-gray-700 space-y-4">
            <p className="font-semibold text-gray-900 text-lg">
              {tipoInventario === 'gestor' ? 'Autoavaliação do Gestor' : 'Autoavaliação do Colaborador'}
            </p>
            <p>
            ...
            <p className="font-semibold text-gray-900">Orientações importantes:</p>
            <ul className="list-disc ml-6 space-y-2">
              <li>Responda com tranquilidade e sinceridade, lembrando que não há respostas certas ou erradas.</li>
              {tipoInventario === 'gestor' ? (
                <li>Reflita sobre sua atuação como gestor, destacando seus pontos fortes e reconhecendo oportunidades de melhoria e desenvolvimento.</li>
              ) : (
                <li>Destaque seus pontos fortes e reconheça as oportunidades de melhoria e desenvolvimento.</li>
              )}
              <li>
                {tipoInventario === 'gestor'
      ...
      <Card className="border border-gray-200 shadow-sm">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label>E-mail institucional <span className="text-red-500">*</span></Label>
              <Input
                type="email"
            ...
            {loadingUnidades ? (
              ...
              <div className="bg-gray-50 rounded-lg px-4 py-3 border border-gray-200">
                <p className="font-medium text-gray-800">
                  {unidadesAutorizadas.find(u => String(u.id) === form.unidade_id)?.nome || '-'}
                </p>
              </div>
              ...
              <>
                <Select
                  ...
                  disabled={tipoInventario === 'gestor' && unidadesAutorizadas.length === 1}
                >
      ...
      {form.unidade_id && (
        <Card className="border border-gray-200 shadow-sm">
          ...
          <CardContent className="space-y-3">
            <Label>Digite seu nome completo <span className="text-red-500">*</span></Label>
            <Input
              placeholder="Ex: João da Silva"
              value={form.nome_completo}
              onChange={e => setForm(prev => ({ ...prev, nome_completo: e.target.value, pessoa_id: 'typed' }))}
            />
          </CardContent>
        </Card>
      )}
          ...
          <div className="rounded-xl bg-amber-50 border border-amber-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>Nesta seção, você encontrará competências técnicas relacionadas às atividades da sua função.</p>
                <p className="font-semibold text-gray-900">Para cada competência:</p>
                <ul className="list-disc ml-6 space-y-1">
                  <li>Faça sua autoavaliação em uma escala de 1 a 5.</li>
                  <li>Registre comentários ou exemplos que evidenciem sua atuação.</li>
                </ul>
                <p className="font-semibold text-gray-900">Escala Técnica (1-5):</p>
          ...
          {loadingCompetencias ? (
            ...
            <Card className="border border-amber-200 bg-amber-50 shadow-sm">
              <CardContent className="p-6">
                <div className="flex items-start gap-3">
                  <AlertCircle className="h-5 w-5 text-amber-600 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-amber-800">Nenhuma competência cadastrada</p>
                    <p className="text-sm text-amber-700 mt-1">
                      Ainda não foram cadastradas competências técnicas para a unidade selecionada.
                      O gestor da unidade precisa cadastrar as competências no Matriz de Competências antes que a autoavaliação possa ser preenchida.
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <>
              {form.respostas.map((resposta, index) => {
                const tecLocked = hasUpdateLock && !!resposta.nota && !changedTecnicasKeys.has(`${resposta.competencia_nome}||${resposta.competencia_descricao || ''}`);
                return (
                <Card key={index} className={`border shadow-sm ${tecLocked ? 'border-gray-100 bg-gray-50 opacity-70' : 'border-gray-200'}`}>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      {tecLocked && <span className="text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded">Sem alteração</span>}
                    </div>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <Label>Nível de domínio <span className="text-red-500">*</span></Label>
                      <EscalaRadioGroup
                        items={ESCALA_NOTAS}
                        name={`nota-${index}`}
                        value={resposta.nota}
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={e => updateResposta(index, 'comentario', e.target.value)}
      ...
      {form.respostas.length > 0 && (
        <>
          <div className="rounded-xl bg-violet-50 border border-violet-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>Nesta seção, você encontrará competências relacionadas a atitudes e comportamentos esperados no ambiente de trabalho.</p>
                <p className="font-semibold text-gray-900">Para cada competência:</p>
                <ul className="list-disc ml-6 space-y-1">
                  <li>Faça sua autoavaliação em uma escala de 1 a 5.</li>
                  <li>Registre comentários ou exemplos que evidenciem sua atuação.</li>
                </ul>
                <p className="font-semibold text-gray-900">Escala Comportamental (1-5):</p>
          ...
          {form.respostas_comportamentais.map((resposta, index) => {
            const locked = isCompLocked(resposta.competencia_nome, resposta.competencia_descricao, resposta.nota);
            return (
            <Card key={`comp-${index}`} className={`border shadow-sm ${locked ? 'border-gray-100 bg-gray-50 opacity-70' : 'border-gray-200'}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">
                  ...
                  {locked && <span className="text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded">Sem alteração</span>}
                </div>
                {resposta.competencia_descricao && (
                  <p className="text-sm text-gray-500 mt-1 [overflow-wrap:anywhere]">{resposta.competencia_descricao}</p>
                )}
              ...
              <CardContent className="space-y-4">
                <div>
                  <Label>Nível de desenvolvimento <span className="text-red-500">*</span></Label>
                  <EscalaRadioGroup
                    items={ESCALA_COMPORTAMENTAL}
                    name={`nota-comp-${index}`}
                    value={resposta.nota}
                    onChange={v => updateRespostaComportamental(index, 'nota', v)}
                    disabled={locked}
                    accentColor="violet"
                  />
                </div>
                <div>
                  ...
                  <Textarea
                    value={resposta.comentario}
                    onChange={e => updateRespostaComportamental(index, 'comentario', e.target.value)}
                    placeholder="Descreva evidências ou justificativas para a nota atribuída (opcional)"
                    className="mt-1"
                    rows={3}
                    disabled={locked}
      ...
      {tipoInventario === 'gestor' && form.respostas.length > 0 && form.respostas_estrategicas.length > 0 && (
        <>
          <div className="rounded-xl bg-blue-50 border border-blue-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>Nesta seção, você encontrará competências relacionadas à compreensão do contexto institucional e ao alinhamento das atividades com os objetivos estratégicos da organização.</p>
                <p className="font-semibold text-gray-900">Para cada competência:</p>
                <ul className="list-disc ml-6 space-y-1">
                  <li>Faça sua autoavaliação em uma escala de 1 a 5.</li>
                  <li>Registre comentários ou exemplos que evidenciem sua atuação.</li>
                </ul>
                <p className="font-semibold text-gray-900">Escala Estratégica (1-5):</p>
          ...
          {form.respostas_estrategicas.map((resposta, index) => {
            const locked = isCompLocked(resposta.competencia_nome, resposta.competencia_descricao, resposta.nota);
            return (
            <Card key={`estr-${index}`} className={`border shadow-sm ${locked ? 'border-gray-100 bg-gray-50 opacity-70' : 'border-gray-200'}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <EscalaRadioGroup
                    ...
                    value={resposta.nota}
                    onChange={v => updateRespostaEstrategica(index, 'nota', v)}
                    disabled={locked}
                    accentColor="blue"
                  />
                </div>
                <div>
                  ...
                  <Textarea
                    value={resposta.comentario}
                    onChange={e => updateRespostaEstrategica(index, 'comentario', e.target.value)}
                    placeholder="Descreva evidências ou justificativas para a nota atribuída (opcional)"
                    className="mt-1"
                    rows={3}
                    disabled={locked}
      ...
      {tipoInventario === 'gestor' && form.respostas.length > 0 && form.respostas_gerenciais.length > 0 && (
        <>
          <div className="rounded-xl bg-rose-50 border border-rose-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>Nesta seção, você encontrará competências relacionadas à liderança de equipes e à condução das atividades de gestão da unidade.</p>
                <p className="font-semibold text-gray-900">Para cada competência:</p>
                <ul className="list-disc ml-6 space-y-1">
                  <li>Faça sua autoavaliação em uma escala de 1 a 5.</li>
                  <li>Registre comentários ou exemplos que evidenciem sua atuação.</li>
                </ul>
                <p className="font-semibold text-gray-900">Escala Gerencial (1-5):</p>
          ...
          {form.respostas_gerenciais.map((resposta, index) => {
            const locked = isCompLocked(resposta.competencia_nome, resposta.competencia_descricao, resposta.nota);
            return (
            <Card key={`ger-${index}`} className={`border shadow-sm ${locked ? 'border-gray-100 bg-gray-50 opacity-70' : 'border-gray-200'}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <EscalaRadioGroup
                    ...
                    name={`nota-ger-${index}`}
                    value={resposta.nota}
                    onChange={v => updateRespostaGerencial(index, 'nota', v)}
                    disabled={locked}
                    accentColor="rose"
                  />
                </div>
                <div>
                  ...
                  <Textarea
                    value={resposta.comentario}
                    onChange={e => updateRespostaGerencial(index, 'comentario', e.target.value)}
                    placeholder="Descreva evidências ou justificativas para a nota atribuída (opcional)"
                    className="mt-1"
                    rows={3}
                    disabled={locked}
