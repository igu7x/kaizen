...
export function CompetenciasEquipeForm({ onSubmitted, editFormulario, validationMode }: CompetenciasEquipeFormProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      qtd_colaboradores: editFormulario.qtd_colaboradores ? String(editFormulario.qtd_colaboradores) : '',
  ...
  const handleEdit = async (formularioId: number, unidadeNome: string) => {
    ...
    try {
      ...
      setForm({
        ...
        qtd_colaboradores: formulario.qtd_colaboradores ? String(formulario.qtd_colaboradores) : '',
  ...
  const handleSubmit = async () => {
    ...
    if (!form.qtd_colaboradores.trim()) return toast.error('Informe a quantidade de colaboradores.');
    ...
    for (let i = 0; i < form.competencias.length; i++) {
      ...
      if (!c.aplicabilidade) return toast.error(`Selecione a aplicabilidade da competência nº ${i + 1}.`);
      if (c.aplicabilidade === 'parte') {
        if (!c.quantidade_pessoas.trim()) {
          return toast.error(`Informe a quantidade de pessoas para a competência nº ${i + 1}.`);
        }
        const qtdPessoas = Number(c.quantidade_pessoas);
        if (qtdPessoas < 1) {
          return toast.error(`A quantidade de pessoas da competência nº ${i + 1} deve ser no mínimo 1.`);
        }
        const totalColab = Number(form.qtd_colaboradores);
        if (totalColab > 0 && qtdPessoas > totalColab) {
          return toast.error(`A quantidade de pessoas da competência nº ${i + 1} não pode exceder o total de colaboradores (${totalColab}).`);
        }
      }
    }
    ...
    try {
      const payload = {
        ...
        competencias: form.competencias.map(c => ({
          ...
          peso: Number(c.peso),
          aplicabilidade: c.aplicabilidade as 'todos' | 'parte',
          quantidade_pessoas: c.aplicabilidade === 'parte' && c.quantidade_pessoas ? Number(c.quantidade_pessoas) : undefined,
        })),
      };
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-xl bg-blue-50 border border-blue-200 p-8">
        <div className="flex gap-4">
          ...
          <div className="text-base text-gray-700 space-y-4">
            <p>
              <strong className="text-gray-900">Objetivo:</strong> Este formulário tem como finalidade identificar as competências essenciais para o bom
              desempenho da equipe sob sua gestão.
            </p>
            <p>
              Registre as competências que considera indispensáveis para todos os colaboradores da sua equipe ou para
              grupos específicos.
            </p>
            <p>Cada competência deve ser classificada de acordo com:</p>
            <ul className="list-disc ml-6 space-y-2">
              <li><strong className="text-gray-900">Importância (peso):</strong> grau de relevância para o desempenho das atividades.</li>
              <li><strong className="text-gray-900">Aplicabilidade:</strong> número de colaboradores que precisam possuir essa competência.</li>
            </ul>
      ...
      <Card className="border border-gray-200 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label>Quantidade de colaboradores na unidade (sem considerar o gestor) <span className="text-red-500">*</span></Label>
            <Input
              type="number"
              min="0"
      ...
      <div className="rounded-xl bg-amber-50 border border-amber-200 p-8">
        <div className="flex gap-4">
          ...
          <div className="text-base text-gray-700 space-y-4">
            ...
            <p>
              Indique entre <strong className="text-gray-900">8 e 12 competências técnicas essenciais</strong> para a sua área. Essas competências devem refletir os
              conhecimentos e habilidades fundamentais para o bom desempenho da equipe.
            </p>
            ...
            <ul className="list-disc ml-6 space-y-2">
              ...
              <li><strong className="text-gray-900">Grau de Impacto:</strong> se é útil, importante ou crítica;</li>
              <li><strong className="text-gray-900">Aplicabilidade:</strong> se deve ser exigida de todos os colaboradores ou apenas de parte da equipe;</li>
              <li><strong className="text-gray-900">Quantidade:</strong> número de colaboradores que precisam possuir essa competência (quando aplicável).</li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {[
                  { value: '1', label: '1 — Útil: agrega, mas não compromete o serviço se ausente' },
                  { value: '2', label: '2 — Importante: ausência gera retrabalho/atrasos' },
                  { value: '3', label: '3 — Crítica: risco operacional/segurança/conformidade' },
                ].map(opt => (
            ...
            <div>
              <Label>Aplicabilidade <span className="text-red-500">*</span></Label>
              <div className="mt-2 space-y-2">
                {[
                  ...
                  <label key={opt.value} className="flex items-start gap-3 cursor-pointer group">
                    <input
                      ...
                      onChange={() => updateCompetencia(index, 'aplicabilidade', opt.value)}
                      className="mt-0.5 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                    />
                    <span className="text-sm text-gray-700 group-hover:text-gray-900">{opt.label}</span>
                  </label>
            ...
            {comp.aplicabilidade === 'parte' && (
              <div className="ml-7 border-l-2 border-blue-200 pl-4">
                <Label>Quantidade de pessoas necessárias para esta competência <span className="text-red-500">*</span></Label>
                <Input
                  type="number"
                  min="1"
                  max={form.qtd_colaboradores ? Number(form.qtd_colaboradores) : undefined}
                  value={comp.quantidade_pessoas}
                  onChange={e => updateCompetencia(index, 'quantidade_pessoas', e.target.value)}
                  placeholder="Informe a quantidade"
                  className="mt-1 max-w-xs"
                />
                {form.qtd_colaboradores && (
                  <p className="text-xs text-gray-400 mt-1">Máximo: {form.qtd_colaboradores} colaboradores</p>
                )}
              </div>
            )}
          </CardContent>
