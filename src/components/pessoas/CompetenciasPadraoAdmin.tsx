  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          ...
          <div>
            ...
            <p className="text-sm text-amber-600 mt-1">
              Você fez alterações que ainda não foram publicadas. Clique em
              "Publicar Alterações" para aplicá-las. Formulários já enviados que
              contenham competências modificadas serão marcados para
              atualização.
            </p>
          </div>
        </div>
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs text-gray-500">
                            {items[0].publicado_por_nome || "Sistema"} —{" "}
                        ...
                        {items.map((item) => {
                          ...
                          if (
                            !m ||
                            ((!m.adicionadas || m.adicionadas.length === 0) &&
                              (!m.removidas || m.removidas.length === 0) &&
                              (!m.alteradas || m.alteradas.length === 0))
                          )
                            return null;
                          return (
                            <div
                              key={`${versao}-${item.tipo}`}
                              className="text-sm ml-3 mb-1"
                            >
                              <span
                                className={`font-medium ${TIPO_ACCENT[item.tipo] || "text-gray-700"}`}
                              >
                                {TIPO_LABELS[item.tipo] || item.tipo}:
                              </span>
                              {m.adicionadas?.length > 0 && (
                                <span className="text-emerald-600 ml-2">
                                  +{m.adicionadas.length} adicionada(s)
                                </span>
                              )}
                              {m.removidas?.length > 0 && (
                                <span className="text-red-600 ml-2">
                                  -{m.removidas.length} removida(s)
                                </span>
                              )}
                              {m.alteradas?.length > 0 && (
                                <span className="text-amber-600 ml-2">
                                  ~{m.alteradas.length} alterada(s)
                                </span>
                              )}
                            </div>
                          );
      ...
      <div className="flex gap-2 border-b border-gray-200 pb-2">
        {(["comportamental", "estrategica", "gerencial"] as const).map(
          (tipo) => (
            <button
              key={tipo}
            ...
            >
              {TIPO_LABELS[tipo]} (
              {competencias.filter((c) => c.tipo === tipo && c.ativo).length})
            </button>
          ),
        )}
      </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <p className="font-medium text-gray-900">
                        <span
                          className={`${TIPO_ACCENT[comp.tipo]} font-bold mr-2`}
                        >
                          {index + 1}.
                        </span>
                        {comp.nome}
