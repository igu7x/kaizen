...
const Organograma: React.FC<OrganogramaProps> = ({
  ...
  const [escalasPorLinha, setEscalasPorLinha] = useState<
    ...
    const calcularEscalas = () => {
      ...
      Object.keys(linhasOrganizadas).forEach((linhaStr) => {
        ...
        const larguraNecessaria =
          qtdCards * larguraCardBase + (qtdCards - 1) * gap;
  ...
  const fetchOrganograma = async () => {
    try {
      setLoading(true);
      const response = await colaboradoresApi.getOrganograma(
        diretoria !== "Todas" ? diretoria : undefined,
      );
  ...
  const organizarPorLinhas = useCallback(() => {
    ...
    Object.keys(linhas).forEach((linha) => {
      linhas[parseInt(linha)].sort(
        (a, b) => (a.ordem_exibicao || 0) - (b.ordem_exibicao || 0),
      );
    });
  ...
  const calcularLinhas = useCallback(() => {
    ...
    dados.forEach((gestor) => {
      ...
      const elementoPai = document.getElementById(
        `card-gestor-${gestor.subordinacao_id}`,
      );
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      marginBottom:
                        escalaLinha < 1 ? `${(1 - escalaLinha) * -50}px` : "0",
                    }}
                  >
                              ...
                              {(provided, snapshot) => (
                                ...
                                >
                                  <CardGestor
                                    ...
                                    onEdit={
                                      canEdit
                                        ? () => handleEditarGestor(gestor)
                                        : undefined
                                    }
                                    onDelete={
                                      canEdit
                                        ? () => handleExcluirGestor(gestor.id)
                                        : undefined
                                    }
