...
import { avaliacaoGestorApi, AvaliacaoGestorFormulario } from '@/services/avaliacaoGestorApi';
...
export function AvaliacaoGestorForm({ onSubmitted, tipoInventario, formularioEdit }: AvaliacaoGestorFormProps) {
  ...
  const [autoavaliacoes, setAutoavaliacoes] = useState<{ id: number; nome_completo: string; cargo_funcao: string; email_institucional: string; unidade_id: number }[]>([]);
  ...
  const loadPessoas = async (unidadeId: number) => {
    ...
    try {
      const avals = await autoavaliacaoApi.getByUnidade(unidadeId, tipoInventario || 'equipe');
      setAutoavaliacoes(avals);
    } catch (err) {
      console.error('Erro ao carregar autoavaliações:', err);
    } finally {
  ...
  const [avaliacaoExistente, setAvaliacaoExistente] = useState<AvaliacaoGestorFormulario | null>(null);
  ...
  const handlePessoaSelect = async (avalId: string) => {
    ...
    try {
      const existente = await avaliacaoGestorApi.getByPessoaAndUnidade(Number(aval.id), aval.unidade_id);
      setAvaliacaoExistente(existente);
      if (existente && existente.status === 'atualizacao_requisitada' && existente.respostas) {
        const tecnicas = (existente.respostas || []).filter((r: any) => (r.tipo || 'tecnica') === 'tecnica');
        ...
        const buildPadrao = (tipo: 'comportamental' | 'estrategica' | 'gerencial', lista: { nome: string; descricao: string }[]) => {
          const used = new Set<number>();
          return lista.map(c => {
            const exactIdx = respostasV1.findIndex((r: any, i: number) =>
            ...
            const nameIdx = respostasV1.findIndex((r: any, i: number) =>
        ...
        if (storedUpdateKeys && Array.isArray(storedUpdateKeys.padrao)) {
          ...
          const checkPadrao = (tipo: string, lista: { nome: string; descricao: string }[]) => {
            ...
            lista.forEach(c => {
              const exactIdx = respostasV1.findIndex((r: any, i: number) =>
                !usedDet.has(i) && r.tipo === tipo &&
                r.competencia_nome === c.nome &&
                (r.competencia_descricao || '') === (c.descricao || '')
              );
              if (exactIdx !== -1) { usedDet.add(exactIdx); return; }
              const nameIdx = respostasV1.findIndex((r: any, i: number) =>
                !usedDet.has(i) && r.tipo === tipo && r.competencia_nome === c.nome
              );
      ...
      console.error('Erro ao detectar avaliação existente:', err);
    }
  };
  ...
  const loadCompetenciasEdit = async (unidadeId: number, respostasTecnicasExistentes: any[]) => {
    setLoadingCompetencias(true);
    try {
      const competencias = tipoInventario === 'gestor'
        ? await competenciasGestorApi.getCompetenciasGestorPorUnidade(unidadeId)
      ...
      const novasRespostas = competencias.map(c => {
        ...
        const exactIdx = respostasTecnicasExistentes.findIndex((r: any, i: number) =>
          !usedIdx.has(i) &&
          r.competencia_nome === c.nome &&
          (r.competencia_descricao || '') === (c.descricao || '')
        );
        ...
        const nameIdx = respostasTecnicasExistentes.findIndex((r: any, i: number) =>
        ...
        const posIdx = respostasTecnicasExistentes.findIndex((r: any, i: number) => !usedIdx.has(i));
        if (posIdx !== -1) {
          usedIdx.add(posIdx);
    ...
    const load = async () => {
      try {
        ...
        if (isEditMode && formularioEdit) {
          const tecnicas = (formularioEdit.respostas || []).filter(r => (r.tipo || 'tecnica') === 'tecnica');
          ...
          setForm(prev => ({
            ...
            respostas_comportamentais: buildPadrao('comportamental', padraoComp),
  ...
  const handleSubmit = async () => {
    ...
    if (!form.pessoa_id) return toast.error('Selecione o colaborador a ser avaliado.');
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-xl bg-teal-50 border border-teal-200 p-8">
        <div className="flex gap-4">
          ...
          <div className="text-base text-gray-700 space-y-4">
            <p>
              <strong className="text-gray-900">Objetivo:</strong> Este formulário tem como finalidade possibilitar que o gestor
              avalie as competências técnicas e comportamentais de cada colaborador de sua equipe, identificando o nível de domínio
              em cada uma delas.
            </p>
            <p>
              As competências técnicas listadas foram previamente definidas na Matriz de Competências da unidade.
            </p>
            ...
            <ul className="list-disc ml-6 space-y-2">
              ...
              <li>Para cada competência, selecione o nível que melhor representa o domínio atual do colaborador.</li>
              <li>Utilize o campo de comentários/evidências para justificar sua avaliação, quando aplicável.</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-lg px-4 py-3 border border-gray-200">
              ...
              <p className="font-medium text-gray-800 mt-0.5">{user?.name || '-'}</p>
            </div>
            <div className="bg-gray-50 rounded-lg px-4 py-3 border border-gray-200">
      ...
      {form.unidade_id && (
        <Card className="border border-gray-200 shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">Colaborador a ser Avaliado</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <Label>Selecione o colaborador a ser avaliado {!isEditMode && <span className="text-red-500">*</span>}</Label>
            {isEditMode ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="bg-gray-50 rounded-lg px-4 py-3 border border-gray-200">
                  <span className="text-sm text-gray-500">Nome</span>
                  <p className="font-medium text-gray-800 mt-0.5">{form.pessoa_nome}</p>
                </div>
                <div className="bg-gray-50 rounded-lg px-4 py-3 border border-gray-200">
                  <span className="text-sm text-gray-500">Cargo/Função</span>
                  <p className="font-medium text-gray-800 mt-0.5">{form.pessoa_cargo || '-'}</p>
                </div>
              </div>
            ) : loadingPessoas ? (
              ...
              <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <AlertCircle className="h-5 w-5 text-amber-600 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-amber-700">
                  Nenhum colaborador realizou a autoavaliação nesta unidade ainda.
                </p>
              </div>
            ) : (
              <>
                  ...
                  <SelectContent className="max-h-[300px]">
                    {autoavaliacoes.map(a => (
                      <SelectItem key={a.id} value={String(a.id)}>
                        {a.nome_completo}{a.cargo_funcao ? ` — ${a.cargo_funcao}` : ''}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <div className="rounded-xl bg-amber-50 border border-amber-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>
                  Avalie o nível de domínio do colaborador em cada competência técnica listada abaixo.
                </p>
          ...
          {loadingCompetencias ? (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700 mt-1">
                      ...
                      O gestor da unidade precisa cadastrar as competências na Matriz de Competências antes que a avaliação possa ser preenchida.
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <>
              {form.respostas.map((resposta, index) => {
                const isChanged = isTecChanged(resposta.competencia_nome, resposta.competencia_descricao);
                ...
                const isInEditMode = isEditMode || avaliacaoExistente?.status === 'atualizacao_requisitada';
                // Só competências alteradas/novas são editáveis. Demais travadas (v1 preservada).
                const isLocked = isInEditMode && !isChanged;
                return (
                <Card key={index} className={`border shadow-sm ${isLocked ? 'border-gray-200 bg-gray-50' : isInEditMode && isChanged ? 'border-amber-300 bg-amber-50/30' : 'border-gray-200'}`}>
                  <CardHeader className="pb-3">
      ...
      {form.respostas.length > 0 && (
        <>
          <div className="rounded-xl bg-violet-50 border border-violet-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>
                  Avalie o nível de desenvolvimento do colaborador em cada competência comportamental listada abaixo.
                </p>
                <p>Utilize a seguinte escala:</p>
          ...
          {form.respostas_comportamentais.map((resposta, index) => {
            const isInEditMode = isEditMode || avaliacaoExistente?.status === 'atualizacao_requisitada';
            const isChanged = isCompChanged(resposta.competencia_nome, resposta.competencia_descricao);
            const isLocked = isInEditMode && !isChanged;
            return (
            <Card key={`comp-${index}`} className={`border shadow-sm ${isLocked ? 'border-gray-200 bg-gray-50' : isInEditMode && isChanged ? 'border-amber-300 bg-amber-50/30' : 'border-gray-200'}`}>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg flex items-center gap-2">
      ...
      {tipoInventario === 'gestor' && form.respostas.length > 0 && form.respostas_estrategicas.length > 0 && (
        <>
          <div className="rounded-xl bg-blue-50 border border-blue-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>
                  Avalie o nível de desenvolvimento do gestor em cada competência estratégica listada abaixo.
                </p>
                <p>Utilize a seguinte escala:</p>
          ...
          {form.respostas_estrategicas.map((resposta, index) => {
            ...
            const isLocked = isInEditMode && !isChanged;
            return (
            <Card key={`estr-${index}`} className={`border shadow-sm ${isLocked ? 'border-gray-200 bg-gray-50' : isInEditMode && isChanged ? 'border-amber-300 bg-amber-50/30' : 'border-gray-200'}`}>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg flex items-center gap-2">
      ...
      {tipoInventario === 'gestor' && form.respostas.length > 0 && form.respostas_gerenciais.length > 0 && (
        <>
          <div className="rounded-xl bg-rose-50 border border-rose-200 p-8">
            <div className="flex gap-4">
              ...
              <div className="text-base text-gray-700 space-y-4">
                ...
                <p>
                  Avalie o nível de desenvolvimento do gestor em cada competência gerencial listada abaixo.
                </p>
                <p>Utilize a seguinte escala:</p>
          ...
          {form.respostas_gerenciais.map((resposta, index) => {
            ...
            const isLocked = isInEditMode && !isChanged;
            return (
            <Card key={`ger-${index}`} className={`border shadow-sm ${isLocked ? 'border-gray-200 bg-gray-50' : isInEditMode && isChanged ? 'border-amber-300 bg-amber-50/30' : 'border-gray-200'}`}>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg flex items-center gap-2">
