                ...
                <div className="space-y-2">
                  ...
                  <div className="pl-6 space-y-2 border-l-2 border-gray-200">
                    {areas.map((area) => {
                      ...
                      return (
                        <label key={area.id} className="flex items-center space-x-2 cursor-pointer p-2 hover:bg-gray-50 rounded">
                          <input
                            type="checkbox"
                            checked={allowedDirectorates.includes(sigla) && !allowedDirectorates.includes('ALL')}
                            disabled={allowedDirectorates.includes('ALL')}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setAllowedDirectorates([...allowedDirectorates.filter(d => d !== 'ALL'), sigla]);
                              } else {
                                setAllowedDirectorates(allowedDirectorates.filter(d => d !== sigla));
                              }
                            }}
                            className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 disabled:opacity-50"
                          />
                          <span className={allowedDirectorates.includes('ALL') ? 'text-gray-400' : ''}>{sigla}</span>
                        </label>
                      );
                    })}
                  </div>
