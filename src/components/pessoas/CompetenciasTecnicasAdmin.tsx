...
export function CompetenciasTecnicasAdmin() {
  ...
  const selectedUnidade = unidades.find(
    (u) => u.formulario_id === selectedFormId,
  );
  ...
  const handleSave = async () => {
    ...
    if (
      !isGestor &&
      editing.aplicabilidade === "parte" &&
      !editing.quantidade_pessoas
    ) {
      ...
      const data = {
        ...
        quantidade_pessoas:
          !isGestor && editing.aplicabilidade === "parte"
            ? Number(editing.quantidade_pessoas)
            : undefined,
      };
      if (editing.id) {
  ...
  const handlePublish = async () => {
    ...
    if (
      !confirm(
        "Publicar alterações? A matriz voltará para o fluxo de validação (enviado → validado autor → validado diretoria → validado final). Os formulários do inventário só serão marcados para atualização após a validação final.",
      )
    ) {
      return;
    }
    ...
    try {
      const result =
        await competenciasGestorApi.publicarTecnicas(selectedFormId);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <p className="text-sm text-gray-500">
        Selecione uma unidade para gerenciar suas competências técnicas. Você só
        vê unidades onde você é responsável e que têm referencial preenchido.
      </p>
          ...
          {unidades.length === 0 ? (
            <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-sm text-amber-700">
                Nenhuma unidade com referencial preenchido encontrada para seu
                usuário.
              </p>
            </div>
          ) : (
            <Select
              ...
              onValueChange={(v) => setSelectedFormId(Number(v))}
            >
              <SelectTrigger>
              ...
              <SelectContent className="max-h-[400px]">
                {unidades.map((u) => (
                  <SelectItem
                    key={u.formulario_id}
                    value={String(u.formulario_id)}
                  >
                    {u.unidade_nome} —{" "}
                    {u.tipo === "gestor" ? "Gestor" : "Equipe"} (v
                    {u.tecnicas_versao || 1})
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-amber-600 mt-1">
                  Clique em "Publicar Alterações" para aplicá-las. Formulários
                  já enviados desta unidade (autoavaliação, avaliação do gestor,
                  integrada) serão marcados para atualização.
                </p>
              </div>
            </div>
          )}
                      ...
                      <CardContent className="p-4">
                        {editing?.id === item.id ? (
                          <ItemForm
                            ...
                            onSave={handleSave}
                            saving={saving}
                            isGestor={selectedUnidade?.tipo === "gestor"}
                          />
                        ) : (
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span
                                  className={`text-xs px-2 py-0.5 rounded ${PESO_COLORS[item.peso] || ""}`}
                                >
                                  {PESO_LABELS[item.peso] ||
                                    `Peso ${item.peso}`}
                                </span>
                              </div>
                              <p className="text-sm text-gray-600 mt-1 [overflow-wrap:anywhere]">
                                {item.descricao}
                              </p>
                              {selectedUnidade?.tipo !== "gestor" && (
                                <p className="text-xs text-gray-500 mt-2">
                                  Aplicabilidade:{" "}
                                  {item.aplicabilidade === "todos"
                                    ? "Todos os colaboradores"
                                    : `Parte da equipe${item.quantidade_pessoas ? ` (${item.quantidade_pessoas} pessoas)` : ""}`}
                                </p>
                              )}
                            </div>
                                ...
                                onClick={() =>
                                  setEditing({
                                    ...
                                    quantidade_pessoas: item.quantidade_pessoas
                                      ? String(item.quantidade_pessoas)
                                      : "",
                                  })
                                }
                              >
...
function ItemForm({
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className={`grid grid-cols-1 ${isGestor ? "" : "md:grid-cols-2"} gap-3`}
      >
        <div>
          <Label>Peso</Label>
          <Select
            value={String(editing.peso)}
            onValueChange={(v) => setEditing({ ...editing, peso: Number(v) })}
          >
        ...
        {!isGestor && (
          <div>
            ...
            <Select
              value={editing.aplicabilidade}
              onValueChange={(v) =>
                setEditing({ ...editing, aplicabilidade: v })
              }
            >
              <SelectTrigger className="mt-1">
                <SelectValue />
      ...
      {!isGestor && editing.aplicabilidade === "parte" && (
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setEditing({ ...editing, quantidade_pessoas: e.target.value })
            }
          />
        </div>
      )}
