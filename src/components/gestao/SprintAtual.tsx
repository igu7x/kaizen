...
export function SprintAtual() {
  ...
  const [formBoardStatus, setFormBoardStatus] =
    useState<BoardStatus>("A_FAZER");
  ...
  const mapInitiativeToKanbanItem = (initiative: Initiative) => {
    ...
    const kr = filteredKeyResults.find(
      (k) => String(k.id) === String(initiative.keyResultId),
    );
    return {
  ...
  const handleEditInitiative = (id: string) => {
    const initiative = filteredInitiatives.find(
        ...
        <DialogContent className="max-w-lg">
          <form onSubmit={handleSaveInitiative}>
            ...
            <div className="space-y-4 py-4">
              <div>
                <Label htmlFor="keyResultId">
                  KR Associado <span className="text-red-500">*</span>
                </Label>
                <Select
                ...
                {filteredKeyResults.length === 0 && (
                  <p className="text-xs text-amber-600 mt-1">
                    Nenhum KR disponível. Crie um KR primeiro em "Monitoramento
                    de OKRs".
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setFormBoardStatus(val as BoardStatus)
                  }
                  disabled={saving}
                >
                  <SelectTrigger>
