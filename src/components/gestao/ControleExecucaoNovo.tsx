...
import { ArrowLeft, Pencil, Plus, Trash2, Eye, Edit, FolderOpen, FileText, Calendar, User, ChevronRight, Building2, FileCheck, FolderKanban, Target, CheckCircle2, AlertTriangle, Package, Search, Check, Clock, Star, CircleDot, Layers, ChevronsUpDown, Upload, FileDown, X, Loader2, Hourglass, RefreshCw } from 'lucide-react';
...
export function ControleExecucaoNovo() {
  const { user } = useAuth();
  ...
  const [tapDialogProjeto, setTapDialogProjeto] = useState<ProjetoCadastro | null>(null);
  ...
  const getTapStatusLocal = (projeto: ProjetoCadastro) => {
    ...
    const hasInstrumentos = (projeto.instrumentos && projeto.instrumentos.length > 0) || ((projeto as any).total_instrumentos && Number((projeto as any).total_instrumentos) > 0);
    const valid = !!(projeto.nome && projeto.tap_vinculado && projeto.data_prevista_inicio && projeto.data_prevista_conclusao && projeto.objetivo && projeto.contexto_justificativa && projeto.patrocinador_id && projeto.gestor_id && projeto.escopo_sintetico && projeto.fora_do_escopo && hasEntregas && hasInstrumentos && projeto.prioridade && projeto.complexidade && projeto.abrangencia);
    if (!valid) return { label: 'Pendente', color: 'bg-gray-400 text-white', step: 0 };
    if (projeto.tap_validado_patrocinador_em) return { label: 'TAP Vigente', color: 'bg-green-600 text-white', step: 3 };
    if (projeto.tap_validado_diretor_em) return { label: 'Validado 2/3', color: 'bg-blue-500 text-white', step: 2 };
    if (projeto.tap_validado_gestor_em) return { label: 'Validado 1/3', color: 'bg-blue-400 text-white', step: 1 };
  ...
  const getTepStatusLocal = (projeto: ProjetoCadastro) => {
    ...
    if (p.tep_validado_patrocinador_em) return { label: 'TEP Vigente', color: 'bg-green-600 text-white', step: 3 };
    if (p.tep_validado_diretor_em) return { label: 'Validado 2/3', color: 'bg-blue-500 text-white', step: 2 };
    if (p.tep_validado_gestor_em) return { label: 'Validado 1/3', color: 'bg-blue-400 text-white', step: 1 };
    if (p.tep_versao || p.tep_tipo_encerramento) return { label: 'Em validação', color: 'bg-amber-500 text-white', step: 0 };
    return { label: 'Não iniciado', color: 'bg-gray-300 text-gray-700', step: -1 };
  };
  ...
  const renderStatusBadge = (bucket: StatusBucket, opts?: { onClick?: (e: React.MouseEvent) => void; tooltip?: string }) => {
    const config = {
      pendente:    { Icon: Hourglass,    bg: 'bg-gray-100',    text: 'text-gray-700',    iconColor: 'text-gray-500',    label: 'Pendente' },
      em_processo: { Icon: RefreshCw,    bg: 'bg-orange-100',  text: 'text-orange-800',  iconColor: 'text-orange-500',  label: 'Em processo' },
      concluido:   { Icon: CheckCircle2, bg: 'bg-emerald-100', text: 'text-emerald-800', iconColor: 'text-emerald-600', label: 'Concluído' },
    }[bucket];
    return (
      <div
        className={cn(
  ...
  const handleTapClick = async (e: React.MouseEvent, projeto: ProjetoCadastro) => {
    e.stopPropagation();
  ...
  const handleTepClick = async (e: React.MouseEvent, projeto: ProjetoCadastro) => {
    e.stopPropagation();
  ...
  const [planoSelecionado, setPlanoSelecionado] = useState<PlanoComProjetos | null>(null);
  ...
  const [projetosVinculados, setProjetosVinculados] = useState<ProjetoCadastro[]>([]);
  const [projetoDetalhes, setProjetoDetalhes] = useState<ProjetoCadastro | null>(null);
  ...
  const [tepDialogProjeto, setTepDialogProjeto] = useState<ProjetoCadastro | null>(null);
  ...
  const [projetoEditDialogMode, setProjetoEditDialogMode] = useState<'create' | 'edit' | 'view'>('edit');
  ...
  const [projetoEditDialogTapMode, setProjetoEditDialogTapMode] = useState(false);
  ...
  const [instrumentoDetalhes, setInstrumentoDetalhes] = useState<InstrumentoPlanejamento | null>(null);
  ...
  const [tarefasPorEntrega, setTarefasPorEntrega] = useState<Record<number, TarefaEntrega[]>>({});
  ...
  const [modalProjetoInfoCompletaOpen, setModalProjetoInfoCompletaOpen] = useState(false);

  // Estado de pesquisa de projetos
  const [buscaProjeto, setBuscaProjeto] = useState('');

  // Estado de filtro por status do projeto
  const [filtroStatus, setFiltroStatus] = useState<'todos' | 'concluido' | 'em_execucao' | 'planejado' | 'suspenso' | 'cancelado'>('todos');
  const [filtroSituacao, setFiltroSituacao] = useState<'todos' | 'no_prazo' | 'em_atraso' | 'finalizado'>('todos');
  const [filtroSaude, setFiltroSaude] = useState<'todos' | 'verde' | 'amarelo' | 'vermelho'>('todos');
  const [filtroPrioridade, setFiltroPrioridade] = useState<'todos' | 'alta' | 'media' | 'baixa'>('todos');
  const [filtroTap, setFiltroTap] = useState<'todos' | 'pendente' | 'em_processo' | 'concluido'>('todos');
  const [filtroTep, setFiltroTep] = useState<'todos' | 'pendente' | 'em_processo' | 'concluido'>('todos');
  ...
  const [projetoIdParaTarefa, setProjetoIdParaTarefa] = useState<number | null>(null);
  const [tarefaEditando, setTarefaEditando] = useState<GestaoTarefa | null>(null);
  const [projetoEditando, setProjetoEditando] = useState<{ id: number; nome: string } | null>(null);
  const [itemParaDeletar, setItemParaDeletar] = useState<{ tipo: 'plano' | 'projeto' | 'tarefa' | 'tarefaEntrega' | 'entrega'; id: number; nome: string } | null>(null);
  ...
  const [novaEntregaAreaId, setNovaEntregaAreaId] = useState<number | null>(null);
  const [novaEntregaPrazo, setNovaEntregaPrazo] = useState('');
  const [novaEntregaAreaPopoverOpen, setNovaEntregaAreaPopoverOpen] = useState(false);
  ...
  const [editEntregaAreaPopoverOpen, setEditEntregaAreaPopoverOpen] = useState(false);
  ...
  const [draggedTarefa, setDraggedTarefa] = useState<{ projetoId: number; index: number } | null>(null);
  const [dragOverTarget, setDragOverTarget] = useState<{ projetoId: number; index: number } | null>(null);
  ...
  const calcularEstatisticasTarefas = () => {
    ...
    const progresso = totalTarefas > 0 ? Math.round((progressoConcluido / totalTarefas) * 100) : 0;

    return { totalTarefas, sprintAtual, foraSprint, concluida, aFazer, fazendo, progressoConcluido, progresso };
  };
  ...
  const carregarDados = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn('Aviso: Não foi possível carregar os sprints:', sprintError);
      ...
      try {
        ...
        console.warn('Aviso: Não foi possível carregar os usuários:', usersError);
  ...
  const carregarProjetosDoPlano = useCallback(async (planoId: number) => {
    try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        ['projetoId', 'openTap', 'openTep'].forEach(p => url.searchParams.delete(p));
        window.history.replaceState({}, '', url.toString());
      } catch (err) {
        console.warn('[ControleExecucaoNovo] Falha no deep-link de TAP/TEP:', err);
      }
    })();
  ...
  useEffect(() => {
    ...
    let canceled = false;
    permissoesTapApi.podeEditarProjeto(projetoDetalhes.id)
  ...
  useEffect(() => {
    if (!projetoDetalhes
      || (projetoDetalhes.status !== 'concluido' && projetoDetalhes.status !== 'cancelado')) {
      setProjetoTep(null);
      return;
    }
    let cancelado = false;
  ...
  const tepPendenteUsuario = useMemo(() => {
    ...
    const patrocinadorUid = Number((projetoDetalhes as any).patrocinador_user_id);
    // Camada 1 - Gestor
    if (gestorUid === uid && !projetoTep.tep_validado_gestor_em) return true;
    // Camada 2 - Diretor (gestor já validou)
    if (diretorUid === uid && projetoTep.tep_validado_gestor_em && !projetoTep.tep_validado_diretor_em) return true;
    // Camada 3 - Patrocinador (diretor já validou)
    if (patrocinadorUid === uid && projetoTep.tep_validado_diretor_em && !projetoTep.tep_validado_patrocinador_em) return true;
    return false;
  ...
  const tapPendenteUsuario = useMemo(() => {
    ...
    const patrocinadorUid = Number((projetoDetalhes as any).patrocinador_user_id);
    // Camada 1 - Gestor
    if (gestorUid === uid && !projetoDetalhes.tap_validado_gestor_em) return true;
    // Camada 2 - Diretor (gestor já validou)
    if (diretorUid === uid && projetoDetalhes.tap_validado_gestor_em && !projetoDetalhes.tap_validado_diretor_em) return true;
    // Camada 3 - Patrocinador (diretor já validou)
    if (patrocinadorUid === uid && projetoDetalhes.tap_validado_diretor_em && !projetoDetalhes.tap_validado_patrocinador_em) return true;
    return false;
  ...
  const projetosBase = activeTab === 'meus'
    ? meusProjetos
    : (planoFiltroId ? projetosVinculados : todosProjetos);
  ...
  const projetosPorDiretoria = filtroUnidade !== 'todos'
    ? projetosBase.filter(p => {
        const areasStr = (p as any).areas_execucao_diretorias || '';
        return areasStr.split(', ').some((a: string) => a.trim() === filtroUnidade);
      })
    : projetosBase;

  // Gestores únicos dos projetos (para o filtro)
  const gestoresDeProjetos = [...new Map(
  ...
  const getSituacaoProjeto = (projeto: Projeto): 'no_prazo' | 'em_atraso' | 'finalizado' | 'encerrado' => {
    if (projeto.status === 'cancelado') return 'encerrado';
    if (projeto.status === 'concluido') return 'finalizado';
    if (projeto.data_prevista_conclusao && new Date(projeto.data_prevista_conclusao) < new Date()) return 'em_atraso';
    return 'no_prazo';
  };

  const situacaoLabels: Record<string, string> = { no_prazo: 'No prazo', em_atraso: 'Em atraso', finalizado: 'Finalizado', encerrado: 'Encerrado' };
  const situacaoColors: Record<string, string> = { no_prazo: 'bg-blue-100 text-blue-700', em_atraso: 'bg-red-100 text-red-700', finalizado: 'bg-green-100 text-green-700', encerrado: 'bg-gray-100 text-gray-700' };
  ...
  const projetosFiltradosPorCampos = projetosPorDiretoria.filter(projeto => {
    ...
    if (filtroSituacao !== 'todos' && getSituacaoProjeto(projeto) !== filtroSituacao) return false;
    if (filtroSaude !== 'todos' && (projeto.saude || 'verde') !== filtroSaude) return false;
    if (filtroPrioridade !== 'todos' && (projeto.prioridade || 'media') !== filtroPrioridade) return false;
    if (filtroTap !== 'todos') {
    ...
    if (filtroGestor !== 'todos' && projeto.gestor_id?.toString() !== filtroGestor) return false;
    return true;
  });

  // Aplicar filtro de pesquisa e ordenar por prazo mais próximo
  const projetosFiltrados = (buscaProjeto.trim()
    ? projetosFiltradosPorCampos.filter(projeto =>
      (projeto.nome || '').toLowerCase().includes(buscaProjeto.toLowerCase()) ||
      (projeto.codigo || '').toLowerCase().includes(buscaProjeto.toLowerCase())
    )
    : projetosFiltradosPorCampos
  ).sort((a, b) => {
    const dataA = a.data_prevista_conclusao ? new Date(a.data_prevista_conclusao).getTime() : Infinity;
    const dataB = b.data_prevista_conclusao ? new Date(b.data_prevista_conclusao).getTime() : Infinity;
    return dataA - dataB;
  });
  ...
  const estatisticasProjetos = {
    ...
    concluidos: projetosPorDiretoria.filter(p => p.status === 'concluido').length,
    emAndamento: projetosPorDiretoria.filter(p => p.status === 'em_execucao').length,
    naoIniciados: projetosPorDiretoria.filter(p => p.status === 'planejado').length,
  };

  // Dados dos gráficos do dashboard
  const dashboardStatusData = useMemo(() => [
    { name: 'Planejado', value: projetosPorDiretoria.filter(p => p.status === 'planejado').length },
    { name: 'Em Execução', value: projetosPorDiretoria.filter(p => p.status === 'em_execucao').length },
    { name: 'Concluído', value: projetosPorDiretoria.filter(p => p.status === 'concluido').length },
  ], [projetosPorDiretoria]);

  const dashboardSituacaoData = useMemo(() => {
    let noPrazo = 0, emAtraso = 0, finalizado = 0, encerrado = 0;
    const now = new Date();
    projetosPorDiretoria.forEach(p => {
      if (p.status === 'cancelado') encerrado++;
      else if (p.status === 'concluido') finalizado++;
      else if (p.data_prevista_conclusao && new Date(p.data_prevista_conclusao) < now) emAtraso++;
      else noPrazo++;
    });
    return [
  ...
  const dashboardSaudeData = useMemo(() => [
    { name: 'Saudável', value: projetosPorDiretoria.filter(p => p.saude === 'verde').length, color: '#4ade80' },
    { name: 'Atenção', value: projetosPorDiretoria.filter(p => p.saude === 'amarelo').length, color: '#fbbf24' },
    { name: 'Crítico', value: projetosPorDiretoria.filter(p => p.saude === 'vermelho').length, color: '#f87171' },
  ], [projetosPorDiretoria]);

  const dashboardPrioridadeData = useMemo(() => {
    const baixa = projetosPorDiretoria.filter(p => p.prioridade === 'baixa').length;
    const media = projetosPorDiretoria.filter(p => p.prioridade === 'media').length;
    const alta = projetosPorDiretoria.filter(p => p.prioridade === 'alta').length;
    const maxVal = Math.max(baixa, media, alta, 1);
    return [
      { label: 'Baixa', value: baixa, stars: 1, barColor: 'bg-blue-500', percent: (baixa / maxVal) * 100 },
      { label: 'Média', value: media, stars: 2, barColor: 'bg-blue-500', percent: (media / maxVal) * 100 },
      { label: 'Alta', value: alta, stars: 3, barColor: 'bg-blue-500', percent: (alta / maxVal) * 100 },
    ];
  }, [projetosPorDiretoria]);
  ...
  const handleUploadEvidencia = async (entregaId: number, file: File) => {
    if (!file.name.toLowerCase().endsWith('.pdf')) {
      toast({ title: 'Formato inválido', description: 'Apenas arquivos PDF são permitidos.', variant: 'destructive' });
      return;
    }
  ...
  const handleVerEntregaDetalhes = async (entrega: Entrega) => {
    ...
    try {
      ...
      if (entrega.status !== statusCorreto) {
        setEntregaSelecionada(prev => prev ? { ...prev, status: statusCorreto as any } : prev);
      }
    } catch (error) {
  ...
  const calcularStatusEntrega = (tarefas: any[]): string => {
    ...
    const emAndamento = statuses.filter(s => s === 'fazendo' || s === 'em_andamento').length;

    console.log('🔍 Calculando status da entrega:', {
      statuses,
      total,
      feitos,
      aFazer,
    ...
    if (feitos === total) {
      console.log('✅ Todas concluídas -> CONCLUÍDA');
      return 'concluida';
    }

    // Regra 2: APENAS se todas as tarefas estiverem marcadas como a fazer
    if (aFazer === total) {
      console.log('✅ Todas a fazer -> NÃO INICIADA');
      return 'nao_iniciada';
    }

    // Regra 3: se pelo menos uma estiver concluida ou em andamento
    if (feitos > 0 || emAndamento > 0) {
      console.log('✅ Pelo menos uma concluída ou em andamento -> EM ANDAMENTO');
      return 'em_andamento';
    }

    console.log('⚠️ Fallback -> NÃO INICIADA');
    return 'nao_iniciada';
  };
  ...
  useEffect(() => {
    ...
    console.log('🔄 useEffect - Atualizando status da entrega:', {
      entregaId: entregaSelecionada.id,
      statusAtual: entregaSelecionada.status,
      novoStatus,
      tarefasCount: tarefasParaCalculo.length,
      precisaAtualizar: entregaSelecionada.status !== novoStatus
    });

    if (entregaSelecionada.status !== novoStatus) {
      console.log('✏️ Atualizando estado da entrega de', entregaSelecionada.status, 'para', novoStatus);
      setEntregaSelecionada((prev: any) => prev ? { ...prev, status: novoStatus } : prev);
  ...
  const handleAbrirEditEntrega = (entrega: Entrega) => {
    ...
    setEditEntregaPrazo(entrega.prazo_estimado ? String(entrega.prazo_estimado).substring(0, 10) : '');
  ...
  const handleSalvarEditEntrega = async () => {
    ...
    if (!editEntregaNome.trim()) {
      toast({ title: 'Atenção', description: 'Digite o nome da entrega', variant: 'destructive' });
      return;
    }
  ...
  const handleEditarProjeto = async () => {
    if (!projetoEditando || !novoProjetoNome.trim() || novoProjetoNome.trim().length < 3) {
      toast({
        title: 'Erro',
        description: 'O nome do projeto deve ter pelo menos 3 caracteres.',
        variant: 'destructive',
  ...
  const handleAbrirModalEditarProjeto = (projeto: { id: number; nome: string }) => {
  ...
  const handleAtualizarStatusTarefa = async (tarefa: GestaoTarefa, novoStatus: GestaoTarefaStatus) => {
    if (!planoSelecionado) return;
    try {
  ...
  const handleAtualizarProgressoTarefa = async (tarefa: GestaoTarefa, novoProgresso: GestaoTarefaProgresso) => {
    if (!planoSelecionado) return;
    try {
  ...
  const handleDragStart = (e: React.DragEvent, projetoId: number, index: number) => {
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/plain', JSON.stringify({ projetoId, index }));
    setTimeout(() => setDraggedTarefa({ projetoId, index }), 0);
  };

  const handleDragOver = (e: React.DragEvent, projetoId: number, index: number) => {
    e.preventDefault();
  ...
  const handleDrop = async (e: React.DragEvent, targetProjetoId: number, targetIndex: number) => {
    e.preventDefault();
    e.stopPropagation();

    if (!draggedTarefa || !planoSelecionado || draggedTarefa.projetoId !== targetProjetoId) {
    ...
    // Encontrar o projeto
    const projeto = planoSelecionado.projetos.find(p => p.id === targetProjetoId);
    if (!projeto) return;
  ...
  const getStatusBadge = (status: GestaoTarefaStatus) => {
    const config = {
      sprint_atual: { label: 'Sprint Atual', className: 'bg-yellow-400 hover:bg-yellow-500 text-gray-900 border-0 rounded-full px-4' },
      fora_sprint: { label: 'Fora da Sprint', className: 'bg-gray-400 hover:bg-gray-500 text-white border-0 rounded-full px-4' },
      concluida: { label: 'Concluída', className: 'bg-green-500 hover:bg-green-600 text-white border-0 rounded-full px-4' },
    };
  ...
  const getProgressoBadge = (progresso: GestaoTarefaProgresso) => {
    const config = {
      a_fazer: { label: 'A Fazer', className: 'bg-orange-400 hover:bg-orange-500 text-white border-0 rounded-full px-4' },
      fazendo: { label: 'Fazendo', className: 'bg-yellow-400 hover:bg-yellow-500 text-gray-900 border-0 rounded-full px-4' },
      feito: { label: 'Feito', className: 'bg-green-500 hover:bg-green-600 text-white border-0 rounded-full px-4' },
    };
  ...
  const getStatusSelectClass = (status: GestaoTarefaStatus) => {
    const classes = {
      sprint_atual: 'bg-yellow-400 text-gray-900 border-yellow-500 hover:bg-yellow-500',
  ...
  const getProgressoSelectClass = (progresso: GestaoTarefaProgresso) => {
    const classes = {
      ...
      fazendo: 'bg-yellow-400 text-gray-900 border-yellow-500 hover:bg-yellow-500',
  ...
  const renderSaudeProjeto = (saude: string) => {
    const config: Record<string, { label: string; bgColor: string; textColor: string; dotColor: string }> = {
      verde: { label: 'Saudável', bgColor: 'bg-emerald-50', textColor: 'text-emerald-700', dotColor: 'bg-emerald-500' },
      amarelo: { label: 'Atenção', bgColor: 'bg-amber-50', textColor: 'text-amber-700', dotColor: 'bg-amber-500' },
      vermelho: { label: 'Crítico', bgColor: 'bg-red-50', textColor: 'text-red-700', dotColor: 'bg-red-500' },
    };
    const { label, bgColor, textColor, dotColor } = config[saude] || config.verde;
    return (
    ...
    <div className="space-y-6">
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-end gap-4 animate-[fade-in-up_0.4s_ease-out_both]" style={{ animationDelay: '0ms' }}>
              ...
              <Select value={filtroUnidade} onValueChange={setFiltroUnidade}>
                ...
                <SelectContent>
                  ...
                  {(() => {
                    ...
                    const projetosRef = planoFiltroId ? projetosVinculados : todosProjetos;
                    const areasUnicas = new Set<string>();
                    projetosRef.forEach(p => {
                      const areasStr = (p as any).areas_execucao_diretorias || '';
                      areasStr.split(', ').forEach((nome: string) => {
                        const trimmed = nome.trim();
                        if (trimmed && !trimmed.startsWith('auto:')) areasUnicas.add(trimmed);
                      });
                    });
                    return Array.from(areasUnicas).sort((a, b) => a.localeCompare(b, 'pt-BR')).map(nomeArea => (
          ...
          <div key={`dashboard-${planoFiltroId || 'all'}-${filtroUnidade}`} className="space-y-6">

          {/* Header do Portfólio/Programa */}
          <div className="bg-blue-50 rounded-xl border border-blue-100 shadow-sm overflow-hidden flex animate-[fade-in-up_0.4s_ease-out_both]" style={{ animationDelay: '80ms' }}>
            {/* Barra lateral azul */}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 xl:gap-4 2xl:gap-5 animate-[fade-in-up_0.4s_ease-out_both]" style={{ animationDelay: '160ms' }}>
            ...
            >
              ...
              <div className="p-4 pl-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-extrabold text-gray-600 uppercase tracking-wide">Projetos</p>
                  <p className="text-3xl font-bold text-slate-700">{estatisticasProjetos.total}</p>
                </div>
              </div>
            </button>
            ...
            >
              ...
              <div className="p-4 pl-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-extrabold text-gray-600 uppercase tracking-wide">Concluídos</p>
                  <p className="text-3xl font-bold text-emerald-600">{estatisticasProjetos.concluidos}</p>
                </div>
              </div>
            </button>
            ...
            >
              ...
              <div className="p-4 pl-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-extrabold text-gray-600 uppercase tracking-wide">Em Andamento</p>
                  <p className="text-3xl font-bold text-amber-600">{estatisticasProjetos.emAndamento}</p>
                </div>
              </div>
            </button>
            ...
            >
              ...
              <div className="p-4 pl-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-extrabold text-gray-600 uppercase tracking-wide">Não Iniciado</p>
                  <p className="text-3xl font-bold text-rose-600">{estatisticasProjetos.naoIniciados}</p>
                </div>
              </div>
            </button>
          </div>
          ...
          {projetosPorDiretoria.length > 0 && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-[fade-in-up_0.4s_ease-out_both]" style={{ animationDelay: '240ms' }}>
              {/* Status Donut */}
              <GraficoRosca
              ...
              <Card className="border border-gray-200 shadow-md rounded-lg flex flex-col !h-[280px]">
                ...
                <CardContent className="flex-1 p-2 overflow-hidden">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={dashboardSaudeData} margin={{ top: 18, right: 5, left: 5, bottom: 0 }}>
                      <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                      <RechartsTooltip />
                      <Bar dataKey="value" radius={[3, 3, 0, 0]} barSize={52}>
                        <LabelList dataKey="value" position="top" style={{ fontSize: 11, fontWeight: 'bold' }} />
                ...
                <CardContent className="flex-1 p-4 flex flex-col justify-center gap-5">
                  {dashboardPrioridadeData.map(item => (
                    <div key={item.label} className="flex items-center gap-2">
                      <span className="text-xs text-gray-600 w-10 text-right flex-shrink-0">{item.label}</span>
                      <div className="flex gap-0.5 flex-shrink-0">
                        {[1, 2, 3].map(s => (
                          <Star key={s} className={`h-3 w-3 ${s <= item.stars ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />
                        ))}
                      </div>
                      <div className="flex-1 bg-gray-100 rounded h-5 overflow-hidden">
                        <div className={`h-5 rounded ${item.barColor} transition-all duration-700`} style={{ width: `${item.percent}%` }} />
                      </div>
                      <span className="text-xs font-bold text-gray-700 w-6 text-right flex-shrink-0">{item.value}</span>
                    </div>
                  ))}
                </CardContent>
              </Card>
          ...
          {ehGestorDeProjeto && (
            <div className="flex gap-1 border-b border-gray-300 animate-[fade-in-up_0.4s_ease-out_both]" style={{ animationDelay: '300ms' }}>
              <button
                type="button"
          ...
          {/* Tabela de Projetos */}
          <div className="bg-gray-300 rounded-2xl border border-gray-400 overflow-hidden shadow-sm animate-[fade-in-up_0.4s_ease-out_both]" style={{ animationDelay: '320ms' }}>
            {/* Header da Tabela */}
            <div className="px-6 py-4 bg-gray-200 border-b border-gray-400">
              <div className="flex items-center justify-between">
                  ...
                  <Select value={filtroGestor} onValueChange={setFiltroGestor}>
                    ...
                    <SelectContent>
                      ...
                      {gestoresDeProjetos.map(([id, nome]) => (
                        <SelectItem key={id} value={id!.toString()}>{nome}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="hidden lg:flex items-center text-lg font-bold text-gray-800">
                  ...
                  <Select value={filtroSituacao} onValueChange={(v: any) => setFiltroSituacao(v)}>
                    <SelectTrigger className="w-28 border-0 !bg-transparent shadow-none h-auto p-0 justify-center gap-1 text-lg font-bold text-gray-800 hover:text-gray-600 focus:ring-0 focus:ring-offset-0 focus:outline-none">
                      <span>Situação</span>
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Select value={filtroTap} onValueChange={(v: any) => setFiltroTap(v)}>
                    ...
                    <SelectContent>
                      <SelectItem value="todos">Todos</SelectItem>
                      <SelectItem value="pendente"><div className="flex items-center gap-2"><Hourglass className="h-3 w-3 text-gray-500" />Pendente</div></SelectItem>
                      <SelectItem value="em_processo"><div className="flex items-center gap-2"><RefreshCw className="h-3 w-3 text-orange-500" />Em processo</div></SelectItem>
                      <SelectItem value="concluido"><div className="flex items-center gap-2"><CheckCircle2 className="h-3 w-3 text-emerald-600" />Concluído</div></SelectItem>
                    </SelectContent>
                  </Select>
                  <Select value={filtroTep} onValueChange={(v: any) => setFiltroTep(v)}>
                    <SelectTrigger className="w-32 border-0 !bg-transparent shadow-none h-auto p-0 justify-center gap-1 text-lg font-bold text-gray-800 hover:text-gray-600 focus:ring-0 focus:ring-offset-0 focus:outline-none">
                      <span>TEP</span>
                    </SelectTrigger>
                    <SelectContent>
            ...
            {/* Lista de Projetos */}
            {loadingPlano ? (
              <div className="text-center py-16 text-gray-400 bg-white">Carregando projetos...</div>
              ...
              <div className="py-20 text-center bg-white">
                ...
                <p className="text-gray-700 font-semibold text-lg">
                  {planoFiltroId
                    ...
                    : (filtroStatus !== 'todos' || filtroSituacao !== 'todos' || filtroSaude !== 'todos' || filtroPrioridade !== 'todos' || filtroTap !== 'todos')
                      ? 'Nenhum projeto com os filtros selecionados'
                      : 'Nenhum projeto encontrado'
                  }
                </p>
                <p className="text-gray-400 text-sm mt-2">
                  {planoFiltroId ? 'Este plano/programa não possui projetos.' : 'Cadastre projetos em Cadastros → Projetos.'}
                </p>
              </div>
            ) : (
              <div className="bg-white">
                {projetosFiltrados.map((projeto, index) => {
                  const progresso = (projeto as any).progresso_percentual || (projeto as any).progresso || 0;
                  const prazoEstimado = (projeto as any).data_prevista_conclusao || (projeto as any).data_fim_prevista
                    ? new Date((projeto as any).data_prevista_conclusao || (projeto as any).data_fim_prevista).toLocaleDateString('pt-BR', { month: '2-digit', year: 'numeric' })
                    : '—';
                  ...
                  return (
                    <div
                      ...
                      className={`group flex items-center justify-between px-6 py-5 hover:bg-slate-50 transition-all cursor-pointer ${index !== projetosFiltrados.length - 1 ? 'border-b border-gray-100' : ''
                        }`}
                    >
                      {/* Info do Projeto */}
                      ...
                      <div className="hidden lg:flex items-center">
                        ...
                        <div className="w-36 flex flex-col items-center gap-1 px-3 mt-2">
                          ...
                          <span className="text-xs text-gray-700 font-bold">{progresso}%</span>
                        </div>

                        {/* Status */}
                        <div className="w-28 flex justify-center">
                          <Badge className={`${statusProjetoColors[projeto.status || 'planejado']} border-0 text-xs shadow-sm whitespace-nowrap`}>
                            {statusProjetoLabels[projeto.status || 'planejado']}
                          </Badge>
                        </div>

                        {/* Situação */}
                        <div className="w-28 flex justify-center">
                          <Badge className={`${situacaoColors[getSituacaoProjeto(projeto)]} border-0 text-xs shadow-sm whitespace-nowrap`}>
                        ...
                        <div className="w-36 flex justify-center">
                          {renderPrioridadeStars(projeto.prioridade || 'media')}
                        </div>
          ...
          </div>{/* fim do wrapper de animações */}
        </>
      )}
    </div>
  );
  ...
  const getProjetoStatusBadge = (status: string) => {
    const configs: Record<string, { label: string; className: string }> = {
      ...
      'cancelado': { label: 'Descontinuado', className: 'bg-gray-200 text-gray-700' },
    };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 rounded-xl border border-blue-100 shadow-sm overflow-hidden flex">
          ...
          <div className="flex-1 px-6 py-4 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-blue-600">{projetoDetalhes.nome}</h2>
              {projetoDetalhes.descricao_sintetica && (
                <p className="text-gray-500 text-sm mt-1">{projetoDetalhes.descricao_sintetica}</p>
              )}
            </div>
            {canEdit && (
              <button
          ...
          <div className="bg-gray-100 border border-gray-200 rounded-lg p-5 h-[650px] overflow-y-auto">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3 text-sm">
              <div>
                ...
                <p className="text-gray-900">{projetoDetalhes.patrocinador_nome || '-'}</p>
              </div>

              <div>
                <p className="text-gray-600 font-medium">Gestor do Projeto</p>
                <p className="text-gray-900">{projetoDetalhes.gestor_nome || '-'}</p>
                {projetoDetalhes.diretorias_nomes && (
                  <p className="text-gray-500 text-xs mt-1">{projetoDetalhes.diretorias_nomes}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="h-8 w-[160px] bg-gray-100 text-slate-700 font-medium rounded-md flex items-center px-3 gap-2 text-sm cursor-default border border-gray-200/50">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${projetoDetalhes.status === 'concluido' ? 'bg-green-500' :
                      projetoDetalhes.status === 'em_execucao' ? 'bg-blue-500' :
                        projetoDetalhes.status === 'cancelado' ? 'bg-gray-500' :
                          'bg-gray-400'
                      }`} />
                    <span>{statusProjetoLabels[projetoDetalhes.status] || projetoDetalhes.status}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {projetoDetalhes.saude === 'verde' ? 'Saudável' :
                    projetoDetalhes.saude === 'amarelo' ? 'Atenção' :
                      projetoDetalhes.saude === 'vermelho' ? 'Crítico' : '-'}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {projetoDetalhes.data_prevista_conclusao
                    ? new Date(projetoDetalhes.data_prevista_conclusao).toLocaleDateString('pt-BR', { month: '2-digit', year: 'numeric' })
                    : '-'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-col gap-2">
              {(() => {
                ...
                const tapRecusadoParaGestor = !!projetoDetalhes.tap_recusado_em
                ...
                return (
                  ...
                  >
                    {destacar
                      ? <AlertTriangle className={cn('mr-2 h-4 w-4', tapRecusadoParaGestor ? 'text-red-600' : 'text-amber-600')} />
                    ...
                    {tapRecusadoParaGestor
                      ...
                      : tapPendenteUsuario
                        ...
                        : (projetoDetalhes.tap_versao && projetoDetalhes.tap_versao > 0)
                          ? (tapPronto ? 'Visualizar TAP' : 'Gerar TAP')
              ...
              {(() => {
                ...
                const uid = user?.id ? Number(user.id) : null;
                const tepRecusadoParaGestor = !!projetoTep?.tep_recusado_em
                ...
                return (
                  <Button
                    onClick={() => {
                      if (tepBloqueadoSemTap) {
                        toast({
                          ...
                          description: 'Conclua o ciclo de validação do TAP (gestor → diretor → patrocinador) antes de finalizar o projeto.',
                          variant: 'destructive',
                        });
                        return;
                      }
                    ...
                    title={tepBloqueadoSemTap ? 'Conclua a validação do TAP antes de finalizar o projeto' : undefined}
                  >
                    {destacar
                      ? <AlertTriangle className={cn('mr-2 h-4 w-4', tepRecusadoParaGestor ? 'text-red-600' : 'text-amber-600')} />
                      : <FileText className="mr-2 h-4 w-4" />}
                    {tepRecusadoParaGestor
                      ? 'TEP recusado — revisar'
                      : tepPendenteUsuario
                        ? 'Validar TEP — Pendente'
                        : tepVigente ? 'Visualizar TEP' : 'Finalizar Projeto'}
                  </Button>
                );
              })()}
            </div>
              ...
              {entregas.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-500 py-12">
                  ...
                  {podeEditarEntregas ? (
                    ...
                    <p className="text-sm text-gray-400">Este projeto ainda não possui entregas cadastradas.</p>
                  )}
                </div>
              ) : (
              <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-blue-600">
                    <th className="text-left py-4 px-6 text-white font-semibold text-sm" style={{ width: '23%' }}>Nome da Entrega</th>
                    <th className="text-center py-4 px-6 text-white font-semibold text-sm" style={{ width: '18%' }}>Área Responsável</th>
                    <th className="text-center py-4 px-6 text-white font-semibold text-sm" style={{ width: '13%' }}>Prazo Estimado</th>
                    <th className="text-center py-4 px-6 text-white font-semibold text-sm" style={{ width: '14%' }}>Status</th>
                    <th className="text-center py-4 px-6 text-white font-semibold text-sm" style={{ width: '22%' }}>Evidências</th>
                    <th className="text-center py-4 px-6 text-white font-semibold text-sm" style={{ width: '10%' }}>Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {[...entregas].sort((a, b) => {
                      ...
                      const pa = a.prazo_estimado ? String(a.prazo_estimado).slice(0, 10) : '';
                      const pb = b.prazo_estimado ? String(b.prazo_estimado).slice(0, 10) : '';
                      if (pa && pb) {
                      ...
                      return (
                        <tr
                          ...
                          onClick={isProduction() ? undefined : () => handleVerEntregaDetalhes(entrega)}
                        >
                            ...
                            {podeEditarEntregas ? (
                              <div className="flex items-center justify-center gap-1">
                                <Button
                                  ...
                                  onClick={() => handleAbrirEditEntrega(entrega)}
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    setItemParaDeletar({ tipo: 'entrega', id: entrega.id, nome: entrega.nome });
  ...
  const renderModalNovaEntrega = () => {
    return (
      <Dialog open={modalNovaEntregaOpen} onOpenChange={setModalNovaEntregaOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Popover open={novaEntregaAreaPopoverOpen} onOpenChange={setNovaEntregaAreaPopoverOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="flex-1 min-w-0 text-left whitespace-normal break-words">
                      {novaEntregaAreaId
                        ? (() => {
                            const a = areas.find(x => x.id === novaEntregaAreaId);
                            return a ? `${a.diretoria} - ${a.nome_area}` : 'Selecione a área responsável';
                          })()
                        : <span className="text-gray-500">Selecione a área responsável</span>}
                    </span>
                      ...
                      <CommandGroup>
                        {areas.map(area => {
                          ...
                          return (
                            ...
                            >
                              <Check className={cn('mr-2 h-4 w-4', novaEntregaAreaId === area.id ? 'opacity-100' : 'opacity-0')} />
                              {label}
                            </CommandItem>
                          );
                        })}
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                max={projetoDetalhes?.data_prevista_conclusao
                  ? String(projetoDetalhes.data_prevista_conclusao).slice(0, 10)
                  : undefined}
              />
              {projetoDetalhes?.data_prevista_conclusao && (
                <p className="text-xs text-gray-500">
                  Conclusão prevista do projeto: {new Date(projetoDetalhes.data_prevista_conclusao).toLocaleDateString('pt-BR')}
                </p>
              )}
              {novaEntregaPrazo && projetoDetalhes?.data_prevista_conclusao
                && new Date(novaEntregaPrazo) > new Date(projetoDetalhes.data_prevista_conclusao) && (
                <p className="text-xs text-red-600 flex items-center gap-1">
                  <AlertTriangle className="h-3 w-3" />
                  Prazo posterior à conclusão prevista do projeto. Ajuste a data antes de salvar.
                </p>
              )}
            </div>
          </div>
  ...
  const renderModalEditEntrega = () => {
    return (
      <Dialog open={modalEditEntregaOpen} onOpenChange={setModalEditEntregaOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
            ...
            <DialogDescription>
              Altere as informações da entrega do projeto {projetoDetalhes?.nome}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Popover open={editEntregaAreaPopoverOpen} onOpenChange={setEditEntregaAreaPopoverOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    role="combobox"
                  ...
                  >
                    <span className="flex-1 min-w-0 text-left whitespace-normal break-words">
                      {editEntregaAreaId
                        ? (() => {
                            const a = areas.find(x => x.id === editEntregaAreaId);
                            return a ? `${a.diretoria} - ${a.nome_area}` : 'Selecione a área responsável';
                          })()
                        : <span className="text-gray-500">Selecione a área responsável</span>}
                    </span>
                      ...
                      <CommandGroup>
                        {areas.map(area => {
                          ...
                          return (
                            ...
                            >
                              <Check className={cn('mr-2 h-4 w-4', editEntregaAreaId === area.id ? 'opacity-100' : 'opacity-0')} />
                              {label}
                            </CommandItem>
                          );
                        })}
            ...
            <div className="grid gap-2">
              ...
              {editEntregaPrazo && projetoDetalhes?.data_prevista_conclusao
                && new Date(editEntregaPrazo) > new Date(projetoDetalhes.data_prevista_conclusao) && (
                <p className="text-xs text-red-600 flex items-center gap-1">
                  <AlertTriangle className="h-3 w-3" />
                  Prazo posterior à conclusão prevista do projeto. Ajuste a data antes de salvar.
                </p>
  ...
  const renderTelaEntregaDetalhes = () => {
    ...
    console.log('🎨 Renderizando tela de detalhes:', {
      entregaId: entregaSelecionada.id,
      entregaNome: entregaSelecionada.nome,
      statusAtual: entregaSelecionada.status
    });
    ...
    const naoIniciado = tarefasParaCalculo.filter(t => getStatusValue(t) === 'a_fazer').length;
    const emAndamento = tarefasParaCalculo.filter(t => {
      const s = getStatusValue(t);
      return s === 'fazendo' || s === 'em_andamento';
    }).length;
    const concluido = tarefasParaCalculo.filter(t => getStatusValue(t) === 'feito').length;

    // Se a entrega tem evidência PDF, considerar 100% concluída
    const temEvidenciaPdf = !!entregaSelecionada.evidencia_filename;
    const progresso = temEvidenciaPdf ? 100 : (total > 0 ? Math.round((concluido / total) * 100) : 0);
        ...
        <div className="flex items-center justify-center">
          <div className="bg-gray-100 rounded-lg border border-gray-200 px-8 py-4 flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-xl font-bold text-blue-600">{entregaSelecionada.nome}</h2>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-5 xl:gap-6 2xl:gap-8">
          ...
          <div className="bg-gray-100 border border-gray-200 rounded-lg p-5 h-[650px] overflow-y-auto">
            ...
            <div className="flex flex-col items-center justify-center mb-6">
              ...
              <p className="mt-2 text-sm text-gray-600">
                {temEvidenciaPdf
                  ? <span className="font-semibold text-green-600">Concluída por evidência</span>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3 text-sm">
              <div>
                ...
                <p className="text-gray-900">{entregaSelecionada.area_responsavel_nome || 'XYZ'}</p>
              </div>
              ...
              <div>
                ...
                <div className="h-8 w-[160px] bg-gray-100 text-slate-700 font-medium rounded-md flex items-center px-3 gap-2 text-sm cursor-default border border-gray-200/50">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${(entregaSelecionada.status === 'concluida' || temEvidenciaPdf) ? 'bg-green-500' :
                      entregaSelecionada.status === 'em_andamento' ? 'bg-orange-500' :
                        'bg-gray-400'
                      }`} />
                    <span>
                      {(entregaSelecionada.status === 'concluida' || temEvidenciaPdf) ? 'Concluída' :
                        entregaSelecionada.status === 'em_andamento' ? 'Em Andamento' :
                          'Não Iniciada'}
                    </span>
                  </div>
                </div>
            ...
            <div className="overflow-x-auto overflow-y-auto flex-1">
              <table className="w-full">
                ...
                <tbody className="bg-white">
                  ...
                  {[...(tarefasPorEntrega[entregaSelecionada.id] || [])].sort((a, b) => (a.nome || '').localeCompare(b.nome || '', 'pt-BR')).map((tarefa) => (
                    <tr key={tarefa.id} className="border-b border-gray-100 hover:bg-gray-50 last:border-b-0">
                      <td className="py-3 px-4 text-gray-900 text-sm">{tarefa.nome}</td>
                      <td className="py-3 px-4 text-gray-900 text-sm text-center">{getSprintNome(tarefa.sprint_id)}</td>
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <Button
                              ...
                              className="h-8 w-8 p-0 text-red-600 hover:text-red-700 hover:bg-red-50"
                              onClick={() => {
                                setItemParaDeletar({ tipo: 'tarefaEntrega', id: tarefa.id, nome: tarefa.nome });
                                setModalConfirmDeleteOpen(true);
                              }}
                            >
                              <Trash2 className="h-4 w-4" />
  ...
  const renderTelaPlanoProjetos = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-gray-900 font-bold text-xl">{instrumentoDetalhes.nome}</h2>
                <p className="text-gray-500 text-sm">
                  {tipoLabels[instrumentoDetalhes.tipo]} • {instrumentoDetalhes.versao}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {instrumentoDetalhes.objetivo && (
              <div className="md:col-span-3">
                ...
                <p className="text-gray-900 text-base">{instrumentoDetalhes.objetivo}</p>
              </div>
            )}
            {instrumentoDetalhes.ambito_institucional && (
              <div>
                ...
                <p className="text-gray-900 text-base">{instrumentoDetalhes.ambito_institucional}</p>
              </div>
            )}
            {instrumentoDetalhes.responsavel_institucional && (
              <div>
                ...
                <p className="text-gray-900 text-base">{instrumentoDetalhes.responsavel_institucional}</p>
              </div>
            )}
            {(instrumentoDetalhes.periodo_vigencia_inicio || instrumentoDetalhes.periodo_vigencia_fim) && (
              <div>
                ...
                <p className="text-gray-900 text-base">
                  {instrumentoDetalhes.periodo_vigencia_inicio
                    ? new Date(instrumentoDetalhes.periodo_vigencia_inicio).toLocaleDateString('pt-BR')
                    : '?'
                  }
                  {' → '}
                  {instrumentoDetalhes.periodo_vigencia_fim
                    ? new Date(instrumentoDetalhes.periodo_vigencia_fim).toLocaleDateString('pt-BR')
                    : '?'
                  }
                </p>
              </div>
        ...
        <div className="bg-gray-200 rounded-xl p-5 shadow-sm border border-gray-100">
          ...
          {loadingPlano ? (
            ...
            <div className="bg-gray-50 rounded-lg p-8 text-center">
              <p className="text-gray-500">Nenhum projeto vinculado a este plano/programa para esta diretoria.</p>
              <p className="text-xs mt-2">Vincule projetos em Cadastros → Projetos com área de execução desta diretoria.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {projetosFiltrados.map((projeto) => {
                const areasExecucao = projeto.diretorias_nomes?.split(', ').filter(Boolean) || [];
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-gray-900 font-medium group-hover:text-[#2d7a5e] transition-colors">{projeto.nome}</h4>
                          </div>
                          <div className="flex flex-wrap items-center gap-2 mt-1">
                            <Badge className={`${statusProjetoColors[projeto.status || 'planejado']} border-0 text-xs`}>
                              {statusProjetoLabels[projeto.status || 'planejado']}
                            </Badge>
                            {(() => {
                              const tap = getTapStatusLocal(projeto);
        ...
        {instrumentoDetalhes.instrumentos_subordinados && instrumentoDetalhes.instrumentos_subordinados.length > 0 && (
          <div className="bg-gray-200 rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 mb-4">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">{tipoLabels[sub.tipo]}</p>
                    </div>
                  </div>
                </button>
              ))}
  ...
  const renderModais = () => (
    <>
      ...
      <Dialog open={modalPlanoOpen} onOpenChange={setModalPlanoOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCriarPlano} className="bg-[#5A8A7A] hover:bg-[#4A7A6A]">
              Criar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={modalProjetoOpen} onOpenChange={(open) => {
        if (!open) {
          setProjetoEditando(null);
          setNovoProjetoNome('');
        }
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{projetoEditando ? 'Editar KR/Projeto' : 'Criar KR/Projeto'}</DialogTitle>
            <DialogDescription>
              {projetoEditando
                ? 'Altere o nome do projeto abaixo.'
                : <>Este projeto será vinculado ao plano: <strong>{planoSelecionado?.nome}</strong></>
              }
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={projetoEditando ? handleEditarProjeto : handleCriarProjeto}
      ...
      <Dialog open={modalTarefaOpen} onOpenChange={setModalTarefaOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{tarefaEditando ? 'Editar Tarefa' : 'Criar Tarefa'}</DialogTitle>
            <DialogDescription>
              {tarefaEditando
              ...
              <>
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setTarefaEditando({ ...tarefaEditando, status: value as GestaoTarefaStatus })
                    }
                  >
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setTarefaEditando({ ...tarefaEditando, progresso: value as GestaoTarefaProgresso })
                    }
                  >
                    <SelectTrigger className="mt-2">
                      <SelectValue />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSalvarTarefa} className="bg-[#5A8A7A] hover:bg-[#4A7A6A]">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {itemParaDeletar?.tipo === 'plano' && (
                <>
                  Deseja realmente excluir o plano <strong>"{itemParaDeletar.nome}"</strong>?
                  <br />
                  <span className="text-red-500">
                    Todos os projetos e tarefas vinculados também serão excluídos.
                  </span>
                </>
              )}
              {itemParaDeletar?.tipo === 'projeto' && (
                <>
                  Deseja realmente excluir o projeto <strong>"{itemParaDeletar.nome}"</strong>?
                  <br />
                  <span className="text-red-500">
              ...
              {itemParaDeletar?.tipo === 'tarefa' && (
                <>
                  ...
                  <br />
                  <span className="text-gray-500">Esta ação não pode ser desfeita.</span>
                </>
              )}
              ...
              {itemParaDeletar?.tipo === 'entrega' && (
                <>
                  ...
                  <br />
                  <span className="text-red-500">
                    Todas as tarefas e a evidência vinculadas também serão excluídas.
                  </span>
                </>
              )}
            </DialogDescription>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        <CommandItem
                          ...
                          onSelect={() => {
                            setNovaTarefaEntrega(prev => ({ ...prev, responsavel: '' }));
                        ...
                        >
                          <Check
                            className={cn(
                              "mr-2 h-4 w-4",
                              !novaTarefaEntrega.responsavel ? "opacity-100" : "opacity-0"
                            )}
                          />
                          Sem responsável
                        </CommandItem>
                        {usuarios.map((usuario) => (
                          <CommandItem
                            ...
                            value={usuario.name}
                            onSelect={() => {
                              setNovaTarefaEntrega(prev => ({ ...prev, responsavel: usuario.name }));
                              setResponsavelPopoverOpen(false);
                            }}
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                novaTarefaEntrega.responsavel === usuario.name ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {usuario.name}
                          </CommandItem>
              ...
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900 font-medium">{instrumentoDetalhes.nome}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Tipo</p>
                    <p className="text-gray-900">{tipoLabels[instrumentoDetalhes.tipo]}</p>
                  </div>
                </div>
                {instrumentoDetalhes.objetivo && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Objetivo</p>
                    <p className="text-gray-900">{instrumentoDetalhes.objetivo}</p>
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">{instrumentoDetalhes.versao || 'v1.0'}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">{instrumentoDetalhes.ambito_institucional || 'Não definido'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">
                    ...
                    <p className="text-gray-900">{instrumentoDetalhes.responsavel_institucional || 'Não definido'}</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Instrumento Superior</p>
                  <p className="text-gray-900">{instrumentoDetalhes.instrumento_superior_nome || 'Nenhum (instrumento raiz)'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Projetos Vinculados</p>
                  <p className="text-gray-900">
                    {instrumentoDetalhes.projetos_nomes || 'Nenhum projeto vinculado'}
                  </p>
                </div>
                {instrumentoDetalhes.total_instrumentos_subordinados && instrumentoDetalhes.total_instrumentos_subordinados > 0 && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Instrumentos Subordinados</p>
                    <p className="text-gray-900">{instrumentoDetalhes.total_instrumentos_subordinados} instrumento(s)</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="p-4 space-y-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 whitespace-pre-wrap">
                    {instrumentoDetalhes.historico_alteracoes || 'Nenhum histórico registrado'}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Observações Gerais</p>
                  <p className="text-gray-900 whitespace-pre-wrap">
                    {instrumentoDetalhes.observacoes_gerais || 'Nenhuma observação'}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-gray-900">{instrumentoDetalhes.diretoria}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Criado em</p>
                    <p className="text-gray-900">
                      {new Date(instrumentoDetalhes.created_at).toLocaleString('pt-BR')}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Atualizado em</p>
                    <p className="text-gray-900">
                      {new Date(instrumentoDetalhes.updated_at).toLocaleString('pt-BR')}
                    </p>
                  </div>
                </div>
              </div>
  ...
  const renderModalProjetoInfoCompleta = () => {
    ...
    return (
      <Dialog open={modalProjetoInfoCompletaOpen} onOpenChange={setModalProjetoInfoCompletaOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-sm font-normal text-gray-500">
                  {projetoDetalhes.codigo} • {statusProjetoLabels[projetoDetalhes.status]}
                </span>
              </div>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-blue-50 px-4 py-3 flex items-center gap-2">
                <FileText className="h-4 w-4 text-blue-600" />
                <span className="font-semibold text-blue-900">Identificação do Projeto</span>
              </div>
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Nome</p>
                    <p className="text-gray-900 font-medium">{projetoDetalhes.nome}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Código</p>
                    <p className="text-gray-900 font-mono">{projetoDetalhes.codigo}</p>
                  </div>
                </div>
                {projetoDetalhes.descricao_sintetica && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Descrição Sintética</p>
                    <p className="text-gray-900">{projetoDetalhes.descricao_sintetica}</p>
                  </div>
                )}
                {projetoDetalhes.objetivo && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Objetivo</p>
                    <p className="text-gray-900 whitespace-pre-wrap">{projetoDetalhes.objetivo}</p>
                  </div>
                )}
                {projetoDetalhes.contexto_justificativa && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Contexto / Justificativa</p>
                    <p className="text-gray-900 whitespace-pre-wrap">{projetoDetalhes.contexto_justificativa}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-purple-50 px-4 py-3 flex items-center gap-2">
                ...
                <span className="font-semibold text-purple-900">Governança e Responsáveis</span>
              </div>
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Patrocinador</p>
                    <p className="text-gray-900">{projetoDetalhes.patrocinador_nome || 'Não definido'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Gestor</p>
                    <p className="text-gray-900">{projetoDetalhes.gestor_nome || 'Não definido'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Diretoria</p>
                    <p className="text-gray-900">{projetoDetalhes.diretoria || 'Não definido'}</p>
                  </div>
                </div>
                {projetoDetalhes.areas_execucao_diretorias && (
                  <div className="mt-4">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Áreas de Execução</p>
                    <p className="text-gray-900">{projetoDetalhes.areas_execucao_diretorias}</p>
                  </div>
                )}
                {projetoDetalhes.instrumentos_nomes && (
                  <div className="mt-4">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Ancoragem Estratégica</p>
                    <p className="text-gray-900">{projetoDetalhes.instrumentos_nomes}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-green-50 px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-green-900">Temporalidade</span>
                </div>
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Início Previsto</p>
                      <p className="text-gray-900">
                        {projetoDetalhes.data_prevista_inicio
                          ? new Date(projetoDetalhes.data_prevista_inicio).toLocaleDateString('pt-BR')
                          : 'Não definido'}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Conclusão Prevista</p>
                      <p className="text-gray-900">
                        {projetoDetalhes.data_prevista_conclusao
                          ? new Date(projetoDetalhes.data_prevista_conclusao).toLocaleDateString('pt-BR')
                          : 'Não definido'}
                      </p>
                    </div>
                  </div>
                  <div>
                    ...
                    <Badge className={statusProjetoColors[projetoDetalhes.status]}>
              ...
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-orange-50 px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-orange-900">Classificação</span>
                </div>
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Prioridade</p>
                      <p className="text-gray-900">{prioridadeLabels[projetoDetalhes.prioridade] || projetoDetalhes.prioridade}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Complexidade</p>
                      <p className="text-gray-900">{complexidadeLabels[projetoDetalhes.complexidade] || projetoDetalhes.complexidade}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Haverá Contratação?</p>
                      <Badge className={projetoDetalhes.havera_contratacao ? 'bg-emerald-100 text-emerald-700 border-0' : 'bg-gray-100 text-gray-700 border-0'}>
                        {projetoDetalhes.havera_contratacao ? 'Sim' : 'Não'}
                      </Badge>
                    </div>
                    {projetoDetalhes.havera_contratacao && (
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Valor Estimado</p>
                        <p className="text-gray-900 font-medium">
                          {projetoDetalhes.valor_estimado_contratacao != null
                            ? Number(projetoDetalhes.valor_estimado_contratacao).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
                            : 'Não informado'}
                        </p>
                      </div>
                    )}
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-amber-50 px-4 py-3 flex items-center gap-2">
                ...
                <span className="font-semibold text-amber-900">Execução e Saúde</span>
              </div>
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Saúde do Projeto</p>
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${projetoDetalhes.saude === 'verde' ? 'bg-green-500' :
                        projetoDetalhes.saude === 'amarelo' ? 'bg-yellow-500' : 'bg-red-500'
                        }`} />
                      <span className="font-medium">{saudeLabels[projetoDetalhes.saude] || projetoDetalhes.saude}</span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${projetoDetalhes.progresso_percentual}%` }}
                        />
                      </div>
                      <span className="text-sm font-bold">{projetoDetalhes.progresso_percentual}%</span>
                    </div>
                  </div>
                </div>
                {projetoDetalhes.saude_justificativa && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Justificativa da Saúde</p>
                    <p className="text-gray-900">{projetoDetalhes.saude_justificativa}</p>
                  </div>
                )}
                {projetoDetalhes.saude_ultima_revisao && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Última Revisão da Saúde</p>
                    <p className="text-gray-900">{new Date(projetoDetalhes.saude_ultima_revisao).toLocaleString('pt-BR')}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* SEÇÃO: ESCOPO */}
            {(projetoDetalhes.escopo_sintetico || projetoDetalhes.fora_do_escopo) && (
              <div className="border rounded-lg overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  {projetoDetalhes.escopo_sintetico && (
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Escopo Sintético</p>
                      <p className="text-gray-900 whitespace-pre-wrap">{projetoDetalhes.escopo_sintetico}</p>
                    </div>
                  )}
                  {projetoDetalhes.fora_do_escopo && (
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Fora do Escopo</p>
                      <p className="text-gray-900 whitespace-pre-wrap">{projetoDetalhes.fora_do_escopo}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {(projetoDetalhes.ancoragem_estrategica_plano_gestao
              ...
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-indigo-50 px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-indigo-900">Ancoragem Estratégica</span>
                </div>
                <div className="p-4">
                  <div className="flex flex-wrap gap-2">
                    {projetoDetalhes.ancoragem_estrategica_plano_gestao && (
                      <Badge className="bg-indigo-100 text-indigo-700 border-0">Plano de Gestão</Badge>
                    )}
                    {projetoDetalhes.ancoragem_estrategica_pep && (
                      <Badge className="bg-indigo-100 text-indigo-700 border-0">PEP</Badge>
                    )}
                    {projetoDetalhes.ancoragem_estrategica_programa_x && (
                      <Badge className="bg-indigo-100 text-indigo-700 border-0">Programa X</Badge>
                    )}
                  </div>
                </div>
              </div>
            ...
            {projetoDetalhes.tap_id && (
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-rose-50 px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-rose-900">TAP — Termo de Abertura</span>
                </div>
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Versão</p>
                      <p className="text-gray-900 font-mono">v{projetoDetalhes.tap_versao || 1}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Gerado em</p>
                      <p className="text-gray-900">
                        {projetoDetalhes.tap_gerado_em
                          ? new Date(projetoDetalhes.tap_gerado_em).toLocaleString('pt-BR')
                          : '-'}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Identificador</p>
                      <p className="text-gray-900 font-mono text-sm">{projetoDetalhes.tap_id}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
                    <div className={`rounded-md border px-3 py-2 ${projetoDetalhes.tap_validado_gestor_em ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50 border-gray-200'}`}>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Gestor</p>
                      <p className={`text-sm font-medium ${projetoDetalhes.tap_validado_gestor_em ? 'text-emerald-700' : 'text-gray-500'}`}>
                        {projetoDetalhes.tap_validado_gestor_em
                          ? new Date(projetoDetalhes.tap_validado_gestor_em).toLocaleString('pt-BR')
                          : 'Pendente'}
                      </p>
                    </div>
                    <div className={`rounded-md border px-3 py-2 ${projetoDetalhes.tap_validado_diretor_em ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50 border-gray-200'}`}>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Diretor</p>
                      <p className={`text-sm font-medium ${projetoDetalhes.tap_validado_diretor_em ? 'text-emerald-700' : 'text-gray-500'}`}>
                        {projetoDetalhes.tap_validado_diretor_em
                          ? new Date(projetoDetalhes.tap_validado_diretor_em).toLocaleString('pt-BR')
                          : 'Pendente'}
                      </p>
                    </div>
                    <div className={`rounded-md border px-3 py-2 ${projetoDetalhes.tap_validado_patrocinador_em ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50 border-gray-200'}`}>
                      <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Patrocinador</p>
                      <p className={`text-sm font-medium ${projetoDetalhes.tap_validado_patrocinador_em ? 'text-emerald-700' : 'text-gray-500'}`}>
                        {projetoDetalhes.tap_validado_patrocinador_em
                          ? new Date(projetoDetalhes.tap_validado_patrocinador_em).toLocaleString('pt-BR')
                          : 'Pendente'}
                      </p>
                    </div>
                  </div>
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 flex items-center gap-2">
                ...
                <span className="font-semibold text-gray-900">Formalização e Observações</span>
              </div>
              <div className="p-4 space-y-3">
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">TAP Vinculado</p>
                  <p className="text-gray-900">{projetoDetalhes.tap_vinculado || 'Não definido'}</p>
                </div>
                {projetoDetalhes.observacoes_gerais && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Observações Gerais</p>
                    <p className="text-gray-900 whitespace-pre-wrap">{projetoDetalhes.observacoes_gerais}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="border rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Cadastrado em</p>
                    <p className="text-gray-900 text-sm">
                      {projetoDetalhes.created_at
                        ? new Date(projetoDetalhes.created_at).toLocaleString('pt-BR')
                        : '-'}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Última Atualização</p>
                    <p className="text-gray-900 text-sm">
                      {projetoDetalhes.updated_at
                        ? new Date(projetoDetalhes.updated_at).toLocaleString('pt-BR')
                        : '-'}
                    </p>
                  </div>
                </div>
  ...
  const renderConteudoPrincipal = () => {
    if (loadingProjeto) {
      return <div className="text-center py-12 text-gray-500">Carregando projeto...</div>;
    }
  ...
  return (
    <div className="flex gap-6">
      ...
      <Dialog open={tapDialogOpen} onOpenChange={setTapDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          {tapDialogProjeto && (() => {
            ...
            const isGestor = currentUserId && tapDialogProjeto.gestor_user_id === currentUserId;
            const isPatrocinador = currentUserId && tapDialogProjeto.patrocinador_user_id === currentUserId;
              ...
              <div className="space-y-4">
                ...
                {tap.label === 'Pendente' && (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <p className="text-sm text-gray-700">Projeto com pendências de informações no cadastro.</p>
                  </div>
                )}

                {tap.label === 'Proposta' && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-3">
                    <p className="text-sm text-gray-700">Projeto pendente de validação pelo gestor.</p>
                    {isGestor && (
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => { generateTAPPdf(tapDialogProjeto); }}>
                ...
                {tap.label === 'Validado 1/3' && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                    <p className="text-sm text-gray-700">Projeto pendente de validação pelo diretor.</p>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => { generateTAPPdf(tapDialogProjeto); }}>
                        <Eye className="h-3 w-3 mr-1" /> Visualizar TAP
                      </Button>
                ...
                {tap.label === 'Validado 2/3' && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                    <p className="text-sm text-gray-700">Projeto pendente de validação pelo patrocinador.</p>
                    {isPatrocinador && (
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => { generateTAPPdf(tapDialogProjeto); }}>
                          <Eye className="h-3 w-3 mr-1" /> Visualizar TAP
      ...
      <Dialog open={tepDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-md">
          ...
          {tepDialogProjeto && (() => {
            ...
            const isGestor = currentUserId && tepDialogProjeto.gestor_user_id === currentUserId;
            ...
            return (
              <div className="space-y-4">
                ...
                {tep.label === 'Não iniciado' && (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <p className="text-sm text-gray-700">
                      TEP ainda não foi iniciado. Acesse o projeto para gerar o termo de encerramento.
                    </p>
                  </div>
                )}
                ...
                {tep.label === 'Em validação' && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-3">
                    <p className="text-sm text-gray-700">TEP pendente de validação pelo gestor.</p>
                    {isGestor && (
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={visualizarTepPdf}>
                ...
                {tep.label === 'Validado 1/3' && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                    <p className="text-sm text-gray-700">TEP pendente de validação pelo diretor.</p>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={visualizarTepPdf}>
                        <Eye className="h-3 w-3 mr-1" /> Visualizar TEP
                      </Button>
                ...
                {tep.label === 'Validado 2/3' && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
                    <p className="text-sm text-gray-700">TEP pendente de validação pelo patrocinador.</p>
                    {isPatrocinador && (
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={visualizarTepPdf}>
                          <Eye className="h-3 w-3 mr-1" /> Visualizar TEP
