...
export function SprintStatsCards({ executionData }: SprintStatsCardsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const backlog = filteredData.filter(
      (item) => item.backlogTasks && item.backlogTasks.trim() !== "",
    ).length;

    // Em Fila = tarefas com status "FORA_SPRINT"
    const emFila = filteredData.filter(
      (item) => item.sprintStatus === "FORA_SPRINT",
    ).length;

    // Concluído = tarefas com progress "FEITO"
    const concluido = filteredData.filter(
      (item) => item.progress === "FEITO",
    ).length;

    // Sprint Atual = tarefas com status "SPRINT_ATUAL"
    const sprintAtual = filteredData.filter(
      (item) => item.sprintStatus === "SPRINT_ATUAL",
    ).length;
