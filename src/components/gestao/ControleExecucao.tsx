...
export function ControleExecucao() {
  ...
  // ============================================================
  const [draggedItem, setDraggedItem] = useState<{ planProgram: string; index: number } | null>(null);
  const [dragOverTarget, setDragOverTarget] = useState<{ planProgram: string; index: number } | null>(null);
  ...
  const handleDragStart = (e: React.DragEvent, planProgram: string, index: number) => {
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/plain', JSON.stringify({ planProgram, index }));
    setTimeout(() => setDraggedItem({ planProgram, index }), 0);
  };

  const handleDragOver = (e: React.DragEvent, planProgram: string, index: number) => {
    e.preventDefault();
  ...
  const handleDrop = async (e: React.DragEvent, targetPlanProgram: string, targetIndex: number) => {
    e.preventDefault();
    e.stopPropagation();
                  ...
                  <table className="w-full min-w-[800px]">
                    <thead className="bg-gray-100 border-b-2 border-[#2d7a5e]">
                      <tr>
                        ...
                        <th className="text-left p-2 lg:p-4 font-semibold text-xs lg:text-sm text-gray-700">KR / PROJETO</th>
                        <th className="text-left p-2 lg:p-4 font-semibold text-xs lg:text-sm text-gray-700">TAREFAS PLANEJADAS (BACKLOG)</th>
                        <th className="text-center p-2 lg:p-4 font-semibold text-xs lg:text-sm text-gray-700 w-32">STATUS</th>
                        <th className="text-left p-2 lg:p-4 font-semibold text-xs lg:text-sm text-gray-700">TAREFAS DA SPRINT ATUAL</th>
                        <th className="text-center p-2 lg:p-4 font-semibold text-xs lg:text-sm text-gray-700 w-32">PROGRESSO</th>
                        {canEdit && <th className="text-center p-2 lg:p-4 font-semibold text-xs lg:text-sm text-gray-700 w-24 lg:w-32">AÇÕES</th>}
                      </tr>
                    </thead>
                    <tbody className="bg-white">
                      {items.map((item, index) => {
                        const isDragging = draggedItem?.planProgram === planProgram && draggedItem?.index === index;
                        const isDragOver = dragOverTarget?.planProgram === planProgram && dragOverTarget?.index === index;
                        
                        return (
                          <tr 
                            key={item.id} 
                            draggable={canEdit}
                            onDragStart={canEdit ? (e) => handleDragStart(e, planProgram, index) : undefined}
                            onDragOver={canEdit ? (e) => handleDragOver(e, planProgram, index) : undefined}
                            onDragLeave={canEdit ? handleDragLeave : undefined}
                            onDrop={canEdit ? (e) => handleDrop(e, planProgram, index) : undefined}
                          ...
                          >
                            ...
                            <td className="p-2 lg:p-4 text-xs lg:text-sm text-gray-800 font-medium">{item.krProjectInitiative}</td>
                            <td className="p-2 lg:p-4 text-xs lg:text-sm text-gray-600">{item.backlogTasks || '-'}</td>
                            <td className="p-2 lg:p-4 text-center">{getStatusBadge(item.sprintStatus)}</td>
                            <td className="p-2 lg:p-4 text-xs lg:text-sm text-gray-600">{item.sprintTasks || '-'}</td>
