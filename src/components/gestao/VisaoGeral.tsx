...
export function VisaoGeral() {
  ...
  const stats = useMemo(() => {
    ...
    const emAndamento = filteredKeyResults.filter(
      (kr) => kr.status === "EM_ANDAMENTO",
    ).length;
    const aIniciar = filteredKeyResults.filter(
  ...
  const statusPorFase = useMemo(() => {
    ...
    return fases.map((fase, index) => {
      ...
      return {
        ...
        emAndamento: krsNaFase.filter((kr) => kr.status === "EM_ANDAMENTO")
          .length,
        naoIniciado: krsNaFase.filter((kr) => kr.status === "NAO_INICIADO")
          .length,
      };
    });
  }, [filteredKeyResults]);
  ...
  const iniciativasPorObjetivo = useMemo(() => {
    return filteredObjectives.map((obj) => {
      ...
      const count = filteredInitiatives.filter((init) =>
        krIds.includes(init.keyResultId),
      ).length;
      return {
        name: obj.code,
        value: count,
  ...
  const iniciativasPorFase = useMemo(() => {
    ...
    return fases.map((fase, index) => ({
      name: fase,
      value:
        Math.floor(filteredInitiatives.length / 5) +
        (index === 0 ? filteredInitiatives.length % 5 : 0),
    }));
  }, [filteredInitiatives]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 xl:gap-5 2xl:gap-6">
        <GraficoBarras
          ...
          xAxisKey="name"
          dataKeys={[{ key: "value", name: "Iniciativas", color: "#3b82f6" }]}
        />
        <GraficoBarras
          title="Iniciativas por Fase"
          data={iniciativasPorFase}
          xAxisKey="name"
          dataKeys={[{ key: "value", name: "Iniciativas", color: "#8b5cf6" }]}
        />
      </div>
    </div>
  );
