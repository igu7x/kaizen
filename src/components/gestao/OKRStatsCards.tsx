...
export function OKRStatsCards() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 xl:gap-3 2xl:gap-4 w-full h-full">
      <CardIndicador title="Totais" value={stats.total} icon={Target} color="blue" />
      <CardIndicador title="Concluido" value={stats.concluido} icon={CheckCircle2} color="green" />
      <CardIndicador title="Em andamento" value={stats.emAndamento} icon={Clock} color="yellow" />
      <CardIndicador title="A iniciar" value={stats.aIniciar} icon={Play} color="orange" />
    </div>
  );
}