      ...
      <CardContent className="p-3 lg:p-4 xl:p-5 h-full flex flex-col justify-center">
        <div className="flex items-center justify-between gap-2">
          <div className={cn('min-w-0', textClasses[color])}>
            <div className="text-2xl lg:text-3xl 2xl:text-4xl font-bold leading-none tracking-tight">{value}</div>
            <div className="text-xs lg:text-sm font-medium capitalize mt-1 opacity-90">{title}</div>
          </div>
          <div className="flex-shrink-0">
            <Icon className={cn('h-6 w-6 lg:h-7 lg:w-7 2xl:h-8 2xl:w-8 opacity-80', textClasses[color])} strokeWidth={1.5} />
          </div>
        </div>
      </CardContent>
    </Card>
