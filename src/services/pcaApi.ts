 ...
 */
export async function getPcaItems(ano?: number, diretoria?: string): Promise<PcaItem[]> {
 ...
 */
export async function getPcaStats(ano?: number, diretoria?: string): Promise<PcaStats> {
    const params = new URLSearchParams();
    if (ano) params.set('ano', String(ano));
    if (diretoria) params.set('diretoria', diretoria);
