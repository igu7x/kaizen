...
export const getApiBaseUrl = (): string => {
    ...
    if (appEnvRaw === 'production') {
        const url = (import.meta.env.VITE_API_URL_PRODUCTION || 'https://kaizen-api.tjgo.jus.br').toString().trim();
        console.log('[API] Ambiente via VITE_APP_ENV=production ->', url);
        return url;
    }
    ...
    if (hostname.includes('tjgo.jus.br')) {
        const url = (import.meta.env.VITE_API_URL_PRODUCTION || 'https://kaizen-api.tjgo.jus.br').toString().trim();
        console.log('[API] Ambiente: PRODUÇÃO ->', url);
        return url;
    }
    ...
    async request<T>(
        ...
        if (devEnv && devEnv !== 'null') {
            ...
            if (parsed && typeof parsed === 'string') {
                ...
                if ((finalEndpoint.startsWith('/api/areas') || finalEndpoint.startsWith('/api/pessoas') || finalEndpoint.startsWith('/api/users'))
        ...
        try {
            if (options?.method && options.method !== 'GET') {
                console.log(`[API] ${options.method} ${url} (tentativa ${retryCount + 1})`);
            }
