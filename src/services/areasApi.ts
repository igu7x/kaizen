...
export async function getByDominio(dominio: string): Promise<Area[]> {
  return apiClient.request<Area[]>(`${BASE_URL}?dominio=${encodeURIComponent(dominio)}`);
}
 ...
 */
export async function updateOrdenacao(ordenacao: { id: number; ordem_linha: number; ordem_posicao: number }[]): Promise<void> {
...
export async function getForSelect(): Promise<{ id: number; nome: string }[]> {
  return apiClient.request<{ id: number; nome: string }[]>(`${BASE_URL}/select`);
}
 ...
 */
export async function getAllUnidades(dominio?: string): Promise<(Unidade & { area_nome?: string; area_sigla?: string })[]> {
  const url = dominio ? `${BASE_URL}/unidades/all?dominio=${encodeURIComponent(dominio)}` : `${BASE_URL}/unidades/all`;
  return apiClient.request<(Unidade & { area_nome?: string; area_sigla?: string })[]>(url);
}
 ...
 */
export async function createUnidade(areaId: number, dto: CreateUnidadeDto): Promise<Unidade> {
  return apiClient.request<Unidade>(`${BASE_URL}/${areaId}/unidades`, {
    method: 'POST',
    body: JSON.stringify(dto),
  });
 ...
 * Atualizar unidade
 */
export async function updateUnidade(id: number, dto: UpdateUnidadeDto): Promise<Unidade> {
...
export async function getUnidadeUsuarios(unidadeId: number): Promise<UnidadeUsuarios> {
  return apiClient.request<UnidadeUsuarios>(`${BASE_URL}/unidades/${unidadeId}/usuarios`);
}

/**
 * Reordenar unidades (trocar posições)
 */
export async function reorderUnidades(areaId: number, ordenacao: { id: number; ordem: number }[]): Promise<void> {
