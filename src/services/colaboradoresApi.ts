...
export const getEstatisticas = async (
  ...
  const params = diretoria ? `?diretoria=${diretoria}` : "";
  return apiClient.get<Estatisticas>(
    `/api/colaboradores/estatisticas${params}`,
  );
};
...
export const getSubordinados = async (gestorId: number): Promise<any[]> => {
  return apiClient.get<any[]>(
    `/api/colaboradores/organograma/subordinados/${gestorId}`,
  );
};
 ...
 */
export const getGestoresPorLinha = async (
  linha: number,
  diretoria?: string,
): Promise<any[]> => {
  const params = diretoria ? `?diretoria=${diretoria}` : "";
  return apiClient.get<any[]>(
    `/api/colaboradores/organograma/linha/${linha}${params}`,
  );
};
 ...
 */
export const getPossiveisPais = async (
  linha: number,
  diretoria?: string,
): Promise<any[]> => {
  const params = diretoria ? `?diretoria=${encodeURIComponent(diretoria)}` : "";
  return apiClient.get<any[]>(
    `/api/colaboradores/organograma/possiveis-pais/${linha}${params}`,
  );
};
