...
export function GestaoProvider({ children }: { children: ReactNode }) {
  ...
  const [objectives, setObjectives] = useState<Objective[]>(() =>
    Storage.load("gestao_objectives", []),
  );
  const [keyResults, setKeyResults] = useState<KeyResult[]>(() =>
    Storage.load("gestao_keyResults", []),
  );
  const [initiatives, setInitiatives] = useState<Initiative[]>(() =>
    Storage.load("gestao_initiatives", []),
  );
  const [programs, setPrograms] = useState<Program[]>(() =>
    Storage.load("gestao_programs", []),
  );
  const [programInitiatives, setProgramInitiatives] = useState<
    ProgramInitiative[]
  >(() => Storage.load("gestao_programInitiatives", []));
  const [executionControls, setExecutionControls] = useState<
    ExecutionControl[]
  >(() => Storage.load("gestao_executionControls", []));
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      const [objData, krData, initData, progData, progInitData, execData] =
        await Promise.all([
      ...
      const cachedObjectives = Storage.load<Objective[]>(
        "gestao_objectives",
        [],
      );
      const cachedKeyResults = Storage.load<KeyResult[]>(
        "gestao_keyResults",
        [],
      );
      const cachedInitiatives = Storage.load<Initiative[]>(
        "gestao_initiatives",
        [],
      );
      const cachedPrograms = Storage.load<Program[]>("gestao_programs", []);
      const cachedProgramInitiatives = Storage.load<ProgramInitiative[]>(
        "gestao_programInitiatives",
        [],
      );
      const cachedExecutionControls = Storage.load<ExecutionControl[]>(
   ...
   */
  const updateKeyResultOptimistic = useCallback(
    async (id: string, updates: Partial<KeyResult>) => {
   ...
   */
  const updateInitiativeOptimistic = useCallback(
    async (id: string, updates: Partial<Initiative>) => {
