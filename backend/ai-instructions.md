...
- **DB & Migrations:** PostgreSQL. Exige-se: JSONB (mapeado via Hypersistence Optimizer se necessário), enums nativos e índices estratégicos. Gerenciamento de schema estritamente via **Flyway** ou **Liquibase**. Migrations devem ser seguras (Zero Downtime). UUIDs (v7 preferencialmente) para IDs expostos na API/URL; IDs sequenciais (`Long`/`BigSerial`) internos apenas para chaves primárias físicas e performance de indexação.
...
4. **Segurança:** O endpoint está devidamente protegido por `@PreAuthorize`? Há risco de ID enumeration (Insecure Direct Object Reference - IDOR)?
